...
} from "../../../listeners/actions/proposal-button.js";
import {
  buildProposalBlocks,
  PROPOSAL_ACTION_ID,
...
describe("buildProposalBlocks", () => {
  ...
  it("escapes agent-authored descriptions", () => {
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, description: "Run <!channel> now" },
    ]);
    const text = /** @type {any} */ (blocks[0]).text.text;
    assert.ok(
      !text.includes("<!channel>"),
      "a forged mention must not survive"
    );
  ...
  it("prefers the SERVER-sent button label over its own table", () => {
    ...
    // bot deploy.
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, operation: "some_new_op", buttonLabel: "Do the thing" },
    ]);
    assert.strictEqual(
      /** @type {any} */ (blocks[0]).accessory.text.text,
      "Do the thing"
    );
  });

  it("falls back to its own table, then to a neutral verb, for an older server", () => {
    const known = buildProposalBlocks([
      { ...PROPOSAL, buttonLabel: undefined },
    ]);
    assert.strictEqual(
      /** @type {any} */ (known[0]).accessory.text.text,
      "Run it"
    );

    const unknown = buildProposalBlocks([
      { ...PROPOSAL, operation: "some_new_op", buttonLabel: undefined },
    ]);
    // Honest rather than wrong: we do not know what this op does.
    assert.strictEqual(
      /** @type {any} */ (unknown[0]).accessory.text.text,
      "Approve"
    );
  });
  ...
  it("keeps the confirm label inside Slack's 30-character allowance", () => {
    ...
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, buttonLabel: "x".repeat(70) },
    ]);
    const accessory = /** @type {any} */ (blocks[0]).accessory;
    assert.ok(accessory.text.text.length <= 75);
    assert.ok(
      accessory.confirm.confirm.text.length <= 30,
      accessory.confirm.confirm.text.length
    );
  });
  ...
  it("says where an external action goes instead of claiming a quota cost", () => {
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, confirmSeverity: "external" },
    ]);
    const section = /** @type {any} */ (blocks[0]).text.text;
  ...
  it("keeps the cost warning in the section for everything else", () => {
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, confirmSeverity: undefined },
    ]);
  ...
  it("falls back to the default copy for a severity it does not recognise", () => {
    ...
    const blocks = buildProposalBlocks([
      { ...PROPOSAL, confirmSeverity: /** @type {any} */ ("nuclear") },
    ]);
    const text = /** @type {any} */ (blocks[0]).accessory.confirm.text.text;
...
describe("rendersRunProposalFor", () => {
  ...
  it("suppresses only the suite a targeted proposal is actually about", () => {
    ...
    assert.strictEqual(
      rendersRunProposalFor(
        [{ ...run, target: { type: "eval_suite", selector: "ts_1" } }],
        suite
      ),
      true
    );
  ...
  it("matches a name selector case-insensitively — models propose by name", () => {
    ...
    assert.strictEqual(
      rendersRunProposalFor([byName], { ...suite, name: "Checkout" }),
      false
    );
  });
  ...
  it("is false for a run proposal pushed past the block cap", () => {
    ...
    assert.ok(
      !rendered.includes("run"),
      "fixture must push the run proposal past the cap"
    );
  ...
  it("is false for a run proposal with no action id — those are skipped too", () => {
    assert.strictEqual(
      rendersRunProposalFor([{ operation: "run_eval_suite" }], suite),
      false
    );
  });
...
describe("announcementFor", () => {
  it("words the announcement from the KIND the server sent", () => {
    ...
    assert.match(
      announcementFor({ operation: "x", kind: "start" }, "U1"),
      /it's away/
    );
    assert.match(
      announcementFor({ operation: "x", kind: "cancel" }, "U1"),
      /Cancelled by <@U1>/
    );
    assert.match(
      announcementFor({ operation: "x", kind: "generate" }, "U1"),
      /being generated/
    );
    // Nothing started — saying otherwise would have the user watching for a run
    // that will not appear until the next interval.
    const scheduled = announcementFor(
      { operation: "x", kind: "schedule" },
      "U1"
    );
    assert.match(scheduled, /schedule is updated/);
    assert.ok(!/away/.test(scheduled));
    assert.match(
      announcementFor({ operation: "x", kind: "external" }, "U1"),
      /the tool ran/
    );
  });
  ...
  it("falls back to operation names for a server that predates `kind`", () => {
    assert.match(
      announcementFor({ operation: "cancel_eval_run" }, "U1"),
      /Cancelled by/
    );
    assert.match(
      announcementFor({ operation: "run_eval_suite" }, "U1"),
      /it's away/
    );
  });
  ...
  it("claims nothing for an operation and kind it has never seen", () => {
    const text = announcementFor(
      { operation: "some_new_op", kind: "teleport" },
      "U1"
    );
  ...
  it("does not let an UNKNOWN kind fall through to the operation-name table", () => {
    ...
    const text = announcementFor(
      { operation: "run_eval_suite", kind: "teleport" },
      "U1"
    );
    assert.strictEqual(text, ":white_check_mark: Approved by <@U1>.");
...
describe("handleProposalButton", () => {
  ...
  function stub({
    executeStatus = 200,
    executeBody = { status: "succeeded", result: {} },
  } = {}) {
  ...
  it("executes as the CLICKER, not the proposer", async () => {
    ...
    assert.match(
      state.executeCalls[0].url,
      /\/projects\/p1\/proposed-actions\/act_1\/execute$/
    );
  ...
  it("refuses an unlinked clicker instead of spending", async () => {
    globalThis.fetch = mock.fn(async (url) => {
      ...
      if (path.endsWith("/slack/channel-bindings/get"))
        return json({ binding: null });
