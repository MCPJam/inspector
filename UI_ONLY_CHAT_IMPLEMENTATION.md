...
| `ui_navigation_rejected` | client | `ui-actions.ts` resolver | `reason` (`unknown`\|`hosted_blocked`\|`flag_off`); `segment` is emitted **only for known reasons** (`hosted_blocked`/`flag_off`, where it's a bounded manifest segment). For `unknown` — an arbitrary model-supplied target — `segment` collapses to the constant `"unrecognized"` so raw model text never reaches analytics. Doubles as capability-demand and flag-friction signal |
