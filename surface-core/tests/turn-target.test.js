...
function backendWith({ threadBinding = null, channelBinding = null, link = null } = {}) {
  return {
...
describe('turn target precedence', () => {
  it('a bound thread wins, and does not need the speaker to be linked', async () => {
    const resolve = createTurnTargetResolver({
      surfaceKind: 'test',
      backend: backendWith({
        threadBinding: { projectId: 'p_thread', organizationId: 'o1', initiatorActorId: 'U9' },
        channelBinding: { projectId: 'p_channel', organizationId: 'o1' },
        link: { organizationId: 'o1', defaultProjectId: 'p_user' },
      }),
    });
    const target = await resolve(CTX, { conversationId: 'C1', threadId: 'ts1' });
  ...
  it('does not run an UNLINKED speaker in a bound channel', async () => {
    ...
    const resolve = createTurnTargetResolver({
      surfaceKind: 'test',
      backend: backendWith({ channelBinding: { projectId: 'p_channel', organizationId: 'o1' } }),
    });
    assert.deepEqual(await resolve(CTX, { conversationId: 'C1' }), { mode: 'unlinked' });
  });
  ...
  it('falls back to the ORG default only when the user has none', async () => {
    const withUserDefault = createTurnTargetResolver({
      surfaceKind: 'test',
      backend: backendWith({
        link: { organizationId: 'o1', defaultProjectId: 'p_user', orgDefaultProjectId: 'p_org' },
      }),
    });
    const chosen = await withUserDefault(CTX, { conversationId: 'C1' });
    assert.equal(chosen.projectId, 'p_user', 'the org default must never override a personal one');
    assert.equal(chosen.orgDefault, undefined);

    const withoutUserDefault = createTurnTargetResolver({
      surfaceKind: 'test',
      backend: backendWith({ link: { organizationId: 'o1', orgDefaultProjectId: 'p_org' } }),
    });
  ...
  it('a backend with no channel-binding support resolves normally', async () => {
    ...
    const resolve = createTurnTargetResolver({
      ...
      backend: {
        ...
        fetchAccountLink: async () => ({ organizationId: 'o1', defaultProjectId: 'p_user' }),
      },
    });
    assert.equal((await resolve(CTX, { conversationId: 'C1' })).projectId, 'p_user');
  });
