...
export async function runTurnForEvent(args) {
	...
	if (args.eventClaims?.hasClaimBackend?.()) {
		try {
			...
			// The durable claim is the guarantee across restarts. If its
			// backend is unavailable, release the local fast-path claim so a
			// retry can proceed once the backend is healthy again.
		...
		if (durable.outcome === "completed") {
			if (durable.resultEnvelope && args.onReplay) {
				try {
					...
					// Replaying the stored answer is delivery work. If delivery
					// fails, allow the next provider retry to attempt the replay
					// rather than suppressing it for the local TTL.
					dedupe.release(eventKey);
					throw error;
				}
			}
	...
	try {
		...
		if (!state.dispatched || isPreflightFailure(error)) {
			dedupe.release(eventKey);
			if (durable)
...
function isPreflightFailure(error) {
	return (
		error &&
		error.status === undefined &&
		["CONFIG", "NO_PROJECT", "UNAUTHORIZED"].includes(error.code)
	);
}
