...
type CheckoutSession = {
  ...
  links?: Array<{ type: string; text?: string; url: string }>;
};
...
function formatMinorAmount(amount: unknown, currency: string): string {
  ...
  return (
    <div className={containerClassName}>
      ...
      <Dialog
        open={checkoutOpen}
        onOpenChange={(open) => {
          if (!open && checkoutCallId != null) {
            respondToCheckout({ error: "Checkout canceled" });
            return;
          }
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="flex items-center justify-end gap-2">
            <Button
              variant="outline"
              onClick={() =>
                respondToCheckout({ error: "Simulated checkout failure" })
              }
