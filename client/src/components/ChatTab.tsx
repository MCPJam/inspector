...
import { TracingSidebar } from "./tracing/tracing-sidebar";
...
export function ChatTab({ serverConfigs, systemPrompt = "" }: ChatTabProps) {
  ...
  // Load sidebar visibility from localStorage, default to true
  const [isTracingSidebarVisible, setIsTracingSidebarVisible] = useState(() => {
    const saved = localStorage.getItem('chat-tracing-sidebar-visible');
    return saved !== null ? JSON.parse(saved) : true;
  });

  // Save sidebar visibility to localStorage when it changes
  useEffect(() => {
    localStorage.setItem('chat-tracing-sidebar-visible', JSON.stringify(isTracingSidebarVisible));
  }, [isTracingSidebarVisible]);
    ...
    return (
      <TooltipProvider>
        <ResizablePanelGroup direction="horizontal" className="h-screen">
          {/* Main Chat Area */}
          <ResizablePanel defaultSize={isTracingSidebarVisible ? 70 : 100} minSize={50}>
            <div className="flex flex-col h-full">
              <div className="flex-1 flex flex-col items-center justify-center px-4">
                ...
                >
                  {/* Traces Toggle Button - Above input */}
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setIsTracingSidebarVisible(!isTracingSidebarVisible)}
                          className="text-xs h-7 px-2"
                        >
                          <Glasses className="h-3 w-3 mr-1" />
                          {isTracingSidebarVisible ? "Hide" : "Show"} Traces
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        {isTracingSidebarVisible ? "Hide" : "Show"} tracing sidebar
                      </TooltipContent>
                    </Tooltip>
                  </div>
              ...
              {/* Remove the floating button from empty state */}
            </div>
          </ResizablePanel>
            ...
            <>
              ...
              <ResizablePanel defaultSize={30} minSize={20} maxSize={50}>
                <TracingSidebar
                  ...
                  isVisible={isTracingSidebarVisible}
                  onToggleVisibility={() => setIsTracingSidebarVisible(!isTracingSidebarVisible)}
                />
              </ResizablePanel>
            </>
          )}
          ...
          <div className="relative bg-background h-full overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 border-t border-border/50 bg-background/80 backdrop-blur-sm">
              <div className="max-w-4xl mx-auto p-4">
                {/* Traces Toggle Button - Positioned above input */}
                <div className="flex justify-end mb-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
