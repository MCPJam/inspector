...
export function TestTemplateEditor({
  ...
  const handleSave = async () => {
    ...
    if (!validateExpectedToolCalls(editForm.expectedToolCalls || [])) {
      toast.error(
        "Cannot save: Please fix invalid tool calls. Ensure at least one tool call exists, all tool names are filled, and no argument values are empty.",
      );
      return;
    }
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      {!areExpectedToolCallsValid
                        ? "Fix invalid tool calls: ensure at least one tool call exists, all tool names are filled, and no argument values are empty"
