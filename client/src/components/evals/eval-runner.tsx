...
export function EvalRunner({
  ...
  // Auto-generate tests when component mounts in inline mode
  useEffect(() => {
    const generateTests = async () => {
      if (!inline || !isAuthenticated || selectedServers.length === 0) {
        return;
      }

      // Only generate once if tests are still empty
      const hasEmptyTests =
        testCases.length === 1 &&
        testCases[0].query === "" &&
        testCases[0].expectedToolCalls.length === 0;

      if (!hasEmptyTests || isGenerating) {
        return;
      }
    ...
    generateTests();
  }, [inline, isAuthenticated, selectedServers, getAccessToken, selectedModel]);
