...
function longestTopLevelArray(payload: unknown): number | undefined {
  ...
  for (const value of Object.values(payload as Record<string, unknown>)) {
    if (
      Array.isArray(value) &&
      (longest === undefined || value.length > longest)
    ) {
...
export function evaluatePredicate(
  ...
  switch (predicate.type) {
    ...
    case "responseCloseTo": {
      if (
        typeof predicate.reference !== "string" ||
        typeof predicate.maxDistance !== "number" ||
        !Number.isFinite(predicate.maxDistance) ||
        predicate.maxDistance < 0 ||
        predicate.maxDistance > 1 ||
        (predicate.caseSensitive !== undefined &&
          typeof predicate.caseSensitive !== "boolean") ||
        (predicate.normalizeWhitespace !== undefined &&
          typeof predicate.normalizeWhitespace !== "boolean")
      )
        return evidenceError(
          predicate,
