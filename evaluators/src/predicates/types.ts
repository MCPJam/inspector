...
export const predicateUnion = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("responseCloseTo"),
    reference: z.string().min(1).max(MAX_RESPONSE_CLOSE_TO_CHARS),
    maxDistance: z.number().finite().min(0).max(1),
...
export const predicateSchema = predicateUnion.superRefine((value, ctx) => {
  if (
    value.type === "responseCloseTo" &&
    normalizeCloseToText(value.reference, value).length === 0
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["reference"],
      message: "reference must remain non-empty after normalization",
    });
  }
