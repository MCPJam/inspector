...
export const HP47_OAUTH_PROFILE_SEED: Record<string, HostConfigOAuthProfileV1> =
  {
    ...
    mcpjam: {
      ...
      oauthSpecVersion: {
        ...
        value: {
          basis: "constant",
          revisions: ["2025-03-26", "2025-06-18", "2025-11-25", "2026-07-28"],
        },
        source:
          "E1 — MCPJam implements FOUR revisions concurrently, one state machine each: sdk/src/oauth/state-machines/debug-oauth-2025-03-26.ts, -2025-06-18.ts, -2025-11-25.ts, -2026-07-28.ts, selected by sdk/src/oauth/state-machines/factory.ts.",
        capturedAt: CAPTURED,
      },
    ...
    "claude-code": {
      ...
      extensions: {
        ...
        cimdOmissions:
          "`scope` and `logo_uri` are absent from the published CIMD document.",
      },
    },
    ...
    goose: {
      ...
      oauthSpecVersion: {
        ...
        source:
          'E1 — behavioral floor: no revision constant exists in Goose or rmcp 2.2.0, but the OAuth layer performs RFC 9728 PRM discovery (rmcp-2.2.0 auth.rs ~L1096, "per SEP-985: Protected Resource Metadata first, then direct OAuth") with a path-aware AS ladder. PRM is a 2025-06-18 addition, so the OAuth layer is 2025-06-18-or-later. NOTE this is the OAUTH layer only — Goose is split-revision and its MCP layer hard-pins 2025-03-26 (see protocolVersionPinning).',
        capturedAt: CAPTURED,
      },
      protocolVersionPinning: {
        status: "verified",
      ...
      extensions: {
        evidenceClass:
          "E1 (local clone + dependency source + live CIMD fetch), E2 (probe)",
    ...
    codex: {
      ...
      oauthSpecVersion: {
        ...
        value: { basis: "behavioral", minimumRevision: "2025-06-18" },
        source:
          "E1 — behavioral floor: the literal 'oauth-protected-resource' appears NOWHERE in the Codex repo, so there is no constant to pin; discovery is 100% rmcp's RFC 9728 PRM + path-aware AS ladder (rmcp auth.rs ~L2238), proven end-to-end by codex-rs/rmcp-client/tests/mcp_2026_oauth_discovery.rs L54-56, L73-76. PRM is a 2025-06-18 addition, hence the floor. CONSERVATIVE: rmcp's ladder comment actually reads \"following the priority order in spec-2025-11-25 4.3\", which would support a 2025-11-25 floor; 2025-06-18 is recorded because that is the revision the observed mechanism unambiguously requires.",
        capturedAt: CAPTURED,
      },
      protocolVersionPinning: {
        status: "verified",
      ...
      extensions: {
        evidenceClass:
          "E1 (local clone + exact-version dependency source), E2 (probe)",
        oauthSpecVersionNotes:
    ...
    copilot: {
      ...
      oauthSpecVersion: {
        ...
        value: { basis: "behavioral", minimumRevision: "2025-06-18" },
        source:
          'E1 — behavioral floor: Microsoft never states an MCP authorization spec revision, but documents both as hard PREREQUISITES — "Your MCP server exposes OAuth 2.0 protected resource metadata at its .well-known/oauth-protected-resource endpoint (RFC 9728)" and "The authorization server publishes its metadata at the .well-known/oauth-authorization-server endpoint (RFC 8414), including a registration_endpoint, and supports dynamic client registration (RFC 7591)". Requiring RFC 9728 places Copilot at 2025-06-18-or-later. Scope: Microsoft 365 Copilot only — see extensions.scopeCorrection.',
        capturedAt: CAPTURED,
      },
      protocolVersionPinning: {
        status: "unverifiable",
    ...
    vscode: {
      ...
      oauthSpecVersion: {
        ...
        value: { basis: "behavioral", minimumRevision: "2025-06-18" },
        source:
          "E1 — behavioral floor, and the cleanest example of why this arm exists: NO MCP_AUTH_SPEC_VERSION constant or equivalent exists anywhere in microsoft/vscode, so there is nothing to pin — yet the behavior is unambiguous. RFC 9728 PRM discovery, path-aware (oauth.ts ~L1260), with a documented three-rung AS ladder (oauth.ts L1326-1339, implemented L1388/L1397/L1406); VS Code ENFORCES RFC 9728 client-side and hard-fails on mismatch (~L1241). PRM is a 2025-06-18 addition, hence the floor.",
        capturedAt: CAPTURED,
      },
      protocolVersionPinning: {
        status: "verified",
    ...
    cline: {
      ...
      oauthSpecVersion: {
        ...
        source:
          "E1 — Cline pins NO revision of its own; the bundled SDK 1.29.0 declares the list at src/types.ts L4-6, SUPPORTED_PROTOCOL_VERSIONS = [2025-11-25, 2025-06-18, 2025-03-26, 2024-11-05, 2024-10-07]. Recorded as the inherited set; see extensions for the dependency-version caveat.",
        capturedAt: CAPTURED,
      },
      protocolVersionPinning: {
        status: "verified",
