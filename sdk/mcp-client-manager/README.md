...
```ts
mcpClientManager.unsubscribeResource("everything", { uri: "file:///README.md" })
```
...
```ts
mcpClientManager.setElicitationHandler("everything", async (params) => ({ content: "ok" }))
```
...
```ts
mcpClientManager.setElicitationCallback(async ({ requestId, message, schema }) => ({ content: message }))
```
