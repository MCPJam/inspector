...
function setup(
  operation: (...args: any[]) => Promise<any> = async () => ({
    content: [],
    structuredContent: { count: 2 },
  })
) {
  const manager = new MCPClientManager();
  vi.spyOn(manager as any, "executeToolUnrecorded").mockImplementation(
    operation
  );
  return {
    manager,
    recorder: createEvalRecorder({ mcpClientManager: manager }),
  };
}
const trace = (result: any) =>
  result.trace as { messages: any[]; spans: any[] };
function deferred<T>() {
...
describe("eval recorder", () => {
  it("snapshots full inputs/results, preserves return values, and detaches results", async () => {
    ...
    expect(
      await recorder.runCase(
        { caseTitle: "coffee", caseId: "coffee" },
        async () => {
          const result = await manager.executeTool("amazon", "search", args);
          args.query = "changed";
          response.structuredContent.count = 99;
          return result;
        }
      )
    ).toBe(response);
    ...
    expect(result).toMatchObject({
      passed: true,
      caseId: "coffee",
      actualToolCalls: [{ toolName: "search", arguments: { query: "coffee" } }],
    });
    const t = trace(result);
    expect(t.messages[1].content[0].output.value).toMatchObject({
      content: response.content,
      structuredContent: { count: 2 },
      _meta: response._meta,
    });
    expect(t.messages[0].content[0].toolCallId).toBe(
      t.messages[1].content[0].toolCallId
    );
    expect(t.spans[0]).toMatchObject({ serverId: "amazon", status: "ok" });
    result.actualToolCalls![0].arguments!.query = "mutated snapshot";
    expect(recorder.getResults()[0].actualToolCalls![0].arguments!.query).toBe(
      "coffee"
    );
  });
  it("isolates concurrent cases and excludes other managers/outside calls", async () => {
    ...
    await Promise.all([
      recorder.runCase({ caseTitle: "first" }, async () => {
        await gate.promise;
        await manager.executeTool("one", "same", { id: 1 });
        await other.executeTool("other", "excluded");
      }),
      recorder.runCase({ caseTitle: "second" }, async () => {
        await manager.executeTool("two", "same", { id: 2 });
        gate.resolve();
      }),
    ]);
    for (const [title, id] of [
      ...
      expect(
        recorder.getResults().find((r) => r.caseTitle === title)
          ?.actualToolCalls
      ).toEqual([{ toolName: "same", arguments: { id } }]);
  });
  it("preserves callback errors and records caught tool failures independently", async () => {
    ...
    const { manager, recorder } = setup(async () => {
      throw toolError;
    });
    const assertion = new Error("expected 3, got 2");
    await expect(
      recorder.runCase({ caseTitle: "assertion" }, async () => {
        await expect(manager.executeTool("a", "broken")).rejects.toBe(
          toolError
        );
        throw assertion;
      })
    ).rejects.toBe(assertion);
    await recorder.runCase({ caseTitle: "expected error" }, async () => {
      await expect(manager.executeTool("a", "broken")).rejects.toBe(toolError);
    });
    expect(recorder.getResults().map((r) => r.passed)).toEqual([false, true]);
    expect(recorder.getResults()[0].error).toBe(assertion.message);
    expect(
      trace(recorder.getResults()[0]).messages[1].content[0].output
    ).toEqual({
      type: "error-json",
      value: { message: "tool broke", code: -32000 },
    });
  });
  it("retains isError without failing a callback and pairs repeated tools across servers", async () => {
    ...
    await recorder.runCase({ caseTitle: "negative" }, async () => {
      await manager.executeTool("a", "same");
      await manager.executeTool("b", "same");
    });
  ...
  it("records empty/repeated cases with unique IDs and rejects nested cases", async () => {
    ...
    await recorder.runCase(
      { caseTitle: "empty", caseId: "same" },
      () => undefined
    );
    expect(
      new Set(recorder.getResults().map((r) => r.externalIterationId)).size
    ).toBe(2);
    expect(recorder.getResults()[0]).toMatchObject({
      actualToolCalls: [],
      metadata: { captureCompleteness: "complete" },
    });
    await expect(
      recorder.runCase({ caseTitle: "outer" }, () =>
        recorder.runCase({ caseTitle: "inner" }, () => undefined)
      )
  ...
  it("rejects active snapshots and freezes incomplete unawaited calls", async () => {
    ...
    const run = recorder.runCase({ caseTitle: "unawaited" }, async () => {
      pending = manager.executeTool("a", "late");
    });
  ...
  it("bounds capture without changing execution and handles unserializable evidence", async () => {
    ...
    const recorder = createEvalRecorder({
      ...
      maxCapturedBytes: 300,
    });
    await recorder.runCase({ caseTitle: "large" }, async () => {
      expect(await manager.executeTool("a", "big")).toBe(response);
    });
    expect(recorder.getResults()[0]).toMatchObject({
      passed: true,
      metadata: { captureCompleteness: "unavailable" },
    });
    expect(recorder.getResults()[0].trace).toBeUndefined();
  });
  it("reports cyclic evidence as unavailable without masking the callback outcome", async () => {
    const cyclic: any = { content: [] };
    cyclic.self = cyclic;
    const { manager, recorder } = setup(async () => cyclic);
    const failure = new Error("assertion after call");
    await expect(
      recorder.runCase({ caseTitle: "cycle" }, async () => {
        expect(await manager.executeTool("a", "cycle")).toBe(cyclic);
        throw failure;
      })
  ...
  it("records task creation without inventing a completed task response", async () => {
    ...
    const { manager, recorder } = setup(async () => response);
    await recorder.runCase({ caseTitle: "task" }, () =>
      manager.executeTool("a", "task")
    );
    expect(
      trace(recorder.getResults()[0]).messages[1].content[0].output.value
    ).toEqual(response);
  });
  it("uploads stable tool evidence and iteration IDs through retry", async () => {
    const { manager, recorder } = setup();
    await recorder.runCase({ caseTitle: "report" }, () =>
      manager.executeTool("a", "search", { query: "coffee" })
    );
    const requests: any[] = [];
    vi.stubGlobal(
      "fetch",
    ...
    expect(
      requests[0].results[0].trace.messages[1].content[0].output.value
