...
| Preview build, fresh checkout | `npm run findings:preview -- --replay … --build`                                                  | **pass** — on a detached `git worktree` with its own `npm install --legacy-peer-deps` and **no SDK build**; the built page then rendered all 12 cases with zero page errors |
