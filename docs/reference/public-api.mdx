...
<Warning>
  The generation routes consume **no credits** — the model cost is on MCPJam.
  They are still metered two ways, by request COUNT, and the two mean
  different waits: a per-minute burst brake (`429`, retry in seconds) and your
  plan's daily generation quota (`429`, resets at UTC midnight). Both normally
  carry `Retry-After` — honor it rather than guessing, and fall back to your
  own backoff on the refusal that arrives without one.
</Warning>
