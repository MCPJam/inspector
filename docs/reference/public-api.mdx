...
<Note>
  **Swarms and User Testing are enabled per organization.** Everything in the
  ...
  — creating a persona or journey, launching a run, publishing a study,
  ...
  *reduces* exposure and spend: cancelling a journey run, unpublishing a
  study, and archiving a persona, journey or swarm (each one's `DELETE`). An
...
journey runs. A pending projection contributes to the unmapped count rather
