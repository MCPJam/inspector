...
| `PATCH .../eval-suites/{suiteId}` | Edit suite settings, including `environmentIds` — the project environments the suite runs against (non-empty array sets/replaces, `null` clears). Attach hosts via `hosts: [{ host, servers }]` where `host` is a host name or id and each entry in `servers` is a bound server name or `projectServerId`. Unknown top-level keys return `400 VALIDATION_ERROR` naming the field |
...
- **Strict write bodies.** The v1 eval-suite, eval-run, and host write routes
  reject unknown top-level keys with `400 VALIDATION_ERROR`. The error message
  names the offending field (e.g. `hostIds: Unrecognized key(s) in object`).
  Callers sending only the documented fields are unaffected. Previously,
  unknown keys were silently dropped — if you were relying on that behavior,
  update your request bodies to use only the documented fields.
