...
- **Manage unified share settings** — read the share envelope for a scenario,
  conformance run, or eval run; change who can open it; rotate the share link
...
**Unified share settings** (`/projects/{projectId}/shares/{resourceType}/{resourceId}`) — read and control who can open a shared scenario, conformance run, or eval run. `{resourceType}` is one of `scenario`, `conformanceRun`, or `evalRun`:

| Endpoint | What it does |
| --- | --- |
| `GET .../shares/{resourceType}/{resourceId}` | Read the full share envelope: mode, policy version, link token, and invited members |
| `PATCH .../shares/{resourceType}/{resourceId}` | Change the share mode (`project_members` / `invited_only` / `anyone_with_link`). `anyone_with_link` grants access to anyone holding the URL, including guests as browser sessions — not verified individuals |
| `POST .../shares/{resourceType}/{resourceId}/rotate-link` | Mint a new share URL and invalidate the old one. **Immediate and irreversible**: everyone holding the old URL loses the ability to redeem it. Invited members keep their access. Use this when a link has leaked, not as routine hygiene |

<Note>
  Share member upsert and delete (`PUT`/`DELETE .../shares/{resourceType}/{resourceId}/members/...`) remain REST-only and are not available through the SDK or agent.
</Note>
