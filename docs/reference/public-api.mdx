...
`scenario` is now **study** on this API. The paths, the operation names, the SDK
types and the `scenarioId` response field all moved:
...
**Every old path still works** and is unchanged, including its `scenarioId`
response spelling. Each one answers with `Deprecation: true` and a `Link` header
naming its successor. They are undocumented on purpose — the spec is what a new
