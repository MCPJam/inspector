...
- **Check host compatibility** — get a per-host verdict (`works` / `degraded` / `blocked` / `unknown`) for Claude, ChatGPT, Cursor, Copilot, Codex, Goose, Mistral, n8n, Perplexity, and Cline, with specific findings explaining each result
...
| `POST .../check-host-compatibility` | Check whether the server's tools and widgets work on each AI host (Claude, ChatGPT, Cursor, Copilot, Codex, Goose, Mistral, n8n, Perplexity, Cline); returns a per-host verdict (`works` / `degraded` / `blocked` / `unknown`) with specific findings |
