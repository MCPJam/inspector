...
<Check>
A `200` with the server's tool list confirms three things at once: the key is
valid, its organization can reach that project and server, and MCPJam
connected to the server. `401 UNAUTHORIZED` points at the key;
`401 OAUTH_REQUIRED` points at the target MCP server instead — see
[Errors](#errors).
</Check>
...
### Which part of the API do you need?

<CardGroup cols={2}>
  <Card title="Discover and configure" icon="folder" href="#discover-and-configure">
    Find the IDs everything else takes, and manage the hosts and environments your runs execute against.
  </Card>
  <Card title="Operate MCP servers" icon="server" href="#operate-mcp-servers">
    Validate, diagnose, and drive a saved server: tools, prompts, resources, and OAuth tokens.
  </Card>
  <Card title="Author and run evals" icon="flask-conical" href="#author-and-run-evals">
    Create suites, launch runs, poll results, and ingest results produced outside the platform.
  </Card>
  <Card title="Swarms and user testing" icon="users" href="#swarms-and-user-testing">
    Personas, journeys, published scenarios, and what the sessions revealed.
  </Card>
</CardGroup>
...
Start here: every ID the rest of the API takes is discoverable, and hosts and
environments define what a run executes against.
...
Everything under `/projects/{projectId}/servers/{serverId}/...` — one saved
server, connected on your behalf for the duration of the call.
...
Authoring, running, and reading results — plus the ingestion routes the SDK
uses for runs executed outside the platform, and the headless agent turn that
can author suites for you.
...
Synthetic sessions you author and launch, environments you publish to real
visitors, and the analytics over both.
...
A [project environment](#discover-and-configure) is launchable only through a suite that has
