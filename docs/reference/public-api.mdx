...
1. Open [**Settings → API keys**](https://app.mcpjam.com/settings/api-keys)
   ...
   directly).
2. Click **Create API key**, name it, pick the **organization** it will act in, and choose an **expiry** (7, 30, 60, 90, 180, or 365 days — default 90).
3. Copy the key (`sk_…`) immediately — **it is shown exactly once** and never
   stored by MCPJam in retrievable form.

Keys can be revoked from the same page at any time. Revocation takes effect immediately. Create a replacement key before the old one expires; the key list shows the expiry date and flags keys expiring within two weeks.
