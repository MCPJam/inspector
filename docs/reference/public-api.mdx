...
| `GET .../eval-runs/{runId}/iterations/{iterationId}/steps` | Per-step verdicts for one iteration: status, reason, and any evidence (screenshots, replay-video offset, widget tool calls). Returns `200` even when the trace blob cannot be loaded — step verdicts still return, and the top-level `evidence` field says whether the evidence read completed (`"resolved"`) or failed (`"unavailable"`). Absent on deployments that predate the field |
