...
<Note>
  **Swarms and User Testing are enabled per organization.** Everything in the
  ...
  Two operations are deliberately outside the gate, because both *reduce*
  exposure and spend: cancelling a journey run, and unpublishing a scenario. An
  organization that loses access can still stop what is already running.
