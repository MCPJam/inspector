...
| `PATCH .../eval-suites/{suiteId}/schedule` | Enable/disable scheduled runs; `environmentId` pins which environment they launch. **Enabling** answers `404` unless the deployment sets `MCPJAM_SCHEDULED_EVALS_WRITE_ENABLED=true`; disabling is always allowed, so a live schedule can be stopped anywhere |
