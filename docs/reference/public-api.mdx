...
| --- | --- |
| `GET /host-catalog` | The versioned host-compat catalog (market hosts, capability matrices, OpenAI-compat presets). Public and unauthenticated. Always returns a catalog — `source: "live"` when the backend is reachable, `source: "bundled"` as a fallback. |
