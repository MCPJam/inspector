...
The MCPJam API lets you run the inspector's live diagnostics and write
operations against MCP servers saved in your [hosted](/hosted/overview)
projects — from CI, scripts, or your own agents — without opening the UI.
...
- **Call a tool** — execute a tool and get the MCP result back directly
- **Render a prompt** — call `prompts/get` with arguments and receive the messages
- **Run eval suites** — start an async eval run and poll for status, per-iteration results, token usage, and traces
- **Import OAuth tokens** — push tokens your agent obtained to close the `OAUTH_REQUIRED` loop

All endpoints operate on servers you have already added to a project in the
hosted inspector. Managing keys is **not** part of the API — see [Not in the API yet](#not-in-the-api-yet).
...
**Requests.** Most v1 endpoints are `POST` and accept a JSON body. Read
endpoints for eval runs and iterations use `GET` with query parameters. List
endpoints take an optional `cursor`; `resources/read` requires a `uri`; write
endpoints require their specific fields (see [Endpoints](#endpoints)).
...
| `NOT_FOUND` | 404 | Unknown project, server, resource, eval run, or suite — also returned with `details.reason: "TRACE_NOT_AVAILABLE"` when a trace hasn't been stored for an iteration |
| `FEATURE_NOT_SUPPORTED` | 422 | The target server doesn't support this MCP capability |
| `RATE_LIMITED` | 429 | Too many requests — see [Rate limits](#rate-limits); also returned with `details.reason: "CONCURRENT_RUN_LIMIT"` when the per-caller eval concurrency cap is reached |
...
Server operation endpoints follow the pattern
`POST /projects/{projectId}/servers/{serverId}/...`. Eval endpoints are under
`/projects/{projectId}/eval-runs/...` and `/projects/{projectId}/eval-suites/...`.
Each endpoint is fully documented — request and response schemas, examples, and
an interactive playground — under **Endpoints** in the sidebar.
...
| `POST .../tools/call` | Execute a tool; returns the MCP `CallToolResult` directly |
| `POST .../prompts` | List the server's prompts |
| `POST .../prompts/get` | Render a prompt with arguments; returns `GetPromptResult` |
...
Eval runs are async: `POST` creates the run and responds **202** immediately with a `runId`. Poll `GET .../eval-runs/:runId` until `status` is terminal (`completed`, `failed`, or `cancelled`).

| Endpoint | What it does |
| --- | --- |
| `POST /projects/:projectId/eval-runs` | Start an eval run (rerun an existing suite and/or supply inline tests) |
| `GET /projects/:projectId/eval-runs/:runId` | Run status, result, and summary |
| `GET /projects/:projectId/eval-runs/:runId/iterations` | Paginated per-iteration results: tool calls, token usage, latency |
| `GET /projects/:projectId/eval-runs/:runId/iterations/:iterationId/trace` | Full trace (messages + spans) for one iteration |
| `GET /projects/:projectId/eval-suites/:suiteId/runs` | Recent runs for a suite |

**OAuth**

| Endpoint | What it does |
| --- | --- |
| `POST .../oauth/import-tokens` | Push externally-obtained OAuth tokens so subsequent calls to that server succeed automatically |
...
- Chat, conformance suites, and browser-completed OAuth flows (the `import-tokens` endpoint covers SDK/CLI agents that complete OAuth themselves)
