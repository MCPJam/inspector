...
| `GET .../eval-runs/{runId}` | Run status, result, and summary — poll until terminal. Carries `gateWaiver`: the audited override in force over this run's gate, or `null`. The `judges.goalCompletion.cases[]` and `judges.groundedness.cases[]` arrays each include an optional `iterationId` — the join key between a judge case and the iteration it graded (use this to pair a score with its iteration; `caseKey` is a storage key and matches nothing the iteration routes return). Absent on judge results written before it was persisted |
