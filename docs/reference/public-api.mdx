...
| `PATCH .../eval-suites/{suiteId}/schedule` | Enable/disable scheduled runs; `environmentId` pins which environment they launch. **Enabling** answers `404` on deployments where scheduled runs are not available; **disabling** is always allowed |
