...
| `GET .../goal-findings` | Criteria that keep failing, tracked across waves |
...
<Warning>
  Requesting wave insights consumes **no credits** — the model cost is on
...
sampling, and freshness; a wave's displayed counts cover only its requested
...
<Warning>
  ...
  It still draws on the same `insightsPerDay` ledger as eval-run and swarm wave
