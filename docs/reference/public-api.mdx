...
3. Copy the key (`sk_mcpjam_…`) immediately — **it is shown exactly once** and
   never stored by MCPJam in retrievable form.
...
If you see `401 UNAUTHORIZED` ("Invalid API key"), the key is invalid,
revoked, or its owner is no longer a member of the organization it was
created in — create a new key from
[Settings](https://app.mcpjam.com/settings/api-keys).
