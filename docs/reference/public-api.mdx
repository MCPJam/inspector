...
| `POST .../widgets/render` | Call an MCP App tool and mount its `ui://` widget in real headless Chromium, then return the render verdict, console errors, blocked requests, and the widget as an accessibility tree. Snapshot is on by default; screenshot is off. Refuses tools with no `ui://` resource (422). Capped at a per-replica concurrency limit (429) and a 45 s wall clock (504). Executes the tool. |
