...
**Errors you might see when using a key**
...
**Errors you might see when creating a key**
- `403 FORBIDDEN` → your account is not a member of the selected organization.
- `404 NOT_FOUND` → the selected organization does not exist.
- `409 VALIDATION_ERROR` ("still being set up" / "still syncing") → the organization or your membership is still being provisioned; wait a moment and try again.
- `400 VALIDATION_ERROR` → the organization or user identifier is malformed.
