...
- To call the **[MCPJam API](/reference/public-api)**, you want the **MCPJam API key** (`sk_mcpjam_…`) — the first one below.
...
- `401 UNAUTHORIZED` ("Invalid API key") → the key is invalid, revoked, malformed, or its owner left the organization it was created in. Create a new one from Settings. Keys issued before the `sk_mcpjam_…` format also land here — re-create them.
- `403 FORBIDDEN` ("API keys cannot manage other API keys") → you pointed an API key at the key-management endpoints; key management is UI-only.
...
| --- | --- | --- | --- |
| **MCPJam API key** | `sk_mcpjam_…` | [Settings → API keys](https://app.mcpjam.com/settings/api-keys) | The [MCPJam API](/reference/public-api) (`/api/v1/*`) |
