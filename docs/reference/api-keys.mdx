...
<Warning>
**Retired.** Project API keys no longer work: existing keys no longer authenticate, new keys can't be generated, and the `/sdk/v1/evals/*` ingestion endpoints they protected return `410 Gone` for every request. The `@mcpjam/sdk` eval-reporting flow that depended on them ([Saving Eval Results](/sdk/reference/eval-reporting)) is disabled until it's re-built on the **MCPJam API key (`sk_…`)** and the [MCPJam API](/reference/public-api). SDK evals still run and assert locally.
</Warning>

This was a project-scoped token for `@mcpjam/sdk` / CLI traffic to the hosted backend, read from the `MCPJAM_API_KEY` environment variable. If your CI logs now show `410` reporting errors, this retirement is why. The SDK's default auto-save path warns and continues (`reportEvalResultsSafely`), so eval runs themselves keep passing — but if you call `reportEvalResults` directly or set `strict: true`, remove that step (or unset `MCPJAM_API_KEY`) until `sk_`-based reporting ships.
