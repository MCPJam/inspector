...
**Who can create one:** by default, an organization's owners and admins. An owner or admin can let members create keys too, from the organization's **Organization keys** settings page.
...
**Expiry.** Every new key expires. The default is **90 days**; you can pick 7, 30, 60, 90, 180, or 365 days when you create one. The key list shows when each key expires and flags keys expiring within two weeks. Keys created before expiry was introduced keep working until they are revoked — they show as "No expiry". Create a replacement key before the old one expires; revoke the old one once traffic has moved.
...
- `401` with `details.reason: "EXPIRED_KEY"` → the key has passed its expiry date; create a new one from Settings.
...
Organization owners and admins have additional controls under **Organization settings → Organization keys**:

- **Revoke any member's key.** Owners and admins can revoke any key bound to the organization, not just their own. The key list shows who minted each key, when it expires, and a revoke action on every row. If a member's account has been removed, their key is still listed (as "Unknown user") and can still be revoked by id.
- **Restrict who can create keys.** The **Key creation** toggle ("Only owners and admins can create keys") limits minting to owners and admins. Members who already have keys keep using them until those expire or are revoked. The default is unchanged: any member can create a key.
- **Truncation warning.** If an organization has more keys than the page can list, a warning banner appears. Revoking keys you no longer need brings the rest into view.
