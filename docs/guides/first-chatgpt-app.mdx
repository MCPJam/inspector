...
description: "Build a simple Coffee Shop ChatGPT App"
icon: "sparkles"
---
...
The full code for this Coffee Shop app is available on GitHub:
...
Widgets run inside a sandboxed iframe. The client injects `window.openai` into this iframe, which is how your widget communicates with the client and invokes tools on your MCP server.

### The `window.openai` API

The client injects `window.openai` into your widget's iframe - this is the bridge between your frontend and the client. It provides globals, methods for calling tools, sending follow-ups, managing layout, and much more.

Here's how our Coffee Shop widget uses it:

```javascript
// Read initial data when the widget loads
const output = window.openai.toolOutput;
render(output.coffeeCount, output.message);
...
document.getElementById("orderBtn").addEventListener("click", async () => {
  ...
  render(result.structuredContent.coffeeCount, result.structuredContent.message);
});

// Listen for updates when users trigger tools via chat
...
- **`toolOutput`** - The `structuredContent` your server returned, available for your widget to read and render
- **`callTool()`** - Lets widget buttons trigger server tools directly (requires `openai/widgetAccessible: true` in the tool's metadata)
...
Your server is now running at `http://localhost:8787/mcp`.
