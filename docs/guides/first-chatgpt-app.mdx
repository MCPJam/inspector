...
`window.openai` provides globals, methods for calling tools, sending follow-ups, managing layout, and much more. Here's a basic widget structure using the `window.openai` API:
...
<body>
  <h2>☕️ Coffee Shop</h2>
  <div id="coffeeGrid"></div>
  <button id="orderBtn">Order ☕️</button>
  <button id="drinkBtn">Drink ☕️</button>
  <div id="status"></div>

  <script>
    function render(count, message) {
      // Update your UI based on current state...
    }
...
- **`window.openai.toolOutput`** - The `structuredContent` your server returned, available for your widget to read and render
