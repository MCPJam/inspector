...
Network access requires **Docker**. A native (`npx`) install binds to `127.0.0.1` (localhost only) and cannot be reached from another machine. The Docker image binds `0.0.0.0`, so it can.

For security, the inspector only issues its session token to `localhost`. When you open a Docker install from another machine (e.g. `http://192.168.1.50:6274`), you'll land on an authentication error. To allow a specific host, set `MCPJAM_ALLOWED_HOSTS` to that hostname and publish the port on your network interface (drop the `127.0.0.1:` prefix from `-p`):

```bash
docker run -p 6274:6274 -e MCPJAM_ALLOWED_HOSTS=192.168.1.50 mcpjam/mcp-inspector:local
```

For an IPv6 host, bracket the entry: `MCPJAM_ALLOWED_HOSTS=[fd00::50]`. Comma-separate multiple hosts. Wildcards like `*.example.com` are supported — if you use a wildcard entry, also set `MCPJAM_ALLOW_WILDCARD_ORIGINS=true` so API calls are accepted (without it, the session token is issued but requests still 403).

<Warning>
Only add hosts you trust. Allowlisting a host also accepts it as a request origin, which exposes the local shell and agent-browser tools that are on by default in self-hosted mode. Set `MCPJAM_LOCAL_COMPUTER_ENABLED=false` and `MCPJAM_LOCAL_BROWSER_ENABLED=false` if you don't want that. Tunnel and relay domains are never allowed, even if listed.
</Warning>
