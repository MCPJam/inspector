...
| `03-run-all-failed.png` | 2. Trigger the failure | XAA Debugger after **Run all**: the sequence diagram + the red "MCP server rejected the access token" card. |
| `04-inspect-token.png` | 3. Inspect the trace | The failed **authenticated MCP request** step expanded → its HTTP entry expanded to the **response**: the 401 whose body reads `token audience does not match this server's required audience (https://demo.example.com/mcp)`. Include the request's `Authorization: Bearer …` header if it fits. |
...
| `06-fix.png` | 5. Apply the fix | The `.env` file (`CANONICAL_URL=http://localhost:8080/mcp`) and/or the toy-server terminal after restart showing the new `audience required: http://localhost:8080/mcp` startup line. |
| `07-run-all-green.png` | 6. Re-run and verify | XAA Debugger after the fix: all steps green, Flow Complete, the MCP call succeeded. |
