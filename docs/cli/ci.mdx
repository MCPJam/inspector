...
**`nextAction` distinguishes conflict from missing data.** When a trial fails but the chain recorded no failure category, `nextAction` is one of two things depending on what the chain actually shows. If the chain validated and every applicable stage passed — meaning the measured chain found nothing wrong but the verdict still says failed — `nextAction` reads `the recorded verdict disagrees with the measured chain; inspect the case trace`. If the chain was unverified, a stage was never measured, or no verdict was recorded, it falls back to `inspect the case trace; no failure category was recorded`. The two are different investigations: a disagreement means two things you hold are in conflict; a fallback means information is absent.
