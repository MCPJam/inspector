...
<Note>
**Runs launched from Actions badge as GitHub, and fill the CI column.** The CLI
...
launch used to show, and its commit, branch and job appear alongside it. Nothing
to configure: the declaration is derived from the standard `GITHUB_*`
environment, exactly as conformance uploads already do it. Outside CI the same
command declares **CLI**.
