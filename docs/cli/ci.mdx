...
<Note>
The `evals-v1` tag is published automatically by the MCPJam Release workflow
whenever `actions/evals` changes. To check whether it has been published yet:

```sh
git ls-remote --tags https://github.com/MCPJam/inspector 'evals-v*'
```

If that command prints nothing, the action has not been released and the example
above cannot resolve. The direct CLI integration in the next section remains
