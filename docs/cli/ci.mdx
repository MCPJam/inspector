...
If you instead run evals inside your own CI job with [`@mcpjam/sdk`](/sdk/concepts/running-evals) (`EvalTest` / `EvalSuite`), set `MCPJAM_API_KEY` and results upload automatically to the Evaluate **Runs** mode (pass-rate trends, per-model breakdowns, and a full trace per iteration):
