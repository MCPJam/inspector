...
<Note>
Reaching a MCPJam that sits behind Cloudflare Access — a staging or self-hosted deployment — also needs a service token. Set `CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET` in the same `env:` block; the action sends them with its MCPJam reads and with nothing else.
</Note>
