...
**MCPJam behind Cloudflare Access:** if your MCPJam deployment is fronted by
Cloudflare Access, the action needs a service token to reach the API. Add
`CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET` to the step's `env:`:
...
Both variables must be set — one alone is ignored. The action sends them only
with its MCPJam requests; GitHub API calls are unaffected.
