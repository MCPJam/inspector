...
<Note>
`evals-v1` is published by MCPJam's release workflow, which smoke-tests the
action live and then moves the tag. Until the first release moves it, pin a full
commit SHA instead — `uses: MCPJam/inspector/actions/evals@<40-character sha>` —
or use the direct CLI integration below, which does not depend on the tag.
</Note>
...
<Note>
**Runs launched from Actions badge as CLI.** The CLI always declares `CLI`,
even inside a GitHub Actions workflow. The **GitHub** badge in the Runs table
means the MCPJam GitHub App — a check run MCPJam built and ran itself — not
your own workflow calling `mcpjam cloud eval run`. Where the run came from is
still recorded: the CI envelope carries the provider, commit SHA, PR, and
branch from the standard `GITHUB_*` environment, and those populate the
provenance columns in the Runs table — nothing to configure.

That commit is also what `eval compare --baseline-sha` resolves against, so a
run launched by the CLI in CI can be compared to a previous commit's run the
...
#### Report them on the pull request

Uploading puts the results in the dashboard; it does not put them in front of a reviewer. Hand the same command to the evals action instead of running it yourself, and each pull request gets one comment that is updated in place on every push, plus a checks summary and the reports as artifacts:
...
`command` replaces `project` and `suite`: the action runs your tests, then reports the runs they uploaded. It finds each run through a receipt the SDK writes, so `@mcpjam/sdk` must be **8.8.0 or newer** — an older SDK uploads normally but writes no receipt, and the step reports that it found no run.

The comment carries the client/model table, the failed cases, and links into MCPJam. The checks summary carries the full report. Without `pull-requests: write` the action warns and leaves the eval verdict alone.

Runs uploaded together — `runWithClient` given several clients — read as one report: one row per client and model in the same table, with a link to each client's run.
