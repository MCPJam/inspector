...
Manage eval suites, run them, and inspect results in your hosted MCPJam projects. All `eval` commands accept the shared platform flags below.
...
Create a new eval suite from a JSON definition.
...
| `--model <id>` | No | Suite-level default model (overrides the file) |
...
Start an eval run of an existing suite (asynchronous — use `eval status` to poll).
...
Fetch the full trace for one eval iteration (full message history and spans).
...
Show the widget screenshot(s) an eval iteration rendered. In an interactive terminal that supports a graphics protocol (iTerm2, WezTerm, Kitty, Ghostty), the PNG renders inline; otherwise the image URL is printed. In non-TTY contexts and with `--format json`, structured output is emitted instead.

```bash
mcpjam eval screenshot --run <id> --iteration <id> --project <id-or-name>
```
...
| `--iteration <id>` | Yes | Iteration ID (from `eval iterations`) |
| `--project <id-or-name>` | Yes | Project the run belongs to (name or ID) |
| `--out <path>` | No | Save the PNG(s) to a file or directory instead of rendering inline. Pass a directory path (or a path ending in `/`) when the iteration rendered multiple screenshots. |
| `--index <n>` | No | Show only the Nth screenshot (1-based) |

JSON output (`--format json`) emits `{ project, runId, iterationId, items[] }` where each item has `{ toolName, toolCallId, promptIndex, status, screenshotUrl, savedTo? }`. Iterations with no rendered screenshots exit `0` with a plain message.
...
| `--arguments <ignore\|partial\|exact>` | No | Argument matching mode |
...
Permanently delete an eval suite and all its cases and runs.
