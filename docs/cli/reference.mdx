...
Manage the hosts saved in your hosted MCPJam projects. All `cloud hosts` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam cloud login`.
...
List the built-in host templates usable with `cloud hosts create --template`. No additional flags.
...
| `--project <id-or-name>` | No | Project name or ID |
| `--template <id>` | No | Built-in template ID (see `cloud hosts templates`), e.g. `claude`, `chatgpt`, `cursor` |
...
All `cloud environments` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam cloud login`. Reading requires project membership; **creating, updating, archiving, and restoring require project admin** — a key bound to a non-admin gets a `FORBIDDEN` error on those.
...
|------|----------|-------------|
| `--environment <id-or-name>` | Yes | Environment name or ID |
| `--expected-revision <n>` | Yes | The revision you last read (from `cloud environments get`) |
| `--project <id-or-name>` | No | Project name or ID |
| `--name <name>` | No | New display name |
...
|------|----------|-------------|
| `--environment <id-or-name>` | Yes | Environment name or ID |
| `--expected-revision <n>` | Yes | The revision you last read (use `cloud environments list --include-archived`) |
| `--project <id-or-name>` | No | Project name or ID |

### `cloud environments ensure-adhoc`

Get or create an **unnamed** environment for a composed stack. Deduplicated by content: the same stack always returns the same environment, with `created: false` on the second call. Ad-hoc environments do not appear in `cloud environments list` — they exist so a one-off combination can be run and reproduced without adding a name to the project's list.
...
```
 All `cloud images` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam cloud login`.
...
Trigger an image build for the sandbox image (async). Poll `cloud images logs` to check build status.
...
Author and run eval suites in your hosted MCPJam projects. All `cloud eval` subcommands require an MCPJam API key (`--api-key` or `MCPJAM_API_KEY`).
...
Create a runnable eval suite from authored test cases. The suite is persisted and returned with a `suiteId`; it is **not** run. Execute it afterward with `cloud eval run`.
...
Start an asynchronous eval run of an existing suite. Responds immediately with a `runId`; poll status with `cloud eval status`.
...
| Flag | Required | Description |
|------|----------|-------------|
| `--run <id>` | Yes | Eval run ID (from `cloud eval run`) |
...
```bash
# HTTP target
mcpjam cloud tunnel http://localhost:9090/mcp --id my-server --project acme

# stdio target (command goes after --)
mcpjam cloud tunnel --id everything --project acme -- npx -y @modelcontextprotocol/server-everything
```
...
## `cloud eval` commands (full reference)

All `cloud eval` commands accept the shared platform flags below.
...
When the run targets an **attached project environment** (`cloud eval environments
...
Every run records the environment and the exact revision it used, so `cloud eval
...
budget.** Returns a pending receipt; read the grades from `cloud eval status`.
...
|------|----------|-------------|
| `--run <id>` | Yes | Eval run ID (from `cloud eval run`) |
| `--iteration <id>` | Yes | Iteration ID (from `cloud eval iterations`) |
| `--project <id-or-name>` | Yes | Project the run belongs to (name or ID) |
...
Offline: this command takes **no `--project`**, builds no API client and reads
no API key. It never touches the network.
...
**What "valid" does not mean.** This command checks the file against the
suite-file contract. It does **not** re-resolve tool names, server references or
fixtures against a project's live discovery — that is project-aware validation,
...
- host attachments, and attached project environments (a suite file names an
  environment, and resolving an id to a name would need a second fetch this
  command does not make);
- a pinned sandbox image, an execution system prompt, or an execution
  temperature — each changes what runs and has no field in the file;
- LLM-as-judge grading, non-default match options, and an iterations floor that
  raises a case above its own count;
...
- `replace`/`extend` check overrides, a case that inherits the suite's checks
  ...
  not recognise;
- a suite with no execution model, no minimum accuracy, no name, no selected
  servers, or no cases; a case with no steps; and a suite or case id that is not
  an opaque id;
...
| `--computer-image <name-or-id\|off>` | No | Sandbox image eval runs boot a fresh computer from (list them with `mcpjam cloud images list`). `off` uses the provider's default base image |
...
|------|----------|-------------|
| `--suite <id-or-name>` | Yes | Eval suite name or ID |
| `--repo <owner/repo>` | Yes | Repository to connect. Must be one `cloud eval checks list` reports as connectable |
...
makes `cloud eval run --environment` available for the suite.
...
repoint the schedule (`cloud eval schedule --enable --environment ...`) or disable it first.
...
Run a single case as a persisted, fully-queryable run — inspect it with `cloud eval iterations` /
`cloud eval steps` like any other run.
...
Every `--compose-*` flag behaves exactly as it does for `cloud eval run` — see
...
## `cloud login` / `cloud logout` / `cloud whoami` commands

These commands manage your MCPJam Cloud account session. `cloud login` opens a browser for OAuth and stores the session locally; `cloud logout` removes it; `cloud whoami` shows the account behind the current credentials.

<Note>
  `mcpjam oauth login` authenticates you to an MCP server (MCP OAuth). `mcpjam cloud login` authenticates you to your MCPJam account. They are separate commands.
</Note>
