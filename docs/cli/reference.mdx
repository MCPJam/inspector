...
Unknown top-level keys in the `--json` / `--file` body are rejected as a usage error (exit code `2`) and the offending key is named. Remove or correct any undeclared keys (for example `hostz`, `hostIds`, or a top-level `servers` field) before retrying.
...
Unknown top-level keys in the `--json` / `--file` body are rejected as a usage error (exit code `2`) and the offending key is named. Remove or correct any undeclared keys before retrying.
