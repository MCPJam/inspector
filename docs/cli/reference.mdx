...
Publish project environments for user testing and take them down again. All `scenarios` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`. Publishing is behind a per-organization beta flag; unpublishing is not gated so you can always take a live scenario down.
...
`--name`, `--description`, and `--mode` are **create-time only**: they are applied in the same call that creates the scenario, so the link is never briefly live in a wider mode than you asked for. On a re-publish they are ignored and the response includes `overridesIgnored: true`. To change an existing scenario's name, description, or mode, use `mcpjam user-testing update`.

A misspelled `--mode` value is caught locally and reported as a usage error before any network call.
...
| `--mode <mode>` | No | Who may open the share link (create time only): `project_members`, `invited_only`, or `anyone_with_link` |

```bash
# Publish with default access
mcpjam scenarios publish --environment env_abc123

# Publish and restrict to invited users only, with a custom name
mcpjam scenarios publish --environment env_abc123 \
  --name "Beta feedback" \
  --mode invited_only
```
