...
List the MCPJam organizations you belong to. This is the source of the `organizationId` that `projects list --organization-id` filters by and `projects create --organization-id` accepts. All `organizations` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`.

An API key is bound to one organization and only ever sees that one. A session login sees all organizations the account belongs to.
...
List your organizations and their IDs. Alias: `orgs list`.

No additional flags beyond the shared platform flags above.

```bash
# List organizations (human-readable table)
mcpjam organizations list

# Alias
mcpjam orgs list

# JSON output for scripting
mcpjam organizations list --format json
```
...
Manage projects in your hosted MCPJam projects. All `projects` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`.
...
mcpjam organizations list                          # find the id
mcpjam projects list --organization-id org_abc123
```

### `projects create`

Create an empty project in one of your organizations.

| Flag | Required | Description |
|------|----------|-------------|
| `--name <name>` | Yes | Display name for the new project |
| `--organization-id <id>` | No | Organization to create the project in (see `mcpjam organizations list`). Defaults to the key's organization |
| `--description <text>` | No | Optional description |
| `--icon <emoji>` | No | Project icon |
| `--visibility <public\|private>` | No | Project visibility |

### `projects update`

Rename a project or change its description, icon, or visibility. Never touches server configurations.
...
| `--icon <emoji>` | No | New icon |
| `--visibility <public\|private>` | No | New visibility |

### `projects delete`

Permanently delete a project and all its resources (servers, eval suites, runs, hosts). This cannot be undone.
