...
|------|----------|-------------|
| `--project <id-or-name>` | No | Project name or ID (defaults to the most recently updated project) |
| `--file <path>` | No | Path to a **create-API JSON** body (or `-` for stdin). A versioned suite file (`schemaVersion: "1"` or `"2"`) belongs on `eval run --file` |
...
`eval run --file` reads the versioned suite file (`schemaVersion: "1"` or
`"2"`), authenticates, then validates. A contract-invalid file exits **2**
after the auth request — it does not start a run. `eval validate` on the same
bytes still exits 1: that command is a verdict on the file; this one is not.
...
conversion refuses rather than approximates. The configured iteration count
(`repetitions` in dialect 1, `iterations` in dialect 2) above **10** is
...
server annotations are advisory and **UNTRUSTED**. The file's configured
iteration count (`defaults.repetitions` in dialect 1, `defaults.iterations`
in dialect 2) is inherited per case and is not uploaded as a suite
`minIterations` floor.
...
describes a suite, its defaults and its cases (`schemaVersion: "1"` or `"2"`,
YAML canonical, JSON accepted, conventionally `.mcpjam/evals/*.yaml`).
...
**Suite file dialects.** Two dialects are accepted. Dialect `"1"` spells the
configured run count `repetitions` and the case's rules `checks` (with
`assertions` as a deprecated alias). Dialect `"2"` spells them `iterations`
and `assertions` respectively; the dialect-1 spellings are unknown keys in
dialect 2. `eval export` always writes dialect 1. A wrong-dialect key produces
a finding that names the other dialect's spelling and offers both fixes.
...
Write a hosted eval suite to a local suite file (always dialect 1, `schemaVersion: "1"`).
