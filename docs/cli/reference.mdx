...
| `--compose-host <id-or-name>` | No | Compose a stack to run instead of naming a saved environment (see below). Default is **ephemeral** — does not attach to the suite. |
| `--compose-computer <id-or-name>` | No | Sandbox image to pin on the composed stack |
| `--compose-model <id...>` | No | One or more models to run on the composed stack. Each id mints one cell; omit to inherit the host's pinned model. |
| `--with-client-default` | No | Also launch an inherit cell (the host's pinned model) alongside `--compose-model` |
| `--save-targets` | No | Attach the composed environments to the suite (append, capped at 10). Default is ephemeral. |
...
`--with-client-default`, `--save-targets`, `--compose-server-group` and
`--compose-skill`. The stack becomes an unnamed, content-addressed environment
(the same one `environments ensure-adhoc` produces), so composing the same
stack twice reuses one environment.

**Default is ephemeral.** Composed environments are minted and launched without
being attached to the suite. Pass `--save-targets` to append them instead
(capped at 10 total environments on the suite).

**Multi-model compose.** Pass `--compose-model` more than once (or as a
space-separated list) to mint one cell per model and launch them as a single
run group under one `runGroupId`. Add `--with-client-default` to also include
an inherit cell that uses the host's pinned model alongside the explicit ones.
Multi-model compose requires a backend that supports `ephemeralEnvironmentLaunch`
(probe `GET /environments/capabilities`); on older backends a single-cell
compose falls back to attach, and multi-model is refused with an upgrade message.

The result always reports what was minted (`composed.environment.created`,
`composed.environments`, `composed.attachment.attached`) even when the launch
...
| `--compose-host <id-or-name>` | No | Compose a stack to run instead of naming a saved environment. Default is ephemeral. |
| `--compose-computer <id-or-name>` | No | Sandbox image to pin on the composed stack |
| `--compose-model <id>` | No | One model to run on the composed stack. A matrix of models is suite-level (`eval run`) only. |
| `--compose-server-group <id>` | No | Standalone server group to pin on the composed stack |
| `--compose-skill <id...>` | No | Project-shared skill IDs to pin on the composed stack |
...
`--compose-model` accepts only one model on a case run; use `eval run` for a multi-model matrix.
