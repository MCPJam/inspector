...
Open a long-lived MCP subscription stream and print events as NDJSON on stdout. Uses shared connection flags, plus:
...
| `--resource-uri <uri...>` | Watch a resource URI for `notifications/resources/updated`. Repeatable. |
| `--relisten <count>` | After an unexpected remote loss, re-listen up to this many times with exponential backoff. Default: `0` (disabled). |
| `--relisten-delay-ms <ms>` | Initial re-listen backoff delay in milliseconds. Requires `--relisten`. |
| `--relisten-max-delay-ms <ms>` | Maximum re-listen backoff delay in milliseconds. Requires `--relisten`. |
| `--max-notifications <count>` | Stop after this many delivered notifications (exits `0`, outcome `cancelled`). |
| `--duration-ms <ms>` | Stop after this many milliseconds (exits `0`, outcome `cancelled`). |

stdout carries only NDJSON events in both `--format json` and `--format human`. Human status goes to stderr and is suppressed by `--quiet`.

**Exit codes:** `graceful` and `cancelled` outcomes exit `0`; `remote` (unexpected stream loss) exits `1` with `SUBSCRIPTION_REMOTE_CLOSED`; `error` (open failed) exits `1` with `SUBSCRIPTION_FAILED`.

See [Subscriptions](/cli/subscriptions) for the full event schema and usage examples.
...
Manage custom Computer sandbox images (Dockerfiles) in your hosted MCPJam projects. All `images` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`.
