...
<Note>
A scorer id named by `--min-scorer-pass-rate` or `--min-mean-score` must resolve to a single definition across all cases in the run. If the same id was configured with different settings in different cases (for example, different thresholds or rubrics), the gate returns `usage_error` (exit 2). Fix it by giving each variant a distinct explicit id, or by making the configuration identical across every case that uses it.
</Note>
...
| `2` | Usage error — an unknown scorer, a threshold out of range, or a scorer id that resolves to multiple different definitions across cases (configure it identically in every case, or give each variant a distinct id) |
