...
Manage Playground chat sessions in your hosted MCPJam projects. All `sessions` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`.
...
List Playground chat sessions, newest first. Scoped to one project by default; pass `--all-projects` to list across every accessible project.

| Flag | Required | Description |
|------|----------|-------------|
| `--project <id-or-name>` | No | Project name or ID (defaults to the most recently updated project). Cannot be combined with `--all-projects`. |
| `--all-projects` | No | List sessions across every accessible project, ignoring the project link and `MCPJAM_PROJECT`. Conflicts with `--project`. |
...
Cancel an in-flight eval run. No-op if already cancelled; errors if the run already finished.
...
|------|----------|-------------|
| `--run <id>` | Yes | Eval run ID (from `eval run`) |
| `--project <id-or-name>` | No | Project the run belongs to (name or ID; defaults to the most recently updated project) |
...
| `--run <id>` | Yes | Eval run ID (from `eval run`) |
| `--project <id-or-name>` | No | Project the run belongs to (name or ID; defaults to the most recently updated project) |
| `--min-pass-rate-percent <0-100>` | No | Minimum pass rate to consider the run passing |
| `--wait` | No | Poll until the run reaches a terminal status before gating |
| `--wait-timeout <ms>` | No | Maximum time to wait when `--wait` is set |
...
| Flag | Required | Description |
|------|----------|-------------|
| `--run <id>` | Yes | Eval run ID to compare |
| `--project <id-or-name>` | No | Project the run belongs to (name or ID; defaults to the most recently updated project) |
| `--base-run <id>` | No | Baseline run ID to compare against |
| `--reporter <reporter>` | No | `json-summary` or `junit-xml` CI report output |
...
Fetch the steps for one eval iteration.
...
Fetch a screenshot from one eval iteration.
...
| `--iteration <id>` | Yes | Iteration ID (from `eval iterations`) |
| `--project <id-or-name>` | No | Project the run belongs to (name or ID; defaults to the most recently updated project) |
| `--out <path>` | No | Write the screenshot to a file |
| `--index <n>` | No | Show only the Nth screenshot (1-based) |

### `eval video`

Fetch a video from one eval iteration.
...
| `--iteration <id>` | Yes | Iteration ID (from `eval iterations`) |
| `--project <id-or-name>` | No | Project the run belongs to (name or ID; defaults to the most recently updated project) |
| `--out <path>` | No | Write the video to a file |
