...
- **`skillSelection`** accepts project-shared skills only. Personal skills, skills with supporting files or extra frontmatter, and plugin-component skills are all rejected. The optional `versionPins` array lets you hold one or more selected skills at a specific version instead of Latest — at most one pin per skill, and every pinned skill must appear in `skillIds`. Probe `GET /environments/capabilities` (`skillVersionPins`) before sending version pins; older deployments reject the field.
