...
Unlike other Cloud commands, `cloud status` never throws on invalid credentials or API URLs. It always emits a complete JSON report and exits as follows:

| Exit code | Meaning |
|-----------|---------|
| `0` | Configuration looks usable (credential present or informatively absent, deployment URL valid) |
| `1` | Configuration has a problem — `ok: false` in the report, with `credential.error` or `deployment.error` describing it |
...
|-------|------|-------------|
| `source` | string | Where the credential came from: `flag`, `env`, `oauth`, or `missing` |
| `kind` | string | `api-key`, `oauth`, or `none` |
| `valid` | boolean \| null | `true` when a usable credential is configured; `false` when an explicit credential is invalid (e.g. a legacy `mcpjam_…` key); `null` when no credential is configured (informational — not an error) |
| `error` | string | Present when `valid` is `false`; describes the problem and how to fix it |
| `redactedKey` | string | Present for API-key credentials; the key with the middle redacted (e.g. `sk_…efgh`) |
| `envShadowsOauth` | boolean | Whether a `MCPJAM_API_KEY` env var is shadowing a stored OAuth session |
| `storedOauthPresent` | boolean | Whether a stored OAuth session exists on disk |

The `deployment` object includes:

| Field | Type | Description |
|-------|------|-------------|
| `apiUrl` | string | The API URL that would be used (the raw value when invalid) |
| `source` | string | Where the URL came from: `flag`, `env`, `oauth`, or `default` |
| `valid` | boolean | `true` when the URL is a valid `http(s)` URL; `false` otherwise |
| `error` | string | Present when `valid` is `false`; describes the problem |
