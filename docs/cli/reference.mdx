...
The `cloud` parent command groups all MCPJam Cloud account operations. Credential flags are declared once here and inherited by every `cloud` subcommand — you can place them before or after the subcommand name:

```bash
# Both forms are equivalent
mcpjam cloud --api-key sk_… projects list
mcpjam cloud projects list --api-key sk_…
```

`mcpjam --api-key sk_… cloud …` (before `cloud`) remains a usage error.

### Cloud credential flags

| Flag | Description |
|------|-------------|
| `--api-key <key>` | MCPJam `sk_` API key (overrides `MCPJAM_API_KEY`). Rejected by `cloud login` — use browser OAuth instead. |
...
Manage the hosts saved in your hosted MCPJam projects. All `hosts` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`. Pass credential flags on `mcpjam cloud` (see [Cloud credential flags](#cloud-credential-flags)).
...
All `environments` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`. Reading requires project membership; **creating, updating, archiving, and restoring require project admin** — a key bound to a non-admin gets a `FORBIDDEN` error on those. Pass credential flags on `mcpjam cloud` (see [Cloud credential flags](#cloud-credential-flags)).
...
```
 All `images` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`. Pass credential flags on `mcpjam cloud` (see [Cloud credential flags](#cloud-credential-flags)).
...
Author and run eval suites in your hosted MCPJam projects. All `eval` subcommands require an MCPJam API key (`--api-key` or `MCPJAM_API_KEY`). Pass credential flags on `mcpjam cloud` (see [Cloud credential flags](#cloud-credential-flags)).
...
```bash
mcpjam cloud --api-key $MCPJAM_API_KEY eval create \
...
Credential flags (`--api-key`, `--api-url`) are inherited from `mcpjam cloud` — see [Cloud credential flags](#cloud-credential-flags).
...
## `eval` commands (extended reference)

All `eval` commands accept the [Cloud credential flags](#cloud-credential-flags) on the `mcpjam cloud` parent.
...
These commands manage your MCPJam platform session. They live under `mcpjam cloud`. `cloud login` opens a browser for OAuth and stores the session locally; `cloud logout` removes it; `cloud whoami` shows the account behind the current credentials.
...
`--api-url` is inherited from the `cloud` parent (see [Cloud credential flags](#cloud-credential-flags)). The value is persisted with the session so subsequent cloud commands target the same deployment without needing `--api-url` again. Must be a valid `http(s)` URL — an invalid value exits with code `2` before any network call.

`--api-key` is **rejected** by `cloud login` — this command uses browser OAuth. Pass `--api-key` to other cloud commands, or set `MCPJAM_API_KEY`.
...
1. Explicit `--api-url` flag on `mcpjam cloud`
...
No additional flags beyond the [Cloud credential flags](#cloud-credential-flags) inherited from `mcpjam cloud`.
