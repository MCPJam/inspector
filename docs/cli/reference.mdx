...
**`--enable` answers `404`** on deployments where scheduled runs are not available. **`--disable` is always allowed**, so a schedule already running can always be stopped regardless of deployment availability.
