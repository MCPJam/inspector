...
| `--client-id <id>` | No | | OAuth client the ID-JAG is issued for. Required for `preregistered` (the default); rejected for `dcr`/`cimd` (the identity is derived) |
| `--registration <method>` | No | `preregistered` | Client-registration strategy: `preregistered`, `dcr`, or `cimd` |
| `--client-metadata-url <url>` | No | | CIMD only: the Client ID Metadata Document URL to present as the `client_id`. Defaults to the hosted XAA debugger document |
...
| `--token-endpoint <url>` | No | | Authorization-server token endpoint. When set, AS-metadata discovery is skipped. Rejected for `dcr`/`cimd` |
| `--email <email>` | No | | Simulated end-user email claim |
| `--client-secret <secret>` | No | | OAuth client secret presented at redemption. Rejected for `dcr`/`cimd` |
| `--token-endpoint-auth-method <method>` | No | Inferred from metadata | `client_secret_basic`, `client_secret_post`, or `none`. Rejected for `dcr`/`cimd` |
...
| Strategy | `--client-id` | `--client-secret` | `--token-endpoint` | Description |
|----------|:-------------:|:-----------------:|:------------------:|-------------|
| `preregistered` (default) | Required | Optional | Allowed | Use a pre-registered client identity |
| `dcr` | Rejected | Rejected | Rejected | RFC 7591 dynamic registration — the RAS mints the `client_id`. A diagnostic that simulates the IdP→RAS mapping; may leave a registration behind each run |
| `cimd` | Rejected | Rejected | Rejected | Client ID Metadata Document — public-client variant; interoperability only, not a recommended production posture |

`--authz-server-issuer` is allowed with all three strategies.
