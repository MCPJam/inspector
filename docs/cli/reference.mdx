...
Manage custom Computer environments (digest-pinned Dockerfile images) in your hosted MCPJam projects. All `env` commands accept the shared platform flags below.
...
Trigger an async image build. Poll `env logs` for status.
...
Attach an environment to your computer, which re-provisions it from the pinned image (installed files are wiped). The environment must have a `ready` build.
...
Share a personal-draft environment with the whole project. Requires project admin.
