...
| `--run <id>` | Yes | Eval run ID (from `eval run`) |
| `--project <id-or-name>` | No | Project name or ID (defaults to the most recently updated project) |
| `--scope <all\|failed>` | No | Regrade all iterations (`all`) or retry only failed or ungraded ones (`failed`) |
...
`--scope failed` retries only iterations that previously errored or were skipped, without regrading ones that already have a scored result. `--scope all` regrading every iteration. Omitting `--scope` uses the suite's default behavior. An empty string is rejected.
...
- non-default match options and an iterations floor that raises a case above its own count;
