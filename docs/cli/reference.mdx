...
| `--wait-timeout <ms>` | No | Maximum wait time (default 600000); requires `--wait`. A run held in `grading` for its judge earns up to 31 more minutes on top of the default; passing this flag makes the budget strict and waives that extension. |
...
| `--wait-timeout <ms>` | No | Maximum wait time (default 600000). A run held in `grading` for its judge earns up to 31 more minutes on top of the default; passing this flag makes the budget strict and waives that extension. |
