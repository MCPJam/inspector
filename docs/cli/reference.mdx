...
Grade a server or plugin against a publisher's directory. See the [directory readiness check guide](/cli/readiness-check) for usage examples.
...
| `--submission-mode <mode>` | Yes | | Declared submission shape. Never inferred. One of: `mcp-only`, `mcp-imported-skills`, `skills-only` |
...
Exit codes: `0` ready · `1` not-ready · `3` incomplete · `2` usage.
...
| `3` | `tasks conformance` and `readiness check`: the run was incomplete — its checks never ran or could not reach the target, so it is neither a pass nor a spec violation |
