...
`--enable` answers `404` unless the deployment sets
`MCPJAM_SCHEDULED_EVALS_WRITE_ENABLED=true` — scheduled runs are off by default
while the feature is being tested. `--disable` is deliberately ungated, so a
schedule already firing can always be switched off.
