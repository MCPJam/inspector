...
<Note>
`sessions send` defaults to a **300 s timeout** instead of the global 30 s default. A model turn with tools routinely runs longer than 30 s, and the client giving up does **not** stop the turn — it keeps running and keeps spending. Pass `--timeout <ms>` to override in either direction. If a turn times out, retry with the **same `--idempotency-key`** to replay the completed turn rather than start a new one.
</Note>
