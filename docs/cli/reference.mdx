...
## MCPJam Cloud commands

All commands under `mcpjam cloud` (and the top-level `login`, `logout`, `whoami`, `tunnel`, `hosts`, `environments`, `images`, `eval`, and `readiness` hosted commands) require an MCPJam Cloud credential — either an `sk_` API key via `--api-key` / `MCPJAM_API_KEY`, or a prior `mcpjam cloud login`. If no credential is configured the command fails immediately with login guidance before making any network call.

### Cloud audience line

Before each Cloud operation, the CLI prints a context line to stderr:

```
Using MCPJam Cloud as <identity> · <scope> · <api-url>
```

- **`<identity>`** is your redacted API key (e.g. `sk_…1234`) or OAuth email address.
- **`<scope>`** describes the resource the operation targets: `account`, `all projects`, `organization <id>`, or `project: <selector>`.
- **`<api-url>`** is the MCPJam API base URL in use.

This line goes to stderr only and never appears in `--format json` stdout output. Pass `--quiet` to suppress it. The line is not printed for `login`, `logout`, `whoami`, `cloud link`, and `cloud status`.
...
Lint a blueprint YAML file with the MCPJam Cloud image linter without saving it. Prints `ok` plus the resolved base digest, or structured errors with the YAML path of each violation.
...
These commands manage your MCPJam Cloud session. `login` opens a browser for OAuth and stores the session locally; `logout` removes the stored MCPJam Cloud login; `whoami` shows the MCPJam Cloud account behind the current credentials.
...
No additional flags. Removes the stored MCPJam Cloud login.
