...
<Note>
  A project environment is not a Computer sandbox image (those are `mcpjam cloud
  images`), and not the STDIO environment variables you pass with `--env`.
</Note>
...
<Warning>
  ...
  server record so the platform can call it. Every re-run rotates the secret and
  disconnects any previous tunnel session for the same server.
</Warning>
...
| `--compose-host-servers`                     | No                    | Run against whatever servers the client points at **right now** instead of pinning a set — editing that client later changes what a rerun tests                                                                      |
...
<Warning>
  **With `--save-targets`, this edits the suite.** The composed environment is
  ...
  except on a deployment with no ephemeral-launch support, where a single-target
  compose still attaches. The result reports both writes
  (`composed.environment.created`, `composed.attachment.attached`) even when the
  launch itself fails, so you always know what changed.
</Warning>
...
each other _is_ a comparability claim, and it holds only across runs that share
...
**say so**: a capped take is a complete, playable _beginning_ of the run rather
...
Turn a document — a test plan, a QA checklist, a spreadsheet of scenarios — into runnable test cases with MCPJam's AI and add them to the suite. Markdown, JSON and CSV are accepted, up to 100 KiB. **Costs money:** each import consumes customer credits.
...
| `--format <markdown\|json\|csv>`                  | No       | Document format (inferred from `--file`'s extension when omitted)                                                |
...
<Note>
  Hosted session browsers require browser availability for your account and
  deployment. An unavailable browser reports a refusal or `browser.attached:
  false` with its reason. The live pane opens in the Playground when the browser
...
Codes above `2` are reserved for outcomes a script should act on _differently_ — not merely for naming a failure. See [MCP Tasks](/cli/tasks#watch-exit-codes) for the full `tasks watch` mapping.
