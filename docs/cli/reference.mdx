...
<Note>
**SDK 8.16.0 requires a new baseline.** The `toolCalls:arguments` scorer introduced in SDK 8.16.0 changes the set of score definitions and therefore the `evaluationConfigHash`. The first run after upgrading exits `3` (not gateable) when gated against a baseline recorded on an earlier version. Record a new baseline from a run on SDK 8.16.0 or later before re-enabling the gate.
</Note>
