...
and the API refuses the flag a suite does not use with a message naming the one
that would work. Changing which criterion decides a suite is a separate,
explicit operation — it re-decides every case, so it is never a side effect of
editing a threshold.
