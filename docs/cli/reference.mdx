...
server annotations are advisory and **UNTRUSTED**. `mode` does the restraining:
`readOnly` permits only tools annotated read-only, `default` permits everything
except tools annotated `destructiveHint`. `deny` removes a named tool under
either mode. `allow` is an **override, not a whitelist** — it exempts a named
tool from the mode-derived rules (including `default` mode's destructive
deny-by-default) and never restricts anything on its own. `{ mode: "default",
allow: ["read_file"] }` therefore restrains nothing; tools you want blocked
belong in `deny`, or the suite belongs in `readOnly` mode. File `defaults.repetitions` is
