...
| `--compose-host <id-or-name>` | No | Compose a stack to run instead (see [`eval run`](#eval-run)); appends the composed environment to the suite |
| `--iterations <n>` | No | Run the case this many times (1-10) |
| `--idempotency-key <key>` | No | Retry-safety key: repeating the call returns the run it already started |

Target selection works exactly as it does for [`eval run`](#eval-run), except
that a single case run targets ONE thing — there is no `--all-targets` here.
