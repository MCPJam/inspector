...
| `--pass-threshold <0-1>` | No | Per-case pass rate: the FRACTION, 0–1, each case must pass of its own iterations (`0.9`, not `90`). Mutually exclusive with `--min-accuracy` |
...
| --- | --- | --- |
| `legacy` | `--min-accuracy <pct>` | `--min-iterations <1-10\|off>` |
...
Passing both flags of a pair is a usage error rather than a precedence rule,
and `eval update` reads the suite's own `settings.policy` before it builds a
body, so a criterion flag the suite does not use is refused locally with a
message naming the one that would work.

The API's own refusals are narrower than that, which is why the CLI checks
first: it rejects a per-case iteration count on a suite-wide suite, but stores
`settings.minimumIterations` on a per-case suite where nothing reads it. And
`settings.repetitions` plus `settings.passThreshold` in one body is how the
switch between criteria is spelled, so a caller writing PATCHes by hand can
change which criterion decides a suite while believing it edited a threshold.
No CLI flag can reach that pair.
