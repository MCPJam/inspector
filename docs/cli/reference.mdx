...
One-shot headless widget render. Uses shared connection flags plus:

| Flag | Description |
|------|-------------|
| `--tool-name <tool>` | Tool name to render |
| `--name <tool>` | Alias for `--tool-name` |
| `--tool-args <json>` | Tool parameters as inline JSON, `@path`, or `-` for stdin |
| `--params <json>` | Alias for `--tool-args` |
...
Render a widget and keep it mounted for stepping. Uses shared connection flags plus:
...
Drive a Computer-Use action on a mounted widget session.

| Flag | Required | Description |
|------|----------|-------------|
| `--session <id>` | Yes | Session ID from `apps session start` |
| `--action <type>` | Yes | `left_click`, `double_click`, `right_click`, `mouse_move`, `type`, `key`, `scroll`, `wait`, or `screenshot` |
| `--coordinate <x,y>` | No | Click/move target, e.g. `640,400` |
| `--text <text>` | No | Text to type or key name to press |
| `--scroll-direction <dir>` | No | `up`, `down`, `left`, or `right` |
| `--scroll-amount <n>` | No | Scroll amount in wheel notches (must be > 0) |
| `--duration <ms>` | No | Wait duration in milliseconds (must be ≥ 0) |
| `--screenshot-out <path>` | No | Write the post-action screenshot to a file (PNG) |
...
Close a widget session and dispose its browser.

| Flag | Required | Description |
|------|----------|-------------|
| `--session <id>` | Yes | Session ID to close |
| `--inspector-url <url>` | No | Local Inspector base URL |
