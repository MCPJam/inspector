...
| `--sandbox-image <id>` | No | Project-shared sandbox image ID (see `mcpjam images`) to pin; eval runs boot a fresh sandbox from it |
| `--file <path>` | No | Environment JSON file (or `-` for stdin) |
| `--json <json>` | No | Inline environment JSON (or `@file`, or `-`) |

\* `--name` and `--host-id` may instead be supplied inside `--file`/`--json`; explicit flags override the same key in the JSON body. Use the JSON body for the structured fields that have no flag — `serverAttachmentId`, `skillSelection`, `pluginVersionIds`, and `sandboxImageId`:
...
| `--sandbox-image <id>` | No | New sandbox-image pin (project-shared image ID); clear via `--json '{"sandboxImageId": null}'` |
| `--file <path>` | No | Environment JSON with the fields to change (or `-` for stdin) |
| `--json <json>` | No | Inline environment JSON (or `@file`, or `-`) |

Only the fields you pass change. To **clear** `serverAttachmentId`, `skillSelection`, `pluginVersionIds`, or `sandboxImageId`, send an explicit `null` in the JSON body — an empty array is rejected, it is not a way to clear:
...
Pin fields are narrower than they look:
...
- **`sandboxImageId`** must be a project-shared image (see `mcpjam images`). Personal draft images are rejected — promote them first with `images promote`.
