...
`--compose-model` is checked against the deployment's capabilities before any environment is minted or attached. On a deployment that does not support model overrides, the command fails immediately with a clear error rather than letting the write proceed and failing inside `ensure-adhoc`. Inherit-only compose (no `--compose-model`) still works on those deployments.
...
Composed (ad-hoc) environments are hidden from `environments list` but can be referenced here by ID. Pass the environment's ID directly — if the name lookup finds nothing, the command tries a direct read before reporting not found. This lets you keep a composed environment in the list alongside named ones without silently detaching it.
