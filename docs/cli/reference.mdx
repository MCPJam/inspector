...
Manage the projects and saved servers in your hosted MCPJam account. All `projects` commands require an `sk_` API key (via `--api-key` or the `MCPJAM_API_KEY` environment variable) or a prior `mcpjam login`.
...
| Flag | Description |
|------|-------------|
| `--organization-id <id>` | Restrict the listing to one organization (see `mcpjam organizations list`) |
...
| `--organization-id <id>` | No | Organization ID |
...
| `--description <text>` | No | New project description |
| `--visibility <visibility>` | No | `public` or `private` |
...
### `projects server` subcommands

Manage the saved MCP servers in a project. All `projects server` subcommands accept `--project <id-or-name>` (defaults to the most recently updated project) plus the shared platform flags.

#### `projects server list`

No additional flags beyond `--project`.

#### `projects server create`

| Flag | Required | Description |
|------|----------|-------------|
| `--project <id-or-name>` | Yes | Project name or ID |
| `--body <json>` | Yes | Server JSON body |
...
#### `projects server connect`

Connect an MCP server URL to a project, authorizing in a browser when needed.
...
When the server requires browser authorization, the command prints the handoff link to stderr and then opens it in a browser (unless `--no-browser` is set). It also prints which account and deployment the link belongs to:

```
Open this link to finish connecting:
  https://app.mcpjam.com/connect/server/...
This link belongs to marcelo@mcpjam.com on app.mcpjam.com. Open it in a browser signed in to that account.
```

The account line is best-effort — if the `/me` lookup fails, the command still prints the link and advises `mcpjam whoami`. The link is bound to the account that created it; a browser signed into a different account will be refused.

Without `--no-wait`, the command polls until the connection reaches a terminal status (`ready`, `failed`, `expired`, or `cancelled`). Ctrl-C stops the polling but not the request — the request continues in the cloud and can still be finished by opening the link.

#### `projects server connect-status`

Check a connection request started by `projects server connect`.
