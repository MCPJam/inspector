...
Check whether an MCP server's tools and widgets work on each AI host. Uses shared connection flags plus:
...
Plus shared connection flags (see `server` commands above).
