...
| `--token-endpoint-auth-method <method>` | No | Inferred from metadata | Token endpoint client authentication: `client_secret_basic`, `client_secret_post`, or `none` |
| `--assertion-format <format>` | No | `oidc` | Identity assertion format the mock IdP mints: `oidc` (ID token) or `saml` (SAML 2.0 assertion) |
| `--scopes <scopes>` | No | | Space-separated scope string |
| `--https-only` | No | off | Reject non-HTTPS / private targets |

`--assertion-format saml` is a preset that sets both identity axes: it mints a SAML 2.0 assertion and uses a `saml-nameid` subject identifier in the ID-JAG. `--assertion-format oidc` (the default) leaves both axes unset so the SDK's own defaults apply.
