...
| [`browser`](/cli/browser) | Drive a local or cloud browser and read its session trace | `open`, `navigate`, `act`, `observe`, `invoke`, `back`, `forward`, `reload`, `sessions`, `trace`, `note`, `close` |
