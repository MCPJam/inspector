...
| [`apps`](/cli/apps-conformance) | Validate MCP Apps metadata, debug app renders, and fetch widgets | `conformance`, `conformance-suite`, `debug`, `mcp-widget`, `chatgpt-widget` |
...
- [MCP Apps conformance](/cli/apps-conformance) — validate `_meta.ui.resourceUri`, debug App Builder rendering, and inspect `ui://` resources
