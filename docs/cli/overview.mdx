...
| [`registry`](/cli/reference#registry-commands) | Search MCP directories and curated cards, then install into a Cloud project | `sources`, `search`, `show`, `servers`, `connections`, `install`, `uninstall` |
