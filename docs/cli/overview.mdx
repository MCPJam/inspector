...
Also under `mcpjam cloud`: `journeys`, `personas`, `swarms`, and `studies` (which publishes an environment for outside testers and reads what they did; `scenarios` and `user-testing` are deprecated aliases for it). Full flag tables are in the [command reference](/cli/reference).
...
| `cloud eval --host`, `cloud journeys`, `cloud studies` | A **saved project Host** (`<id-or-name>`), the client config an eval or journey is stamped with. |
