...
An explicit `--api-url` or `MCPJAM_API_URL` that is not an `http(s)` URL is a usage error (exit `2`) for all Cloud commands except `mcpjam cloud status`. `cloud status` reports invalid credentials and URLs in-band — it always emits a complete JSON report, sets `ok: false`, and exits `1` instead of throwing.
