...
The `mcpjam` CLI has two modes:

- **Local MCP testing** — stateless commands for inspecting, debugging, and testing MCP servers from the command line or CI. No account required.
- **MCPJam Cloud** — account-bound commands for managing projects, running evals, exposing tunnels, and more. All Cloud commands live under `mcpjam cloud`.
...
### Local commands (no account required)
...
| [`oauth`](/cli/oauth-conformance) | Test MCP OAuth flows and conformance | `conformance`, `conformance-suite`, `login`, `metadata`, `proxy` |
...
### `mcpjam cloud` commands (MCPJam account required)

All account-bound commands live under the `mcpjam cloud` namespace. Run `mcpjam cloud --help` to see the full list.

<Note>
  `mcpjam oauth login` is MCP OAuth (authenticates you to an MCP server). `mcpjam cloud login` is your MCPJam account login. They are separate commands.
</Note>

| Subcommand | Purpose | Key commands |
|------------|---------|-------------|
| `cloud login` / `cloud logout` / `cloud whoami` | Manage your MCPJam account session | — |
| `cloud organizations` | List organizations | `list` |
| [`cloud projects`](/cli/reference#projects-commands) | Manage projects and their servers | `list`, `create`, `update`, `delete`, `servers`, `server add`, `server connect` |
| [`cloud eval`](/cli/reference#eval-commands) | Author and run eval suites | `create`, `list`, `run`, `status`, `validate`, `export` |
| [`cloud tunnel`](/cli/reference#tunnel) | Expose a local MCP server through a public MCPJam tunnel URL, registered as a server in your project | `tunnel <url>`, `tunnel -- <command>` |
| [`cloud hosts`](/cli/reference#hosts-commands) | Manage hosts in your hosted MCPJam projects | `list`, `get`, `create`, `update`, `delete`, `templates` |
| [`cloud environments`](/cli/reference#environments-commands) | Manage project environments — the host + servers + pinned skills/plugins bundles eval suites and journeys run against | `list`, `get`, `resolve`, `create`, `update`, `archive`, `restore` |
| [`cloud images`](/cli/reference#images-commands) | Manage custom Computer sandbox images (blueprints) | `list`, `get`, `create`, `validate`, `edit`, `build`, `logs`, `use`, `reset`, `promote`, `delete` |
| `cloud scenarios` | Publish and manage user-testing scenarios | `publish`, `unpublish`, `list`, `get` |
| `cloud journeys` | Manage and run Swarms journeys | `list`, `run`, `status`, `create`, `update` |
| `cloud sessions` / `cloud chat-sessions` | Search and list chat sessions | `search`, `list` |
