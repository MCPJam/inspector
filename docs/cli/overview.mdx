...
| [`apps`](/cli/apps-conformance) | Validate MCP Apps metadata, render widgets headlessly, and drive interactive sessions | `conformance`, `conformance-suite`, `render`, `session start`, `session action`, `session close` |
