...
| [`cloud link` / `cloud status`](/cli/reference#cloud-link--cloud-status-commands) | Pin a directory to a Cloud project and inspect the current credential, deployment, and project scope offline | `link`, `status` |
