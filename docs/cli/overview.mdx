...
Running `mcpjam --help` shows commands in three groups: **Local MCP testing**, **MCPJam Cloud**, and **CLI**. Running `mcpjam cloud --help` shows Cloud subcommands grouped as **Account**, **Workspace**, **Eval and environments**, and **Swarms and user testing**.
...
| [`readiness`](/cli/reference#readiness-hosted-commands) | Grade a server against a publisher's directory (local check + hosted runs) | `check`, `start claude`, `start openai`, `status`, `list`, `cancel`, `report` |
...
All Cloud commands require an `sk_` API key or a prior `mcpjam cloud login`. See [MCPJam Cloud commands](/cli/reference#mcpjam-cloud-commands) for credential requirements and the Cloud audience line.

| Group | Purpose | Key commands |
|-------|---------|-------------|
| [`cloud login` / `logout` / `whoami`](/cli/reference#login--logout--whoami-commands) | Manage your MCPJam Cloud session | `login`, `logout`, `whoami` |
| [`cloud link`](/cli/reference#mcpjam-cloud-commands) | Link a local directory to a project | `link`, `status` |
| [`cloud organizations`](/cli/reference#mcpjam-cloud-commands) | List organizations | `list` |
| [`cloud projects`](/cli/reference#mcpjam-cloud-commands) | Manage projects | `list`, `create`, `servers` |
