...
For agent/browser automation, open `http://127.0.0.1:6274/#app-builder` in the automation browser first, then run `tools call --ui --quiet --format json`. JSON/quiet `--ui` runs attach-first and will not open a system browser unless you pass `--open`. The normal JSON output is compact; pass `--debug-out <path>` for the full render envelope.
