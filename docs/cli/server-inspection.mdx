...
# If it returns oauth_required, login and save credentials
mcpjam oauth login --url https://unknown-server.com/mcp --credentials-out creds.json

# Then re-run with the credentials file
mcpjam server doctor --url https://unknown-server.com/mcp --credentials-file creds.json
```

You can also pass tokens directly if you prefer not to use a credentials file:
