...
<Steps>
<Step title="Start with doctor">
```bash
mcpjam server doctor --url https://unknown-server.com/mcp
```

<Check>
`status` is `ready`, `oauth_required`, or `error`. `ready` means you can move straight on to the tools and resources.
</Check>
</Step>

<Step title="If it returns oauth_required, login first">
```bash
...
<Step title="Re-run with the token">
```bash
mcpjam server doctor --url https://unknown-server.com/mcp --oauth-access-token $TOKEN
```

<Check>
`status` is now `ready` and the counts show the tools, resources, templates, and prompts the server exposes.
</Check>
</Step>
</Steps>
...
```bash
set -o pipefail
mcpjam server doctor --url $MCP_SERVER_URL --access-token $TOKEN --format json | jq -r '.status'
# exits 0 if ready, 1 if not
```

Without `pipefail` the pipeline reports `jq`'s exit status, which is `0` even
when `doctor` failed. Set it before the pipe, or capture the JSON first and
read `.status` from the file.

## Inspect what the server negotiated
