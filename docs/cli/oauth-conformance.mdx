...
**What an incomplete run looks like** (applicable steps could not run):

```text
OAuth conformance: INCOMPLETE
Run incomplete: 1 applicable step(s) could not run (oauth_invalid_redirect), so the run does not establish conformance
```

An incomplete run exits with code 3. It means the server was not proven conformant or non-conformant for the steps that could not run — it is distinct from a failure.
...
|-----------------|-------------|-----|
| `request_without_token` returned 200 | Server allows anonymous access or missing `WWW-Authenticate` | Return 401 with a `WWW-Authenticate: Bearer` challenge |
| `oauth_resource_metadata_challenge` failed on 2025-06-18 | `WWW-Authenticate` Bearer challenge omitted `resource_metadata` | On 2025-06-18 the header is a flat MUST — add `resource_metadata="<absolute-url>"` to the challenge |
| `oauth_resource_metadata_challenge` failed on 2025-11-25 / 2026-07-28 | Neither the `WWW-Authenticate` challenge nor either RFC 9728 well-known URI served valid metadata | Implement at least one of: (1) `resource_metadata` in the Bearer challenge, or (2) protected-resource metadata at `/.well-known/oauth-protected-resource[/<path>]` returning `application/json` with a `resource` field matching your server URL |
...
| Code | Meaning |
|------|---------|
| `0` | All flows passed (or were not applicable) |
| `1` | At least one flow failed — the server violated the spec |
| `2` | Usage error (bad flags, missing config) |
| `3` | At least one flow was incomplete — applicable steps could not run, so conformance was never established |

For suites, the worst-of ordering applies: a failed flow (exit 1) outranks an incomplete flow (exit 3).
