...
- **3 registration strategies x 4 protocol versions x 3 auth modes** = 36 possible flow combinations. The suite runner covers the matrix in one invocation.
...
When `--conformance-checks` is enabled, nine additional checks run after the flow — six negative checks that probe client-facing security, and three server-side obligation checks that verify the server meets normative spec requirements:
...
- **Unauthenticated request challenge** — sends an unauthenticated MCP request and confirms the server responds with HTTP 401 and a `WWW-Authenticate: Bearer` challenge, never a 5xx; skipped if the server accepts anonymous `initialize` (which is spec-legal)
- **Resource metadata URL in challenge** — confirms the `WWW-Authenticate` Bearer challenge includes an absolute `resource_metadata` URL (RFC 9728 §5.1); skipped on protocol version `2025-03-26`, which predates RFC 9728
- **Stale session rejection** — sends an authenticated request with an unknown `Mcp-Session-Id` and confirms the server rejects it with a 4xx (404 preferred), never a 5xx; skipped on the stateless `2026-07-28` transport
...
```json
{
  ...
  "flows": [
    ...
    { "label": "2026-07-28 / cimd", "protocolVersion": "2026-07-28", "registrationStrategy": "cimd" }
  ]
}
```
...
This adds 9 checks after the main flow: 6 negative checks (DCR redirect URI policy, invalid client rejection, invalid redirect enforcement at the authorization endpoint, invalid bearer token rejection at the MCP server, invalid redirect enforcement at the token endpoint, and token format validation) plus 3 server-side obligation checks (unauthenticated request challenge, resource metadata URL in challenge, and stale session rejection).

## Registration strategies

| Strategy | Description | Protocol versions |
|----------|-------------|-------------------|
| `cimd` | Client ID Metadata Document — the client publishes its identity at a URL | 2025-11-25, 2026-07-28 |
...
**When to choose what:**
- **CIMD** (2025-11-25, 2026-07-28): preferred for production when the auth server supports it — avoids mutable DCR state.
...
|--------------|:----------:|:-------------:|:--------------------:|
| `cimd` (2025-11-25, 2026-07-28) | yes | yes | no |
...
OAuth conformance proves a token was issued. Verification proves the token is **usable for MCP operations**.
...
| `oauth_unauthenticated_challenge` failed — "instead of 401" | Server returned 5xx on an unauthenticated request | Return HTTP 401 with `WWW-Authenticate: Bearer` for unauthenticated requests (RFC 6750 §3) |
| `oauth_unauthenticated_challenge` failed — "expected HTTP 401, received 403" | Server returns 403 instead of 401 | Return 401 with a Bearer challenge so clients can discover the auth server |
| `oauth_resource_metadata_challenge` failed — "omitted the resource_metadata parameter" | Bearer challenge missing `resource_metadata` | Add `resource_metadata="https://..."` to the `WWW-Authenticate: Bearer` header (RFC 9728 §5.1) |
| `oauth_resource_metadata_challenge` failed — "must be an absolute http(s) URL" | `resource_metadata` is a relative URL | Use an absolute `https://` URL for `resource_metadata` |
| `oauth_stale_session_rejection` failed — "instead of a 4xx" | Server crashes with 5xx on an unknown `Mcp-Session-Id` | Return 404 (or another 4xx) when the session ID is not recognized |
...
| `--protocol-version <v>` | Yes | | `2025-03-26`, `2025-06-18`, `2025-11-25`, or `2026-07-28` |
