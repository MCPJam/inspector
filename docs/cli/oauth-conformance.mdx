...
| `received_resource_metadata` — AS URL mismatch | `authorization_servers` points to a URL without AS metadata, or the value is not an absolute URL | Verify the AS URL is absolute (e.g., `https://auth.example.com`) and has `/.well-known/oauth-authorization-server`. The sequence diagram annotates non-absolute values with `(not an absolute URL)` so you can see the raw value. |
...
| `token_request` 400 `invalid_grant` | Authorization server rejected the authorization code — code expired, reused, or the redirect URI, client ID, or PKCE verifier doesn't match | Retry the flow; ensure the redirect URI and client ID are consistent end-to-end. The error message includes the server's own reason (e.g., "Token is not active") to help narrow down the cause. |
