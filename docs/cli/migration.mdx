...
| `mcpjam personas` / `journeys` / `scenarios` / `swarms` / `user-testing` | `mcpjam cloud …` (`journeys` is now `cloud goals`, `scenarios` and `user-testing` are now `cloud studies`; the old names still work as aliases) |
