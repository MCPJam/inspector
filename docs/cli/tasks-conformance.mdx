...
<Check>
Naming the tool clears the selection blocker, not the verdict. If the named tool returns a normal result instead of creating a task, or the task never becomes readable, the task-dependent checks still cannot run and the outcome stays `incomplete`.
</Check>
</Step>
...
An incomplete run carries a root `incompleteReason` naming the checks that did not run and what to change; non-quiet runs also print it to stderr. Only a `could-not-run` skip holds the verdict back; a run whose every skip is `not-applicable` still passes.
