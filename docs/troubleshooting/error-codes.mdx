...
**Likely causes**
- Refresh token was revoked or expired — server returned `invalid_grant` or `invalid_client`.
- The OAuth client registration was deleted or rotated on the authorization server.
- In hosted mode, the authorization server was unreachable when the cloud backend tried to refresh (you'll see a "Could not reach `<host>`" toast with a **Retry** option instead of **Reconnect**).

**Next steps**
- If the toast shows **Reconnect**: click it (or use the **Reconnect** button on the server card) to run a fresh OAuth flow and re-register the client.
- If the toast shows **Retry**: the authorization server was temporarily unreachable — wait a moment and click **Retry**. If it keeps failing, check whether the authorization server is online.
...
This model's provider is not enabled on MCPJam's hosted AI gateway, so MCPJam can't run it. This is a setting on MCPJam's side, not a problem with your API key. Retrying or changing your key won't help.
