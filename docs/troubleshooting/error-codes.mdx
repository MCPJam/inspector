...
**Scenario owner (share panel):** when a scenario's environment stops resolving, the tester link is withheld on the scenario detail page — the link field reads "Withheld — this scenario can't run." and both **Copy link** and **Invite** are disabled. The reason from the environment resolution service is shown inline. The link is restored automatically once the scenario points at an environment that resolves; nothing is rotated or revoked.
