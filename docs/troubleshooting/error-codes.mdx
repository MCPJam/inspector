...
When the MCPJam model limit stops one or more sessions during a swarm wave, the run screen shows an amber banner:

> **N sessions stopped at your MCPJam model limit.**

When the limit stops every session in the wave, the run-level failure banner carries the same message instead. Either way, switching models does not lift this limit — add credits or wait for the allowance to reset.
