...
### MCPJam shared free allowance unavailable

`provider/mcpjam_platform_budget`

MCPJam's shared free allowance is exhausted or temporarily paused. This limit applies across all free usage on the platform, not just your account.

**Next steps**
- Wait until the reset time shown in the error card, if one is present.
- Use purchased credits — if your organization has a paid credit balance, new requests will automatically draw from it while the shared allowance is unavailable.
- Add your own API key under **Settings → LLM Providers** to use chat without drawing from the shared allowance.

### Account suspended

`account/suspended`

This account has been suspended by MCPJam.

**Next steps**
- Contact [founders@mcpjam.com](mailto:founders@mcpjam.com) to request a review.
