...
**Next steps**
- **Playground** — a sign-in prompt appears automatically. Click **Sign in** to authenticate and get access to frontier models. Click **Not now** to dismiss and continue with a standard model. Your credit balance is not affected.
- **User Testing scenario** — if the scenario uses a frontier model, you are sent to the scenario sign-in gate instead. Sign in to continue; there is no standard-model fallback because the scenario owner chose the model.
