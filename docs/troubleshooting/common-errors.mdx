...
### OAuth refresh token failure toast

When a stored OAuth refresh token fails in the hosted app, a toast appears with details taken directly from the provider's response. The toast distinguishes two cases:

- **Could not reach `<host>`** — the authorization server was unreachable (for example, HTTP 530 or a network error). The toast shows the URL that was attempted and the HTTP status. Click **Retry** to try the refresh again without re-authorizing.
- **Refresh token declined for `<server name>`** — the authorization server responded and rejected the token (for example, `InvalidGrantError` or `refresh_token_not_found`). The toast shows the provider's own error body. Click **Reconnect** to start a new OAuth flow and obtain fresh credentials.

The existing inline error banner remains visible alongside the toast. The toast provides the specific cause and the correct next action; the banner provides the general connection status.
