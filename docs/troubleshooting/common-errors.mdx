...
4. **Right auth setting?**
   ...
   New servers default to **Auto**, which connects without credentials and escalates to OAuth on 401. For local development servers that have no auth, pick **No Authentication** to skip the escalation prompt. If you get `401`, see [Getting 401 Unauthorized errors](#getting-401-unauthorized-errors) below.
...
- **Auto** (default) - Connects without credentials first; if the server returns 401, MCPJam prompts you to continue with OAuth. Use this when you're unsure what the server requires.
...
### "Saved credentials were not sent" (403 on connect)

If a connection fails with a message like:

> "my-server" was not connected. Its saved credentials were entered for `https://old.example.com`, and it now points at `https://new.example.com`. Re-enter the credentials for the new address to connect.

This means the server's URL was changed to a **different origin** (scheme, host, or port) after credentials were saved, and MCPJam refused to send those credentials to the new destination.

When this error occurs, MCPJam automatically opens the server's **Configuration** panel so you can re-enter the credentials immediately.

**Solution:** Re-enter the server's credentials for the current URL.

1. In the **Connection overrides** or **Environment variables** section of the configuration panel, reveal and re-enter the credentials.
2. Save the server. The new credentials are now bound to the current origin.

If you see this error and did not intentionally change the server's URL, check the **Destination history** in the server detail panel to see when the origin changed and who made the edit.

### "Credential export policy" (403 on connect)

If a connection fails with a message like:

> "my-server" uses saved credentials that your organization keeps inside MCPJam-hosted connections. Connect it from MCPJam in the browser, or ask an organization admin to change the credential export policy.

This means your organization has a credential export policy that prevents saved credentials from being used outside MCPJam-hosted connections. Re-authorizing will not help — the policy applies to the credential itself, not to your session.

**Solutions:**

- **Connect from MCPJam in the browser** — open the server from [app.mcpjam.com](https://app.mcpjam.com) instead of a local inspector or CLI.
- **Ask an organization admin to change the policy** — an admin can update the credential export policy in the organization settings to allow credentials to be used from outside hosted connections.
...
- **If you're using Auto (the default):** MCPJam will prompt "Continue with OAuth?" when it receives a 401. Accept the prompt to start the OAuth flow.
- **If you're using No Authentication:** Change your authentication type to **Auto** or **OAuth 2.0** in the server configuration and go through the OAuth authorization flow.
...
### OAuth flow stops with a missing authorization server or PKCE error

Starting with the 2025-06-18 and 2025-11-25 protocol versions, connect flows fail closed when required OAuth metadata is missing rather than silently downgrading.

**"Protected resource metadata does not list an authorization server"** — The RFC 9728 `authorization_servers` field in the server's protected-resource metadata is empty or absent. The flow stops before the browser redirect because there is no server authorized to issue tokens for this resource.

Fix: ensure your authorization server's protected-resource metadata document includes `authorization_servers` with at least one entry. This is required by the 2025-06-18 profile and later.

**"Authorization server advertises PKCE without S256"** or **"does not advertise code_challenge_methods_supported"** — The authorization server's metadata does not include `S256` in `code_challenge_methods_supported`. The flow stops to prevent a silent downgrade to a weaker code challenge method.

Fix: configure your authorization server to advertise `S256` in `code_challenge_methods_supported`. Advertising `plain` alongside `S256` is fine. This check applies to the 2025-11-25 profile and later; the 2025-03-26 era is unaffected.

If you are debugging a nonconforming server and need to observe its behavior despite these failures, use the [OAuth Debugger](/inspector/guided-oauth) — it runs in observe mode and warns rather than stopping the flow.
...
### Update button does nothing after a download completes

If you click **Update** and nothing happens — or the app refuses to quit after a download finishes — this was a known bug fixed in 3.8.1. The app was continuing to check for updates even after a build was already staged, which caused the system to forget the staged build. After that, both the Update button and quitting were silently refused.

The fix is included in 3.8.1 and later. If you are on an affected version:

- **Quitting:** quit normally (Cmd+Q / Alt+F4). The app now always completes the quit even if the install fails.
- **Update button stuck:** click **Update** once. If the staged build has gone missing, the app relaunches automatically, re-downloads the update, and installs it without a second click.
- **If neither works:** use the **Reset app data and quit** option in the recovery dialog, or download the latest release manually from the [releases page](https://github.com/MCPJam/inspector/releases).
