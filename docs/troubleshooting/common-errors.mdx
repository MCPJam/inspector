...
### Desktop update button states

The update button in the sidebar changes label to reflect what the app is doing:

| Label | What it means |
|---|---|
| **Downloading…** | The update is being downloaded. No action needed. |
| **Retrying download…** | A download error occurred; the app is waiting before trying again automatically. |
| **Retry download** | Automatic retries were exhausted. Click to start a fresh download attempt. |
| **Relaunch to retry** | The download timed out and cannot be cancelled in the current session. Click to relaunch and retry. |
| **Relaunch to update** | The download finished successfully. Click to relaunch and install the update. |

Failed downloads retry automatically twice (after 30 seconds and 2 minutes of active time) before surfacing a manual action. Sleep and offline time do not count toward the retry delays or the download deadline.
