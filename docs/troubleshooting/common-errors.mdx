...
> Server "my-server" now points at `https://new.example.com`, but its saved credentials were saved for `https://old.example.com`. They were not sent. Re-enter this server's credentials for the new URL.
...
1. Open the server's edit form.
2. In the **Connection overrides** or **Environment variables** section, reveal and re-enter the credentials.
3. Save the server. The new credentials are now bound to the current origin.
