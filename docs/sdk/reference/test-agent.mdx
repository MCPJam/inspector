...
|----------|------|----------|---------|-------------|
| `tools` | `Record<string, Tool>` | Yes | - | MCP tools from `manager.getToolsForAiSdk()` |
...
The MCP tools available to the agent. Obtained from `MCPClientManager.getToolsForAiSdk()`.

```typescript
const tools = await manager.getToolsForAiSdk();
