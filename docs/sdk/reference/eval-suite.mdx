...
| `mcpjam` | [`MCPJamReportingConfig`](/sdk/reference/eval-reporting) | No | Auto-report results to MCPJam when the suite completes |

<Note>
When an API key is available — via `mcpjam.apiKey` or the `MCPJAM_API_KEY` environment variable — all test results are consolidated into a single run and reported to MCPJam after the suite completes. Individual `EvalTest` auto-saves are suppressed to avoid duplicate uploads. Set `mcpjam.enabled: false` to disable.
</Note>
...
async function main() {
  ...
  // Build suite (with MCPJam auto-reporting)
...
- [Eval Reporting Reference](/sdk/reference/eval-reporting) - Reporting APIs
