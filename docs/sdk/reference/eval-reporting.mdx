...
**Stage chain vs. verdict:** The `response` stage in the user-value chain and the iteration verdict answer different questions and are allowed to disagree. The verdict answers "did policy fail this trial"; the chain answers "what happened". Concretely:

- If a tool errors on the server during a run, the `response` stage shows `failed / toolError` (under `serverData`) regardless of what the case authored — even if the case only authored transcript predicates and nothing about tools at all. Before this, such runs could show every applicable stage green while the verdict was red, with no stage able to explain why.
- Under `failOnToolError: false` the verdict passes on the very run whose tool errored, but the `response` row is still red. Without this, a suite run entirely under that policy would report a clean funnel over servers that were failing calls.
- A transport-local failure (a span carrying an MCP error code, meaning the call never reached the server's handler) does **not** turn the `response` stage on — that is a setup fact, not the server's answer.
