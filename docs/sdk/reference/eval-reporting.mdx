...
| `matchOptions` | `EvalMatchOptions` | No | Per-result match options. When present, the inspector snapshots these onto the iteration's `testCaseSnapshot.matchOptions` so historical pass/fail computation honors them. See [match options](#evalmatchoptions). |
...
Controls how tool calls are compared when computing pass/fail for an iteration. All fields are optional; omitted fields fall back to the suite or global default.

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `toolCallOrder` | `"ignore" \| "strict"` | `"ignore"` | Whether actual tool calls must appear in the same order as expected. |
| `allowExtraToolCalls` | `boolean` | `true` | When `false`, any tool call not in the expected list fails the iteration. |
