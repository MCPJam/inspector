...
`executor` is an optional fallback for host-config pickup. Any object that exposes `getHostSnapshot()` (for example a `HostRunner` or `HostRuntime`) qualifies. The reporter reads the snapshot once and does not hold a reference to the executor. When `MCPJamReportingConfig.host` is also supplied, `executor.getHostSnapshot()` takes precedence over it; per-iteration `hostSnapshot` values take precedence over both.
...
| `host` | `Host` | No | Host configuration that drove this eval run. When provided (and the backend supports it), the SDK computes a content hash and attaches `{ hostConfig, hostConfigHash }` to the upload. Per-iteration `hostSnapshot` values take precedence; this field is the fallback for runs without per-iteration capture. Do not set `hostConfigHash` manually — the SDK fingerprints the `Host` internally. |
