...
<Warning>
**Retired.** Project API keys (`mcpjam_…`) have been shut down: the `/sdk/v1/evals/*` ingestion endpoints now return `410 Gone` for every request, keys can no longer be generated, and existing keys no longer authenticate. The APIs documented below still exist in the SDK but their uploads will fail until reporting is re-built on **MCPJam API keys (`sk_…`)** — see [API keys](/reference/api-keys). Evals themselves (`EvalTest`/`EvalSuite` runs and assertions) are unaffected.
</Warning>
