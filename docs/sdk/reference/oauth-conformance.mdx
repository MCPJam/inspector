...
| `profile` | `{ enabled?: boolean; expectSameAccount?: boolean }` | | Live profile verification options (see below) |

#### Profile verification

When `listTools` is enabled and the server exposes a designated profile tool (marked with `_meta["openai/profile"]: true`), the runner automatically probes it unless you opt out with `profile: { enabled: false }`.

Four steps run in sequence:

| Step ID | What it checks |
|---------|---------------|
| `verify_profile_shape` | The tool returns a valid profile in `structuredContent` |
| `verify_profile_stable` | A second call with the same credentials returns the same `id` |
| `verify_profile_id_opaque` | Heuristic warning when the `id` resembles display metadata (email, name); warns only, never fails |
| `verify_profile_stable_after_refresh` | After a token refresh, the profile `id` is unchanged (runs only when a refresh token is available) |

Set `profile: { expectSameAccount: true }` on flows in a suite to assert that every such flow returns the same profile `id`. A flow that declares `expectSameAccount` but supplies no profile identity fails the suite rather than silently passing.
...
To opt out of profile probing entirely:
...
### Cross-flow profile identity

Set `verification.profile.expectSameAccount: true` on flows that should all authenticate as the same account. The suite compares the `profileId` returned by each such flow and fails if they differ or if any flow supplies no identity:
  ...
  flows: [
    {
      ...
      verification: { listTools: true, profile: { expectSameAccount: true } },
    },
    {
      protocolVersion: "2025-11-25",
      registrationStrategy: "dcr",
...
When the check runs, a `verify_profile_identity_stable_across_flows` step is added to each participating flow's `steps` array.
...
| `profileId` | `string` | Profile `id` returned by the server's profile tool, if profile verification ran and succeeded |
...
| `intrusiveness` | `"active" \| "passive"` | Whether the step makes outbound calls to the server under test (`active`) or only inspects already-collected data (`passive`). Present on profile verification steps. |
