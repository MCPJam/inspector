...
console.log(result.outcome);  // "passed" | "failed" | "incomplete" | "not-applicable"
console.log(result.passed);   // true only when outcome === "passed"
...
for (const flow of result.results) {
  console.log(`${flow.outcome} ${flow.label}`);
  // outcome is "passed" | "failed" | "incomplete" | "not-applicable"
  if (flow.outcome === "incomplete") {
    console.log("  Reason:", flow.incompleteReason);
  }
}
```
...
| Property | Type | Description |
|----------|------|-------------|
| `passed` | `boolean` | `true` only when `outcome` is `"passed"` |
| `outcome` | `"passed" \| "failed" \| "incomplete" \| "not-applicable"` | Run verdict. `"incomplete"` means applicable steps could not run — conformance was never established. `"not-applicable"` means the server does not require OAuth. |
| `incompleteReason` | `string` | Present when `outcome` is `"incomplete"`: which steps could not run and what must change for them to run. |
...
| `passed` | `boolean` | `true` only when every flow is `"passed"` or `"not-applicable"`. An incomplete flow is not a pass. |
| `results` | `Array<ConformanceResult & { label }>` | Per-flow results |
| `summary` | `string` | Human-readable suite summary. Names failed flows and incomplete flows separately. |
