...
`serverUrl` is the only required config; everything else narrows or tunes the run.
...
`summary` gives you the counts in one line, and every skipped check says which kind of skip it is in `skipReason`:
...
Pass `categories` to restrict the run to whole categories, or `checkIds` to select individual checks.
...
Use a suite when one server needs several labelled runs — a core-surface pass and a security pass, for example — reported together.
...
Each `MCPCheckResult` includes a `skipReason` field whenever `status` is `"skipped"` — the two values are described under [Read the result](#read-the-result).
...
<Accordion title="Current readiness checks">
