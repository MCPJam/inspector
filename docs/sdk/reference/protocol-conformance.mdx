...
| `protocolVersion` | `McpProtocolVersion \| undefined` | The revision the run was judged against: the caller's pin, or the version the server negotiated. Absent when an unpinned run never connected. |
...
Every result carries a 0–100 conformance score. Use `scoreFromProtocolResult` to compute it from a single run, or `pooledConformanceScore` to combine scores across suites:
...
const result = await new MCPConformanceTest({ serverUrl: "..." }).run();
const score = scoreFromProtocolResult(result);

console.log(score.score);          // e.g. 98
console.log(score.applicable);     // denominator: passed + failed + couldNotRun
console.log(score.passed);         // checks that passed
console.log(score.advisories);     // readiness warnings that cost points
console.log(describeConformanceScore(score));
// "98/100 — 13/13 applicable checks passed, 1 advisory (−2) [2025-11-25]"
```

**How the score is calculated:**

- MUSTs own 95 of the 100 points: `95 × passed / applicable`
- Readiness advice owns the last 5: SHOULD/RECOMMENDED warnings deduct 2 points each, MAY warnings deduct 1, capped at 5 total
- `not-applicable` checks leave the denominator entirely — a server without auth, a stdio server's HTTP checks, or era-gated checks on the wrong version cost nothing
- `could-not-run` checks stay in the denominator as unearned points, so an incomplete run cannot reach 100
- The 95–100 band is reserved for MUST-clean, complete runs; failed or incomplete runs cap at 94
- 100 means every applicable check passed and nothing was left to advise

`score.score` is `null` when nothing was applicable (zero denominator).

The CLI prints a `Score:` line to stderr after each run (suppressed by `--quiet`):

```
Score: 98/100 — 13/13 applicable checks passed, 1 advisory (−2) [2025-11-25]
```

`toConformanceReport` attaches the score to `ConformanceReport.score`. JUnit XML output includes it in a `<properties>` block under each `<testsuite>`:

```xml
<properties>
  <property name="mcpjam.conformance.score" value="98"/>
  <property name="mcpjam.conformance.summary" value="98/100 — ..."/>
</properties>
```
