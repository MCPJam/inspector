...
| `systemPrompt`    | `string`                         | No       | `undefined` | System prompt for the LLM. An empty string (`""`) is treated as "none given" and falls back to `"You are a helpful assistant."`. |
...
Optional system prompt to guide the LLM's behavior. An empty string (`""`) is treated the same as omitting the field — both fall back to `"You are a helpful assistant."`. This matters for saved MCPJam clients, which store a missing prompt as `""`.
