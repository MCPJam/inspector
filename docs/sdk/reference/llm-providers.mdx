...
<Note>
  Claude Opus 4.7 and later, Sonnet 5, Fable 5, and Mythos 5 reject
  `temperature` with a 400 error if the field is present at all. `HostRunner`
  automatically omits it for those models — including when accessed through
  Bedrock inference profiles — so the provider uses its own default sampling.
</Note>
