...
|----------|-------|
| Prefix | `mcpjam/` |
| Environment Variable | `MCPJAM_API_KEY` (an `sk_…` key from Settings → API keys) |

```typescript
const agent = new HostRunner({
  tools,
  // The vendor model id follows the prefix.
...
Spend appears under the organization's usage. Eval runs also show an estimated cost per iteration, priced from the tokens used at MCPJam's rate for the model. It can differ slightly from the credits billed.
...
```typescript
import { releaseMcpjamModelLeases } from "@mcpjam/sdk";
