...
```typescript
pingServer(serverId: string): Promise<boolean>
```
...
`Promise<boolean>` - `true` if responsive, `false` otherwise.
...
Converts MCP tools to Vercel AI SDK format for use with `TestAgent`.

```typescript
getToolsForAiSdk(serverId?: string): Promise<Record<string, Tool>>
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `serverId` | `string` | Optional. If provided, returns tools from that server only. If omitted, returns tools from all connected servers. |

#### Returns

`Promise<Record<string, Tool>>` - Tools in Vercel AI SDK format.

#### Example

```typescript
// Tools from one server
const serverTools = await manager.getToolsForAiSdk("myServer");
...
<Note>
Tools with `visibility: ["app"]` are automatically filtered out per MCP specification.
</Note>
