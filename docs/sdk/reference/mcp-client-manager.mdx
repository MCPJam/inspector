...
| `supportsMrtr` | `boolean` | No | When `false`, simulates a client that never implemented the 2026 MRTR (`resultType: "input_required"`) pattern. On a modern (`2026-07-28`) connection, `elicitation` is removed from the advertised capabilities and MRTR retry rounds are not driven — an `input_required` result surfaces as `UNSUPPORTED_RESULT_TYPE` instead of looping. Has no effect on legacy (`2025-11-25`) connections, where elicitation is server-initiated and fulfilled by a different bridge. Defaults to `true` (MRTR enabled). |
