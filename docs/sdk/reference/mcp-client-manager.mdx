...
| `options` | `ClientRequestOptions` | Optional request options (e.g. timeout) |
| `task` | `{ ttl?: number }` | Optional. When provided, sends the call as a task-augmented `tools/call` on the 2025-11-25 legacy tasks wire. `ttl` is the task time-to-live in milliseconds. |

#### Returns

`Promise<unknown>` - The tool's return value. When `task` is provided and the server creates a task, the result is a `CreateTaskResult` containing a `task` object with a `taskId`.
...
// Task-augmented call (requires a 2025-11-25 server with tasks capability)
const taskResult = await manager.executeTool(
  "myServer",
  "long_running_tool",
  {},
  undefined,
...
console.log(taskResult.task.taskId);
```
...
- `MCPTasksWireError` if `task` is provided but the server does not speak the 2025-11-25 legacy tasks wire
...
## Tasks (2025-11-25 legacy wire)

These methods are relevant when connecting to servers that implement the in-core tasks utility from the 2025-11-25 MCP protocol version. The SDK automatically determines which tasks wire (if any) a connection speaks based on the negotiated protocol version and the server's advertised capabilities.

### getTasksWire()

Returns the tasks wire the connection speaks.
...
| Value | Description |
|-------|-------------|
| `"none"` | Tasks are not available on this connection (unknown version, pre-tasks version, or missing capabilities). |
| `"legacy"` | The 2025-11-25 in-core tasks utility is available (`params.task` on `tools/call`, `tasks/list|get|result|cancel`). |
| `"extension"` | The `io.modelcontextprotocol/tasks` extension is declared (2026-07-28+). Inert until extension support is fully implemented. |

#### Example

```typescript
const wire = manager.getTasksWire("myServer");
if (wire === "legacy") {
  // Safe to pass task opt-in to executeTool
}
```
...
Returns the MCP protocol version negotiated during `initialize`.
...
`string | undefined` — the negotiated wire literal (e.g. `"2025-11-25"`), or `undefined` if the connection has not completed initialization.

#### Example

```typescript
const version = manager.getNegotiatedProtocolVersion("myServer");
console.log(version); // "2025-11-25"
```
...
Returns `true` if the server supports task-augmented tool calls on the legacy wire.

```typescript
supportsTasksForToolCalls(serverId: string): boolean
```

Returns `false` on any protocol version other than `2025-11-25`, even if the server advertises task capabilities.

---

### supportsTasksList()

Returns `true` if the server supports `tasks/list` on the legacy wire.
...
Returns `true` if the server supports `tasks/cancel` on the legacy wire.
...
Thrown when a tasks operation is attempted against a server whose negotiated protocol version or advertised capabilities do not match the required wire. Nothing is sent to the server when this error is thrown.

```typescript
import { MCPTasksWireError, isMCPTasksWireError } from "@mcpjam/sdk";
```
...
| Property | Type | Description |
|----------|------|-------------|
| `wire` | `string` | The resolved wire value at the time of the error (e.g. `"none"`, `"extension"`). |
...
try {
  await manager.executeTool("myServer", "tool", {}, undefined, { ttl: 5000 });
} catch (err) {
  if (isMCPTasksWireError(err)) {
    console.error(`Tasks wire mismatch: ${err.wire}`);
  }
}
```
