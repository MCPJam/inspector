...
Returns MCP tools in Vercel AI SDK format for use with `TestAgent`.

```typescript
getTools(serverId?: string): Promise<Record<string, Tool>>
```
...
const serverTools = await manager.getTools("myServer");
