...
| `request` | `ExecuteToolRequest` | Optional request options |
| `taskOptions` | `TaskOptions` | Optional task options (2025-11-25 legacy wire only) |

#### ExecuteToolRequest

| Property | Type | Description |
|----------|------|-------------|
| `request` | `ClientRequestOptions` | Request options for the tool call |
| `task` | `TaskOptions` | Task options for task-augmented tool calls (2025-11-25 legacy wire only) |
| `allowTaskResult` | `boolean` | When `true`, declares per-request eligibility to receive a `CreateTaskResult` on the `io.modelcontextprotocol/tasks` extension wire (2026-07-28+). The server decides whether to return a task; a plain result is always valid. Ignored on the legacy wire; throws on `wire: "none"`. |
| `retry` | `RetryPolicy` | Explicit retry policy for tool execution |

#### Returns

`Promise<unknown>` - The tool's return value. When `allowTaskResult: true` is set on the extension wire and the server returns a task, resolves with a `CreateTaskExtResult` (a task handle with `taskId`, `status`, timestamps, etc.). Otherwise resolves with the tool's normal result.
...
// Extension wire: allow the server to return a task instead of an inline result
const maybeTask = await manager.executeTool(
  "myServer",
  "long_running_tool",
  { input: "data" },
...
if ("taskId" in maybeTask) {
  // Server returned a task — poll for completion
  const task = await manager.getTaskExt("myServer", maybeTask.taskId);
  console.log(task.status); // "working" | "completed" | ...
}
```
...
- `TypeError` if `allowTaskResult: true` is set but the server has no tasks wire

---

### getTasksSupport()

Returns the full tasks capability matrix for a connection. Use this to determine which tasks wire a server speaks and which operations are available before calling any tasks methods.
...
`TasksSupport` — an object describing the connection's tasks capabilities:

| Property | Type | Description |
|----------|------|-------------|
| `wire` | `"none" \| "legacy" \| "extension"` | Which tasks wire the server speaks. `"legacy"` = 2025-11-25 in-core tasks; `"extension"` = `io.modelcontextprotocol/tasks` (SEP-2663, 2026-07-28+); `"none"` = no tasks support. |
| `toolCalls` | `boolean` | Whether a `tools/call` may produce a task on this connection. |
| `list` | `boolean` | Whether `tasks/list` exists (legacy wire only). |
| `cancel` | `boolean` | Whether `tasks/cancel` may be sent. |
| `update` | `boolean` | Whether `tasks/update` exists (extension wire only). |
| `inlineResult` | `boolean` | Whether a completed `tasks/get` carries its result inline (extension wire only). |

#### Example

```typescript
const support = manager.getTasksSupport("myServer");

if (support.wire === "extension") {
  // Use getTaskExt / updateTask / cancelTaskExt
} else if (support.wire === "legacy") {
  // Use legacy tasks/get, tasks/list, tasks/cancel
} else {
  // No tasks support
}
```
...
Fetches a task by ID on the `io.modelcontextprotocol/tasks` extension wire (2026-07-28+). A completed task carries its `result` inline; an expired or unknown task raises the server's `-32602`.
...
|-----------|------|-------------|
| `serverId` | `string` | The server ID |
| `taskId` | `string` | The task ID returned by `executeTool` |
| `options` | `ClientRequestOptions` | Optional request options |

#### Returns

`Promise<GetTaskExtResult>` — the task with its current status. When `status` is `"completed"`, the `result` field contains the inline tool result. When `status` is `"input_required"`, the `inputRequests` field contains the pending input requests.

#### Example

```typescript
const task = await manager.getTaskExt("myServer", "task-abc");

if (task.status === "completed") {
  console.log(task.result);
} else if (task.status === "input_required") {
  // Submit responses via updateTask
}
```

#### Throws

- `TypeError` if the server does not speak the extension wire

---

### updateTask()

Submits input responses for a task on the extension wire. The result is an empty acknowledgement — re-poll `getTaskExt` for the updated status.
...
| `inputResponses` | `InputResponses` | Keyed map of input responses (partial responses are allowed) |
| `options` | `ClientRequestOptions` | Optional request options |

#### Returns

`Promise<UpdateTaskExtResult>` — an empty acknowledgement object. The ack confirms the request was accepted; it carries no task state. Re-poll `getTaskExt` for the post-update status.

#### Example

```typescript
await manager.updateTask("myServer", "task-abc", {
  k1: { action: "accept", content: { city: "Madrid" } },
});

// Re-poll for updated status
const updated = await manager.getTaskExt("myServer", "task-abc");
```
...
Cancels a task on the extension wire. Cancellation is cooperative — the server may complete the task normally or keep working. Re-poll `getTaskExt` rather than treating the ack as the task's new state.
...
`Promise<Record<string, unknown>>` — an empty acknowledgement. Re-poll `getTaskExt` to observe the actual task state after cancellation.

#### Example

```typescript
await manager.cancelTaskExt("myServer", "task-abc");

// Re-poll to confirm cancellation
const task = await manager.getTaskExt("myServer", "task-abc");
console.log(task.status); // "cancelled" or still "working"
```
