...
`MCPJAM_API_KEY` enables result uploads. If your evals connect to an authenticated HTTP MCP server and you also want MCPJam to securely persist replay credentials for debugging and reruns, attach the connected `MCPClientManager` to your `TestAgent` or pass `agent` / `mcpClientManager` to the manual reporting APIs. The SDK will derive the replay config automatically; you do not need to build a second secret object yourself.
