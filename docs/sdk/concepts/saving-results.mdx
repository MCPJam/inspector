...
Pass every turn of a multi-turn case in one call — `promptsToEvalResult([first, second, third], …)` — and the timeline merges them into one iteration. Anything you set in the overrides wins over what it derives, so `error`, `model` and `provider` stay yours to declare.
