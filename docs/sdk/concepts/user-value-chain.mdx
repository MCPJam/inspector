...
Five graders file at a link without being checks you write in a list:

| Grader | Link | What it is |
|--------|------|------------|
| Tool-call selection | Selection | `toolCalls:match` — the `expectedToolCalls` comparison for WHICH tools were called: no expected call missing, extras within `maxExtraToolCalls`. Configured by [match options](/sdk/reference/validators) rather than authored as a predicate. |
| Tool-call arguments | Tool call | `toolCalls:arguments` — HOW the expected tools were called: every expected call was made with the expected arguments. Declared only when `argumentMatching` is not `"ignore"`. Its reason names the tool and the argument key, never the value. |
...
The two tool-call graders together pass exactly when the old single row did, so a gate on the pair means what a gate on the old row meant. A case where the right tool was called with the wrong argument now shows Selection passing and **Arguments match** failing at Tool call, rather than Selection failing as if the wrong tool were chosen.
...
Runs graded before this split show no Arguments match row and render as before. After upgrading, the first run cannot be gated against a `--baseline` from before it — `eval gate --baseline <older run>` exits 3 (not gateable). Record a new baseline from a run on the current version.
