...
`client.requestEvalRunJudge({ projectId, runId, scope: "failed" })` retries only failed or ungraded iterations without rerunning the agent. Omit `scope` to use the suite's default behavior; `scope: "all"` regrades every iteration. `client.backtestEvalRunJudge({ projectId, runId, rubric })` previews a draft rubric — including `rubric.instructions` — against one iteration without changing the recorded verdict. Continue with the same rubric and `continuation: { cursor, sourceHash, reservationId }` from the preceding response; completed page retries reuse their saved result. The matching CLI is `mcpjam cloud eval judge --run <id> --scope failed`; the MCP operation is `backtest_eval_run_judge`.
