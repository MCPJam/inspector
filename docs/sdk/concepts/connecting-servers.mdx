...
- keeps using the newest refresh token the authorization server returns, **for the life of the process**

<Warning>
**A refresh token is usually single-use, so persist the rotation or the next
process cannot authorize.** Most authorization servers hand back a new refresh
token on every exchange and immediately invalidate the old one. Inside one
process the SDK handles that for you. Across processes it cannot: the value you
passed as `refreshToken` is spent, so a long-lived caller configured from a
secret — a CI job, a cron, a worker — authorizes once and then fails, with
nothing in the logs to say a credential was replaced.
...
It fires only when the token actually changed, and it never fails a connection
that has already authorized — so if the write has to be reliable, treat this as
a notification and do the work somewhere it can be retried.
</Warning>
