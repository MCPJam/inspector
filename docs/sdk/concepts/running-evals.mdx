...
The SDK provides three equal first-class reporting APIs for CI and local flows:
...
// 1) One-shot report (framework agnostic)
await reportEvalResults({
  suiteName: "Nightly",
  results: [{ caseTitle: "healthcheck", passed: true }],
});

// 2) Incremental reporter (long-running processes)
...
// 3) Artifact upload (post-step in CI)
