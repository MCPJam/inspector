...
<Warning>
**Retired.** Project API keys (`mcpjam_…`) no longer work, so result uploads to MCPJam are disabled. Local eval execution and assertions are unaffected. See [API keys](/reference/api-keys).
</Warning>

`EvalTest` and `EvalSuite` previously auto-saved results to MCPJam when `MCPJAM_API_KEY` was set. Don't wire this up in new code: the `mcpjam:` run options and the manual save APIs no longer upload anything (auto-save warns and continues; `strict` reporting throws). The [Save Results to MCPJam](/sdk/concepts/saving-results) guide documents the retired flow.
