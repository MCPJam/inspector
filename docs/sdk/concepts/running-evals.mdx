...
Predicates are authored in the corpus JSON (the Inspector's per-case **Checks** editor writes the same shape) and threaded through the runner automatically; there is no predicate field to set on individual uploaded results. A case passes iff every **required** predicate passes (`"gating"` is the legacy spelling of that role and is still accepted) — an **advisory** one is recorded and never fails the case, including when it could not be scored. A required check that could not be scored does hold the gate shut. An absent or empty list means no predicate gate.

The built-in predicate types. **Measures** is the link of the [user-value chain](/sdk/concepts/user-value-chain) the check is filed at — Connection is measured by the runner; Discovery combines runner evidence with optional catalog assertions:

| Type | Measures | What it checks |
|------|----------|----------------|
| `toolDescriptionsPresent` | Discovery | Trimmed descriptions meet the minimum length (default 20). |
| `toolAnnotationsPresent` | Discovery | Every tool declares annotations; optionally require boolean readOnlyHint and destructiveHint. |
...
| `responseCloseTo` | User value | Normalized text edit distance from a reference (bounded; not semantic similarity). |
...
| `noEndingQuestion` | User value | **Observation.** The final message does not end with a question. Advisory only |
...
| `noRepeatedIdenticalCall` | Tool call | **Observation.** No identical call repeated back-to-back. Advisory only |
...
| `noDeprecatedToolCalled` | Selection | **Observation.** No called tool's description marks it deprecated. Advisory only |
| `noDestructiveToolCalled` | Selection | No called tool declares `destructiveHint` |
| `toolErrorNamesInput` | Response | **Observation.** Tool errors name an input key or a sent value. Advisory only |
| `fullPageHasContinuation` | Response | **Observation.** Full pages carry continuation metadata. Advisory only |
