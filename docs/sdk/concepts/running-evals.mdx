...
See [Eval reporting — Tool execution and `passed`](/sdk/reference/eval-reporting/report-results#tool-execution-and-passed) for full detail and [`createEvalRunReporter`](/sdk/reference/eval-reporting/incremental#createevalrunreporter) overrides.
...
When an iteration records no widget render observations, the three `widget*` predicates fail rather than pass — a missing signal counts as a failure. See [Predicate gate reference](/sdk/reference/eval-reporting/predicates#predicate-gate) for the full type definitions and a code example.
