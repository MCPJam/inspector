...
Predicates are authored in the corpus JSON (the Inspector's per-case **Checks** editor writes the same shape) and threaded through the runner automatically — they are not a field you set on individual uploaded results. A case passes iff **all** predicates pass. An absent or empty list means no predicate gate.

The twelve built-in predicate types:
...
The three `widget*` predicates fail closed: when an iteration recorded no widget render observations in scope, they fail rather than silently pass. See [Predicate gate reference](/sdk/reference/eval-reporting#predicate-gate) for the full type definitions and a code example.
...
const host = new Host({ style: "mcpjam", model: "openai/gpt-5-nano" })
  .setComputer();
...
**Eval reporting (Stage 5).** When you upload results to MCPJam, the reporter additionally sends the canonical host config alongside each run (`{ hostConfig, hostConfigHash }`) so the persisted row in the Evals UI shows the exact host you ran with — the v1 ingest surface always accepts the pair. The snapshot comes from `executor.getHostSnapshot?.()`, falling back to `MCPJamReportingConfig.host`; uploaded results carry no per-iteration `hostSnapshot` field today, so per-iteration wire pickup is a later stage.
