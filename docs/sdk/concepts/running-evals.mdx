...
**Skill selection.** By default a host advertises all visible standalone skills. Use `skillSelection` (type `HostSkillSelection`, exported from `@mcpjam/sdk`) to restrict which skills are advertised:

```typescript
import { Host, type HostSkillSelection } from "@mcpjam/sdk";

// Restrict to an explicit list of skill ids
const host = new Host({
  style: "mcpjam",
  model: "openai/gpt-4o",
  skillSelection: { mode: "explicit", skillIds: ["sk-abc123", "sk-def456"] },
});

// Explicitly advertise no skills
const noSkillsHost = new Host({
  style: "mcpjam",
  model: "openai/gpt-4o",
...
Absent `skillSelection` and `{ mode: "all-visible" }` are equivalent at runtime and produce the same content-addressed identity — existing host configs are unaffected. `{ mode: "explicit", skillIds: [] }` (explicitly no skills) is distinct from absent and hashes differently. Skill ids are platform-internal and are stripped from external eval wire configs automatically.
