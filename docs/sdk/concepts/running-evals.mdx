...
<Warning>
**Retired.** Project API keys (`mcpjam_…`) no longer work, so auto-save uploads fail (with a warning; the run itself still completes). See [API keys](/reference/api-keys).
</Warning>
