...
<Warning>
Saving SDK eval results to MCPJam via `MCPJAM_API_KEY` is **retired** — project API keys (`mcpjam_…`) no longer work and the ingestion endpoints have been shut down. Evals still run and assert locally; see [API keys](/reference/api-keys) for details.
</Warning>
