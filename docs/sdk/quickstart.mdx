...
If `MCPJAM_API_KEY` is set, attaching the connected `MCPClientManager` also lets MCPJam securely persist replay credentials for authenticated HTTP MCP servers during eval reporting.
