...
- **Unreachable server banner in User Testing sessions** — when a tester opens a User Testing share link and one of the session's servers cannot be reached, a banner now names the server and explains that anything requiring its tools won't work in that session. Previously, every server showed a green connected indicator regardless of whether anything had actually reached it, so a tester could spend an entire session asking a model with no tools why it couldn't do the thing they were sent to try. The composer is also held until the connection checks finish, so sending a message before the session's tools are confirmed available is no longer possible. Servers that require OAuth are not affected — those belong to the existing OAuth authorization flow.
