...
- **Stale-enrichment warning no longer references "Analyze again"** — when the evidence behind a finding changed after the analysis ran, the findings panel now shows "The evidence changed after this analysis ran." instead of the previous message that referenced an Analyze button that no longer exists.
