...
- **ChatGPT template default model updated to GPT-5.6 Luna**
  - The ChatGPT host template now defaults to `openai/gpt-5.6-luna` instead of `openai/gpt-5-nano`.
  - This affects all creation paths: the template picker, home page recommended hosts, the Compat tab, the host bar, and `--template chatgpt` in the CLI.
  - Existing hosts keep their previously pinned model; only newly created ChatGPT hosts are affected.
