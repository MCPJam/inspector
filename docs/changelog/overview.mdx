...
- **Grading section on User Testing scenario settings** — The Edit route for a User Testing scenario now includes a **Grading** section (between the share settings and the action row). From there you can enable production scoring, set a sampling percentage (0–100 %) of real tester sessions to grade, and define a rubric of deterministic checks using the same check vocabulary as swarm journeys. Verdicts appear on each graded session and in Insights. Save is gated: the button stays disabled while the form is pristine, while any check row is incomplete, while the sampling field is blank or out of range, and when grading is enabled but the rubric is empty.
