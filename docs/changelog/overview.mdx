...
- **Analysis coverage chip replaces the "Built X ago" timestamp** — the freshness chip in the Session flow and Topic map panels now shows **Analyzed N of M** (for example, "Analyzed 9 of 10 · 1 analyzing") instead of a run timestamp. Opening the chip's popover shows a full coverage breakdown: deferred sessions, failure reasons, taxonomy status, and map projection progress. The **Re-analyze** button in the popover is the one place to request a fresh analysis pass; voluntary rebuild buttons no longer appear elsewhere in the panels.

- **User Testing studies no longer start analysis automatically on open** — opening a study's Findings or Insights tab when sessions exist but no analysis has run now shows "No session has been analyzed yet." instead of queuing analysis silently. Use **Re-analyze** in the freshness chip's popover to start one.
...
- **Cluster tuning controls removed** — the **Balanced** cluster tuning control and the voluntary **Rebuild clusters** button have been removed from the Session flow and Topic map panels. Error recovery (**Re-analyze**) remains available in the freshness chip's popover when sessions or projections have failed.
...
- **User Testing scenarios no longer auto-analyze on open** — opening a User Testing scenario's Findings or Insights tab when sessions exist but no analysis has run now shows "No session has been analyzed yet." instead of starting analysis automatically. To trigger analysis, use the **Re-analyze** button in the freshness chip's popover. Swarm and Benchmark scopes are unchanged.
