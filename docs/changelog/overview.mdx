...
- **Grading card removed from User Testing study Settings** — the Grading card (enable/disable toggle, session sample percentage, and Checks list) no longer appears in a study's Settings. Sharing permissions, Ratings, and Delete remain in place; Ratings now sits directly above Delete with no gap. The backend grading pipeline is unchanged — studies that already have grading configured keep their stored settings.
