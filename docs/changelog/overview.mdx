...
- **Swarm Findings headline is now the suggested fix** — when a wave has a generated summary, the suggested fix text appears directly as the summary card headline on the Findings tab. Previously it appeared below the template paragraph under a "Suggested fix:" label. When no generated summary is available, the template paragraph remains the headline as before.

- **Removed "Suggested fix is model-written" and failed-to-launch chips from Findings** — the two honesty chips that repeated launch tallies and flagged the model-written line have been removed from the Findings tab. Rate-limited session counts are still shown when present.

- **Removed per-run "Run decision not available." entries from Findings** — the Findings tab no longer lists a placeholder line for each journey run that has no decision yet. Runs without a decision are now silent.
