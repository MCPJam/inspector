...
- **Swarm grading now uses evaluator terminology throughout** — every swarm surface that previously said "check" or "rubric check" now says "evaluator". The goal grading editor is labelled **Evaluators** (was **Checks**), and its description reads "Every evaluator must pass for a session to pass." The goal completion judge blurb now states that its verdict decides whether the session passed. Session chips show `x/y evaluators passed` instead of `x/y required · z advisory findings`. The run scorecard column header, running-step completion headline ("All evaluators passed"), Findings evidence rows (`Evaluator "…" failed`), and the Findings footnote ("Evaluator findings only") are updated to match. The advisory tier has been removed from swarm rendering: Findings no longer adds stage rows for failed advisory observations, and the report panel no longer shows each evaluator's role.
