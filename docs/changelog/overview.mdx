...
- **Artifact links in v1 API responses are now signed URLs or `null`** — screenshot and recording fields (`screenshotUrl`, `videoUrl`) in eval iteration traces (`GET .../iterations/{iterationId}/trace`), eval step evidence (`GET .../iterations/{iterationId}/steps`), and chat session browser responses now carry only signed, expiring `/web/artifact` links. Any value that was a raw storage URL is now reported as `null`. If your integration reads these fields, treat `null` as "not yet available" and re-poll or re-fetch once the backend has minted a signed link. The `videoMeta` field on a step's evidence is also omitted when `videoUrl` is `null`, since metadata for an unreachable recording is not useful.
