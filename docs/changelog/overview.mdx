...
- **Platform launch announcement in the sidebar** — a compact, dismissible card appears in the bottom-left corner for all users (signed in and guests). It shows the four new platform features — Swarm, User Testing, Evaluate, and CI/CD — with a feature grid and a **Learn more** button. Clicking **Learn more** opens a dialog with a launch video tab and individual tabs for each feature, each showing a representative screenshot and a button that navigates directly to that feature inside the app. The announcement stays visible after you open the dialog; only the **✕** button permanently dismisses it. Your seen and dismissed state is stored in the browser and persists across sign-in.
