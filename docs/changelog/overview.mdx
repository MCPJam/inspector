...
- **Client · model chip in the session detail header** — the session detail header in both Swarm and User Testing now shows a `Client · Model` chip beside the session name. The client is the product's brand word ("Claude", "ChatGPT") from the session's pinned host config — never the host row's nickname. The model is resolved to its curated display name (e.g. "GPT-5", not "openai/gpt-5"). The chip renders nothing when the session recorded neither; it shows whichever half is known when only one is available.

- **Labeled "Share this session" button** — the icon-only copy button in the session detail header has been replaced with a labeled **Share this session** button. It copies the same session deep link as before; hovering it shows a tooltip describing what was copied.
