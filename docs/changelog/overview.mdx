...
- **OAuth debugger names every authorization-server metadata URL it tried** — when every candidate metadata URL returns a 4xx, the debugger now lists each URL it attempted and what each one returned, instead of showing "Last error: null". This makes it clear which discovery paths were tried and why each failed.

#### Added

- **Run disclosure shows analysis provider retention policy** — the run-disclosure tooltip and `mcpjam eval run` output now include an **Analysis providers:** line when the backend reports provider retention facts. The line describes the zero-data-retention and no-training policy sent with every platform-key analysis call, and what it does not cover. Backends that predate this field omit the line entirely.
