...
### Release rollup — 2026-07-09

Minor releases of `@mcpjam/inspector` and `@mcpjam/sdk`; patch release of `@mcpjam/widget-react`.

#### Added

- **Voice messages and audio transcription**
  - Inspector now supports sending voice messages and receiving audio transcriptions in the chat interface.

- **MCP Apps / widget improvements**
  - Download confirmation dialog before installing MCP App widgets.
  - Widget runtime behavior fixes and spec/security polish.

#### Improved

- **Evals**
  - Better error surfacing in Cloud Skills, evals, and swarm execution flows.
  - Pinned-turn support for more deterministic eval replays.
  - Run-source scoped CI views for clearer per-commit eval results.

- **Host catalog and compatibility**
  - Image-support mappings and generated fallback catalog support.
  - Host update affordances and safer catalog fallback handling.
