...
- **User Testing scenario description moved to Edit** — the description field no longer appears in the scenario header. To read or edit a scenario's description, open the scenario and go to **Edit → Description**. The field saves on blur or when you leave Edit. Existing descriptions are preserved and remain editable; the header now shows only the scenario name and tab strip.
