...
- **Editing a yes/no question uses the same modal as Add** — clicking an existing question column title now opens the same dialog used for adding a question, including the Yes/No reminder. The inline header form is gone. The dialog title reads **Edit a yes/no question** and the submit button reads **Save**; cancelling or pressing Escape leaves the chart unchanged.
