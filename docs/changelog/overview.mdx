...
- **Run tab moved to last on the swarm run detail page** — the tab strip is now **Findings · Insights · Sessions · Run**. Findings leads because it is what you want from a settled wave; Run trails because it is the live watch surface, reached automatically for an in-flight wave rather than by clicking. Landing is unchanged: a live wave with no `?tab=` still opens Run, `?tab=run` still works on a finished wave, and `?session=` deep-links still open Sessions.
