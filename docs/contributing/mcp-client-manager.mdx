...
<ResponseField
  name="executeTool"
  type="(serverId: string, toolName: string, args: Record<string, unknown>, options?, taskOptions?) => Promise<CallToolResult | CreateTaskResult>"
>
  Execute a tool on a specific server. Pass `taskOptions` to execute as a background task. Returns `CreateTaskResult` with task info if executed as task, otherwise returns `CallToolResult`.
</ResponseField>
...
<ResponseField
  name="listTasks"
  type="(serverId: string, cursor?, options?) => Promise<ListTasksResult>"
>
  List all tasks for a server. Returns empty list if server doesn't support `tasks/list`.
</ResponseField>

<ResponseField
  name="getTask"
  type="(serverId: string, taskId: string, options?) => Promise<Task>"
>
...
<ResponseField
  name="getTaskResult"
  type="(serverId: string, taskId: string, options?) => Promise<unknown>"
>
  Get the result of a completed task. Returns the underlying operation result (e.g., CallToolResult for tool calls).
</ResponseField>
...
<ResponseField
  ...
  type="(serverId: string, taskId: string, options?) => Promise<Task>"
>
  Cancel a running task. Only available if server supports `tasks/cancel` capability.
</ResponseField>

<ResponseField
  name="getServerCapabilities"
  type="(serverId: string) => ServerCapabilities | undefined"
>
  Get server capabilities to check task support.
</ResponseField>

<ResponseField
  name="supportsTasksForToolCalls"
  type="(serverId: string) => boolean"
>
  Check if server supports task-augmented tool calls (`tasks.requests.tools.call` capability).
</ResponseField>

<ResponseField
  name="supportsTasksList"
  type="(serverId: string) => boolean"
>
  Check if server supports `tasks/list` operation.
</ResponseField>

<ResponseField
  name="supportsTasksCancel"
  type="(serverId: string) => boolean"
>
  Check if server supports `tasks/cancel` operation.
</ResponseField>
...
if (manager.supportsTasksForToolCalls("myserver")) {
  // Execute tool as background task with 60 second TTL
  const result = await manager.executeTool(
    "myserver",
    "long_running_operation",
    { input: "data" },
    undefined, // options
    { ttl: 60000 } // taskOptions
  );
  ...
  if ("task" in result) {
    ...
    const task = await manager.getTask("myserver", result.task.taskId);
    console.log("Task status:", task.status);
    
    // Get result when completed
    if (task.status === "completed") {
      const taskResult = await manager.getTaskResult("myserver", result.task.taskId);
