...
>
  ...
  tool has `_serverId` metadata attached. App-only tools (`_meta.ui.visibility =
  ["app"]`) are excluded by default per SEP-1865.
</ResponseField>
...
<ResponseField name="includeAppOnly" type="boolean">
  - `includeAppOnly?: boolean` - When `true`, includes SEP-1865 app-only tools
    (`_meta.ui.visibility = ["app"]`) in the returned tool set. Defaults to
    `false`. Use only when intentionally mirroring a host that does not
...
<CardGroup cols={2}>
  ...
  <Card title="Elicitation Handling" icon="message-circle">
    **DO**:
