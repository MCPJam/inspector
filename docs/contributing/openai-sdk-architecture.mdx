...
All widget utilities are consolidated in `server/routes/mcp/openai.ts`. Located at lines 14-46:
...
- `client/src/components/tools/ResultsPanel.tsx` - Detects OpenAI components using tool definition metadata
...
- `server/routes/mcp/openai.ts` - Widget storage, serving, OpenAI bridge injection, and all widget utilities
- `server/routes/mcp/http-adapters.ts` - HTTP adapter routes (widget handling removed, uses centralized openai.ts)
