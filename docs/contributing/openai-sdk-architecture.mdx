...
This guide explains how MCPJam Inspector implements the OpenAI Apps SDK to render custom UI components for MCP tool results. This enables MCP server developers to create interactive visualizations for their tool outputs.
...
MCPJam Inspector supports the [OpenAI Apps SDK](https://developers.openai.com/apps-sdk), letting MCP tools return custom UI components that render in iframes with a sandboxed `window.openai` API bridge.
...
MCPJam Inspector implements a **triple-iframe architecture** that matches ChatGPT's implementation for compatibility and security isolation:
...
The triple-iframe design provides cross-origin isolation:
...
The system uses a triple-iframe architecture with cross-origin isolation:
...
This example traces a full interaction where a widget calls a tool:
