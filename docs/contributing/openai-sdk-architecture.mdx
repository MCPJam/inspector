...
4. Inject `window.openai` API runtime script with:
...
**Runtime Script Architecture (PR #1044):**

As of PR #1044, the `window.openai` API bridge is implemented as a separate TypeScript file that gets bundled and injected into widget HTML. This provides better type safety and maintainability compared to the previous inline script approach.

The runtime script is located in `server/routes/apps/widget-runtime.ts` and is bundled into `server/routes/apps/chatgpt.bundled.ts` using esbuild. When making changes to the widget runtime:

1. Edit `server/routes/apps/widget-runtime.ts`
2. Run `npm run bundle:chatgpt` to regenerate the bundled script
3. Commit both files to source control

The bundled script is then injected into widget HTML via a JSON configuration element:

```html
<script type="application/json" id="openai-runtime-config">
{
  "toolId": "...",
  "toolName": "...",
  "toolInput": {...},
...
<script>/* Bundled runtime script */</script>
```

This architecture separates configuration data from runtime code, making it easier to maintain type safety and test the widget bridge independently.
...
The `window.openai` API bridge is implemented in `server/routes/apps/widget-runtime.ts` and bundled into `server/routes/apps/chatgpt.bundled.ts`. The runtime script reads configuration from a JSON element and bootstraps the API:
...
2. **`window.openai` is undefined**
   - Verify bundled runtime script is injected in widget HTML
   - Check that `openai-runtime-config` JSON element exists
   - Check browser console for CSP violations or script errors
   - Ensure `<head>` tag exists in HTML for injection
   - Verify `chatgpt.bundled.ts` is up to date (run `npm run bundle:chatgpt`)
...
1. Update the widget runtime script (`server/routes/apps/widget-runtime.ts`)
2. Run `npm run bundle:chatgpt` to regenerate the bundled script
3. Add postMessage handler in parent (`client/src/components/chat-v2/chatgpt-app-renderer.tsx`)
4. Update TypeScript types if needed
5. Commit both the source and bundled files

**Example: Adding a custom method:**

```typescript
// 1. Widget runtime (server/routes/apps/widget-runtime.ts)
const openaiAPI: OpenAIAPI = {
  // ... existing methods ...
  
  customMethod(options: any) {
    window.parent.postMessage({
...
// 2. Run bundling command
// $ npm run bundle:chatgpt

// 3. Parent-side handler (client/src/components/chat-v2/chatgpt-app-renderer.tsx)
...
<Warning>
  Always run `npm run bundle:chatgpt` after editing `widget-runtime.ts`. The bundled file must be committed to source control for production builds.
</Warning>
...
- `server/routes/apps/chatgpt.ts` - Widget storage, serving, and content injection (moved from `/mcp/openai` to `/apps/chatgpt` in PR #1044)
- `server/routes/apps/widget-runtime.ts` - **TypeScript source for `window.openai` API bridge** (new in PR #1044)
- `server/routes/apps/chatgpt.bundled.ts` - **Auto-generated bundled runtime script** (new in PR #1044, must be committed)
- `server/routes/apps/chatgpt-sandbox-proxy.html` - Middle iframe proxy for cross-origin message relay
- `server/routes/apps/index.ts` - Mounts apps routes at `/api/apps`
- `scripts/bundle-openai-script.js` - **Bundling script for widget runtime** (new in PR #1044)
