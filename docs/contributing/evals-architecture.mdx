...
External results enter through these `POST` endpoints, implemented as a thin proxy in `mcpjam-inspector/server/routes/v1/eval-ingest.ts` over the Convex `/v1/evals/ingest/*` surface:
...
| `POST /api/v1/projects/:projectId/eval-ingest/artifacts` | Store one large blob (widget HTML snapshot) sent as raw bytes; returns its `storageId` |
| `POST /api/v1/projects/:projectId/eval-ingest/artifacts/upload-url` | Mint an upload URL for large blobs (older SDK releases) |
...
- The proxy caps JSON request bodies at 6 MiB (the backend enforces its own 5 MiB limit) and artifact bodies at 20 MiB, and times out after 60 s; status and body pass through verbatim. For `artifacts`, the project travels as a `?projectId=` query parameter because the body is the artifact itself.
...
- **Flow** — a result whose widget HTML would push it past the 1 MiB request limit first uploads that HTML (via `artifacts`); then, if the run fits in one request (≤ 200 results and ≤ 1 MiB), a single `report` call; otherwise `runs/start` → chunked `runs/iterations` → `runs/finalize`.
