...
Source order, run-level only: `iteration.hostSnapshot` → `executor.getHostSnapshot?.()` → `MCPJamReportingConfig.host`. Pass-1 homogeneity gate: send only when all available iteration snapshots canonicalize to the same hash; heterogeneous runs omit (per-iteration wire support is a later stage). Any error while resolving or canonicalizing the snapshot logs a warning and omits the pair rather than failing the upload.
...
- **Retry** — 429/5xx, network errors, and timeouts retry with fixed delays (250/750/1750 ms, ±20% jitter). Billing-limit errors never retry. If `runs/start` reports the run already completed (idempotent reuse), the reporter short-circuits.
