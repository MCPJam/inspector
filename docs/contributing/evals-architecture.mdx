...
<Note>
**Licensing**: The evals system (`/server/services` and `/server/routes/mcp/evals.ts`) is licensed under the MCPJam Enterprise Edition (EE) license. Production use requires a valid EE subscription. Development and testing use is permitted without a subscription. See [server/evals/LICENSE](https://github.com/MCPJam/inspector/blob/main/server/evals/LICENSE) for full terms.
</Note>
