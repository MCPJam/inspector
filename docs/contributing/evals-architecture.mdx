...
└─ SuiteIterationsView (Detailed view)
   ├─ SuiteEnvironmentEditor (edit mode — server config)
   ├─ SuiteExecutionConfigEditor (edit mode — default model/prompt/temperature)
...
```typescript
{
  ...
  /**
   * Set to true for reruns of already-persisted suites. Skips the per-case
   * upsert so suite-default-derived fields (substituted model, merged
   * advancedConfig) are not baked into per-case overrides.
   */
  suiteRerun?: boolean
}
```
...
type EvalSuite = {
  ...
  /** Suite-wide default model/prompt/temperature. Per-case advancedConfig takes precedence. */
  defaultConfig?: {
    modelId: string;
    provider?: string;
    systemPrompt: string;
...
| **defaultConfig**    | Suite-level default model, system prompt, and temperature; overridden by per-case `advancedConfig` |
| **suiteRerun**       | Flag on `RunEvalsRequest` that skips per-case upserts during reruns to preserve suite-default semantics |
