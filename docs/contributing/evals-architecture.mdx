...
#### **Evals Runner** (`server/services/evals-runner.ts`)

The core orchestrator that executes evaluation tests using AI SDK.

**Entry Point:**

- `runEvalSuiteWithAiSdk()`: Executes eval suite with AI SDK integration

**Execution Flow:**

```mermaid
flowchart TD
    A[Start runEvalSuiteWithAiSdk] --> B[Validate tests]
    B --> C[Create RunRecorder with Convex auth]
    C --> D[Get tools from MCPClientManager]
    D --> E[Ensure suite in DB]

    E --> F{For each test case}
    F --> G[Record test case in DB]
    G --> H{For each run iteration}
    H --> I[Start iteration in DB]
    I --> J[Build model definition]
    J --> K{Is MCPJam model?}

    K -->|Yes| L[runIterationViaBackend]
    K -->|No| M[runIterationWithAiSdk]

    L --> N[Execute agentic loop]
    M --> N

    N --> O[Evaluate results]
    O --> P[Finish iteration in DB]

    P --> Q{More iterations?}
    Q -->|Yes| H
    Q -->|No| R{More test cases?}
    R -->|Yes| F
    R -->|No| S[Complete suite]
```

**Agentic Loop (AI SDK Models):**

```mermaid
sequenceDiagram
    participant Runner as evals-runner.ts
    participant AI as AI SDK
    participant MCP as MCPClientManager
    participant DB as Database

    Runner->>AI: generateText(messages, tools)
    AI-->>Runner: Response with tool calls

    Runner->>Runner: Extract tool names
    Runner->>Runner: Evaluate: expected vs actual
    Runner->>DB: Update iteration with results
```

**Agentic Loop (Backend Models):**

```mermaid
sequenceDiagram
    participant Runner as evals-runner.ts
    participant Backend as MCPJam Backend
    participant MCP as MCPClientManager
    participant DB as Database

    loop Up to 20 steps
        Runner->>Backend: POST /stream (mode: step)
        Backend-->>Runner: Response with tool calls
        Runner->>Runner: Extract tool names

        alt Has unresolved tool calls
            Runner->>MCP: Execute tool calls
...
- Max 20 conversation steps for backend models to prevent infinite loops
- Token usage tracking (input + output tokens) for AI SDK models
- Duration measurement
- Tool call recording
- Dual execution paths: AI SDK for external providers, backend for MCPJam models
