...
└─ SuiteIterationsView (Detailed view)
   ├─ SuiteDashboard
   │  ├─ SuiteRunsChartGrid (accuracy sparkline + model bar chart)
   │  ├─ SuiteInsightsCollapsible (AI narrative)
   │  ├─ SuiteRunsList (run history)
   │  └─ TestCasesOverview
   │     └─ CrossHostDashboard (when ≥2 hosts attached)
   │        └─ CrossHostMatrix (per-case × per-host grid)
