...
- API Clients:
  - `client/src/lib/mcp-tools-api.ts`
  - `client/src/lib/mcp-prompts-api.ts`
...
// 2. API client calls backend
// client/src/lib/mcp-prompts-api.ts:12-29
...
// 2. API client calls backend
// client/src/lib/mcp-prompts-api.ts:31-54
...
// 3. Backend converts args to strings
// server/routes/mcp/prompts.ts:80-126
...
The Prompts Tab uses a dedicated API client (`mcp-prompts-api.ts`) that provides type-safe, error-handled wrappers around the backend endpoints:
...
// Batch listing from multiple servers
...
- Consistent API patterns across Tools and Prompts
...
<ResponseField name="POST /api/mcp/prompts/list-multi" type="{ serverIds: string[] }">
  Batch list prompts from multiple servers in parallel.
...
{
  prompts: Record<string, MCPPrompt[]>;
  errors?: Record<string, string>; // Per-server errors
}
```

**Note**: Returns partial results even if some servers fail. Check `errors` field for failures.
...
### Batch Prompt Listing

The `/api/mcp/prompts/list-multi` endpoint enables efficient parallel fetching of prompts from multiple servers:

```typescript
// server/routes/mcp/prompts.ts:30-78
prompts.post("/list-multi", async (c) => {
  const { serverIds } = await c.req.json();
  ...
  await Promise.all(
    serverIds.map(async (serverId) => {
      try {
        ...
        console.error(`Error fetching prompts for server ${serverId}:`, error);
        errors[serverId] =
...
- **Parallel execution**: Uses `Promise.all` to fetch from all servers simultaneously
- **Partial success**: Returns results for successful servers even if some fail
- **Error tracking**: Per-server error messages in `errors` field
- **Graceful degradation**: Failed servers return empty arrays

**Use Cases**:

- Dashboard views showing prompts across all connected servers
- Bulk operations on multiple servers
- Server comparison and discovery tools

**Example Usage**:

```typescript
// Fetch prompts from multiple servers
const { prompts, errors } = await listPromptsForServers([
  "server-1",
  "server-2",
  "server-3",
]);
...
// prompts = {
//   "server-1": [{ name: "prompt1", ... }],
//   "server-2": [{ name: "prompt2", ... }],
//   "server-3": []
// }
// errors = {
//   "server-3": "Connection timeout"
// }
```
