...
This guide gets you up to speed on the MCPJam Inspector codebase and ready to contribute.
...
<Step title="Set Environment Variables">
  Copy the sample env file into the inspector workspace, then add your provider
  keys to it. The path matters: the dev server and the Electron app both read
  `mcpjam-inspector/.env.development`, not a `.env` at the repo root. ```bash cp
  mcpjam-inspector/.env.local mcpjam-inspector/.env.development #
...
npm run electron:make -w @mcpjam/inspector
```

Forge builds for the host platform; there are no per-platform scripts. The macOS
and Windows installers come from the `mac-release` and `windows-release`
workflows. Packaging needs the Node version in `mcpjam-inspector/.nvmrc` — on
Node 25+ forge exits 0 without writing anything, so `electron:make` checks the
version first and prints the fix.
