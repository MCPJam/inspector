...
The XAA Debugger supports negative test modes (for example, "Bad Signature" or "Unknown KID") that deliberately send a broken assertion to your authorization server. In these modes, a rejection from the authorization server is the **expected, correct outcome** — not a failure.

### How the stepper and scorecard agree

Both the run-rail stepper and the scorecard use the same pass/fail logic in negative mode:

| Authorization server response | Outcome | Stepper chip | Banner |
|---|---|---|---|
| Rejects the broken assertion (non-2xx) | **Pass** | Green | "Correctly rejected — exactly what should happen" |
| Issues a token anyway (2xx) | **Security risk** | Red | "Accepted a broken assertion — security risk" |

When a negative test completes, the "Continue" button changes to **"Negative test complete"** and is disabled, because the run has reached its terminal outcome.

### What the banners mean

**Green — Correctly rejected**
Your authorization server rejected the broken assertion with a non-2xx status. This is the pass condition for a negative test: the server caught the deliberate flaw. The scorecard reports this as a pass for the same reason.

**Red — Accepted a broken assertion**
Your authorization server issued an access token for the broken assertion. This is the failure the negative test checks for — the server should have rejected it. The debugger stops at the token step and does not use the token against the MCP server.

The happy-path (valid) flow is unaffected: a rejection in valid mode is still reported as an error.
