...
**Authorization Server Issuer Mismatch (`409 VALIDATION_ERROR`)**
- The stored server configuration expects a specific authorization server issuer, but the issuer advertised in the server's metadata is different. The token exchange is aborted and the stored client secret is never sent. The error message names both the expected issuer and the advertised issuer.
- **To fix:** update the server's Authorization Server issuer setting to match the advertised value, or correct the server URL so it points at the right authorization server.
- **TLS-proxy variant:** if only the scheme differs (e.g. the stored issuer is `https://` but the metadata advertises `http://`), the authorization server is likely behind a TLS-terminating proxy that advertises an `http://` issuer. Update the issuer in the server config to the `http://` value the metadata actually returns, or configure the authorization server to advertise its public `https://` issuer.
