...
<Warning>
**Hosted issuer URL changed to `https://`**

The hosted XAA identity provider at `app.mcpjam.com` now advertises an `https://` issuer and JWKS URI. If you previously configured a downstream authorization server to trust the `http://app.mcpjam.com/api/web/xaa` issuer, update that configuration to `https://app.mcpjam.com/api/web/xaa`. The issuer URL shown in the Register Issuer dialog already reflects the correct `https://` value.
</Warning>
