...
The **Negative-test scorecard** sends deliberately invalid ID-JAGs to your
authorization server and checks that it refuses to issue an access token for
each one. Run a successful flow first to unlock it.
...
A test passes when the authorization server rejects the broken assertion. It
fails when the authorization server issues an access token for one.

The scorecard checks these 11 cases:
