...
The header also shows an **Identity assertion** toggle that lets you switch between **OIDC** and **SAML** assertion modes for the selected server. OIDC (the default) mints a standard OpenID Connect ID token as the identity assertion. SAML mints a SAML 2.0 assertion instead — the ID-JAG still carries a JWT signed by this issuer, but the subject identifier uses the `saml-nameid` format. Choose the mode that matches what your authorization server expects. This setting is saved per server and applies only to bar-server runs; runs against a registered app always use OIDC.
