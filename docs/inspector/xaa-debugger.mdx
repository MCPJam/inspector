...
The **Override identity for this server** fields let you set a specific subject
(`sub`) and email that MCPJam, acting as the test identity provider, puts in
the ID token for this server. Both fields must be filled or both must be empty
— a partial pair blocks the save with an actionable error.

Leave both fields blank to use the identity resolved from the following
precedence order:

1. **Selected person** (Run as) — the person selected in the XAA Flow tab, if any.
2. **Server override** — the subject and email saved on this server's configuration.
3. **Project default** — the project-level XAA test identity default set by a project admin (Convex-backed projects only).
4. **MCPJam demo identity** — the terminal fallback when none of the above is configured.
