...
2. **Select a target** - Choose a server from the server bar or select a registered app (see [Active target](#active-target) below)
...
The XAA Debugger always has one active target at a time. The current target is shown in the header bar as **Target: `<name>` · from server** or **Target: `<name>` · registered app**, depending on how it was selected.

There are two ways to set the active target:

- **Server bar** — Click a server chip in the bar. The badge reads **· from server**.
- **Registered app** — Select an app from the registered apps list. The badge reads **· registered app** and a notice appears: _Using registered app — overrides the bar selection_.

These two sources are mutually exclusive. Selecting a server chip from the bar clears any active registered-app selection. When a registered app is active, a **Use bar server** button appears in the header; clicking it returns to the bar-server target.

If no server is selected, the header shows **No server selected** and the run controls are disabled.
