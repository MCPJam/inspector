...
- **Okta** - Native jwt-bearer support. Register MCPJam as a trusted identity issuer using the Issuer URL or JWKS URL from the **Use MCPJam as your test IdP** card.
...
- **Actions** - Buttons to fix the issue (Configure Target, Reset Flow)
...
- The AS returned `invalid_grant`. Common causes: MCPJam isn't registered as a trusted issuer, the `aud` claim doesn't match the AS issuer, or the `resource` claim isn't recognized. Use the **Use MCPJam as your test IdP** card (always visible at the top of the tab) to copy the Issuer URL and JWKS URL for registering trust.
...
To start fresh, open the target configuration modal and click **Clear configuration**. This deletes the saved profile and returns the tab to the initial "Configure XAA Target" screen. The auto-fill from a connected server is suppressed after a clear, so the tab stays empty until you configure a new target.
...
- See [Use MCPJam as a Test IdP](/inspector/xaa-test-idp) to register MCPJam's issuer and JWKS with your authorization server
