...
The **Identity** button in the run bar lets you control the simulated user that MCPJam's built-in IdP mints a mock ID token for before exchanging it for an ID-JAG. This identity applies globally to every target.

Click **Identity** to open the popover and set:

- **Subject (sub)** - The `sub` claim stamped on the mock ID token (default: `user-12345`)
- **Email** - The `email` claim on the mock ID token (default: `demo.user@example.com`)

A blue dot on the Identity button indicates a non-default identity is active. Settings persist across sessions.
