...
## Connecting an XAA server from the Connect page

You can connect to an XAA-protected MCP server directly from the **Servers** tab without going through the XAA Debugger first. When you add or edit a server, select **Cross-App Access (XAA)** from the authentication dropdown. This reveals the following fields:

- **Identity provider** — locked to **MCPJam test identity provider** in this release.
- **Client ID** (required) — the client ID registered with the server's authorization server.
- **Client Secret** (optional) — for confidential clients.
- **Scopes** — space-separated scopes to request.
- **Advanced** (collapsed by default):
  - **Authorization Server Issuer** — leave blank to auto-discover from the server's protected-resource metadata (RFC 9728). Set this explicitly if auto-discovery fails.
  - **Simulated identity** — the subject (`sub`) and email the MCPJam test IdP mints for the mock login. Defaults to your signed-in account when left blank.

When you click **Connect**, MCPJam mints the access token server-side and injects it as a Bearer header. If the server returns a 401, the token is re-minted once automatically; a second 401 surfaces an error with an **Open XAA Debugger** shortcut in the toast.

A server configured this way also appears in the XAA Debugger's server selector, so you can step through the full handshake without re-entering its credentials.
...
2. **Configure Target** - Provide your MCP server URL and authorization server details, or select a server already configured with the Cross-App Access auth type from the Servers tab
