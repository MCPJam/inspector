...
2. **Select a server** - Pick an HTTP + OAuth server from the server bar at the top. STDIO servers and servers without OAuth enabled show a not-testable state and cannot be run. If you need to add or adjust a server, click **Configure Server to Test**.
...
<Note>
If you have a registered resource app, selecting it in the registration panel overrides the bar server selection for that run. A banner appears letting you switch back to the bar server at any time.
</Note>
...
- **Actions** - Buttons to fix the issue (Configure Server to Test, Register Issuer, Reset Flow)

### Common Error Scenarios

**Client ID Required**
- The ID-JAG needs a `client_id` claim. Configure it via **Configure Server to Test** and register the client at your authorization server.
