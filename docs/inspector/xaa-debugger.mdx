...
The XAA (Cross-App Access) Debugger tests an MCP server and its authorization setup. It runs the whole cross-app access exchange for you and shows where it breaks, so you can tell an untrusted issuer apart from a rejected grant apart from a server that refuses a perfectly valid token.
...
```mermaid
flowchart TD
    S1["1. Discover the MCP server<br/>and authorization server"] --> S2["2. Resolve the client identity<br/>pre-registered, DCR, or CIMD"]
    S2 --> S3["3. Simulate sign-in at MCPJam's<br/>identity provider"]
    S3 --> S4["4. Request an ID-JAG"]
    S4 --> S5["5. Request an access token<br/>JWT-bearer grant"]
    S5 --> S6["6. Call the MCP server"]
```
...
<Steps>
<Step title="Trust MCPJam's identity provider">
...
<Check>
Your authorization server accepts MCPJam's issuer and can fetch its JWKS.
</Check>
</Step>
...
Pick a **Registration method** — pre-registered client, CIMD, or open dynamic registration — and, optionally, scopes. Each option is described under [Registration methods](#registration-methods).

<Check>
The server is configured and discovery finds its authorization server. Compatibility results appear after discovery.
</Check>
</Step>

<Step title="Inspect the ID-JAG before it is sent">
...
<Check>
The inspector's checks pass, and `aud`, `resource`, and `client_id` name the parties you expect. Claim-by-claim detail is in the [ID-JAG reference](#id-jag-reference).
</Check>
</Step>

<Step title="Locate a failure">
...
<Check>
You can name the failing step and see its exact request and response. A green flow through step 6 means your MCP server accepted a token minted from an ID-JAG — the next thing to check is what your server does with a *broken* one, under [Negative tests](#negative-tests).
</Check>
</Step>
</Steps>
...
The XAA Flow header has an **Identity assertion** (OIDC / SAML) toggle that sets the format MCPJam mints. Choose the protocol MCPJam's identity provider uses to authenticate users.
...
Leave it blank to use the authorization server MCPJam discovers from the MCP server. Enter a URL to use a different authorization server.

#### Path-scoped authorization server

Keep this off for normal RFC 8414 behavior. Turn it on only when the discovered URL and metadata issuer differ on the same origin.

#### Simulated identity

Choose the user MCPJam puts in the ID token. Leave the fields blank to use a built-in demo user (`user-12345` / `demo.user@example.com`). The header's **Run as** control chooses the simulated identity.
