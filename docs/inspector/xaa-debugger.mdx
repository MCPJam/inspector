...
1. **Configure a target** — Provide the MCP server URL, a client ID, and (for your own authorization server) its issuer. Start here.
2. **Run the flow** — MCPJam mints an ID-JAG; your authorization server redeems it for an access token, one step at a time.
3. **Trust MCPJam at your auth server** *(only when targeting your own authorization server)* — Expand the **Use MCPJam as your test IdP** card at the top of the tab and register the IdP endpoints so your authorization server accepts the ID-JAG MCPJam mints.
