...
3. **Simulate sign-in at MCPJam IdP** - MCPJam simulates the user signing in at its identity provider to obtain an ID token
...
- **Okta** - Native jwt-bearer support. Register MCPJam as a trusted identity issuer using the JWKS URL from the **Before you run this test** setup guide.
...
- **IdP** - MCPJam's identity provider (MCPJam IdP, for testing)
...
- Click **Before you run this test** in the MCPJam IdP card to get the JWKS URL and step-by-step setup guidance for configuring issuer trust
