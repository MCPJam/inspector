...
2. **Select or add a target server** - Choose an existing server from the server picker, or click **Add Server +** to save a new server config. On the XAA tab, adding a server saves the configuration and selects it as the target without opening a browser OAuth redirect.
...
XAA testing requires an MCP server that exposes an HTTP URL and supports OAuth. If you select a server that doesn't meet these requirements (for example, a STDIO server), the tab shows a **Not XAA-compatible** screen that names the selected server and explains what it needs. From that screen you can:

- **Configure Server to Test** - Open the server configuration dialog to update the connection settings.
- **Back to start** - Clear the selection and return to the welcome screen to choose a different server.
