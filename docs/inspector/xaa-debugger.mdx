...
The debugger performs strict metadata discovery to prevent misconfigured or compromised well-known endpoints from redirecting tokens to the wrong authorization server.

### Protected-resource metadata (RFC 9728)

The `resource` field in the protected-resource metadata document must match the MCP server URL you provided **exactly** — trailing slashes and query parameters are part of the identity. If the field is absent or differs, the debugger rejects that candidate and tries the next well-known URL.

If no candidate returns matching metadata, the flow stops with a discovery error. You can bypass discovery by supplying the authorization server issuer directly in the target configuration.

### Authorization-server metadata (RFC 8414)

The `issuer` field in the authorization-server metadata document must match the candidate issuer **exactly** — a trailing slash difference is treated as a mismatch. If the issuer doesn't match, the debugger moves on to the next advertised authorization server.

When a protected-resource document advertises multiple `authorization_servers` entries, the debugger tries each one in order until it finds one that returns matching metadata with a token endpoint.
...
After a successful token exchange, the debugger sends an MCP `initialize` request to your server. The server's response must negotiate protocol version `2025-11-25`. If the server returns a different version (or omits it), the flow is marked as failed and the mismatch is reported in the results.

## Diagnostic Redaction

Sensitive values — access tokens, refresh tokens, identity assertions, ID-JAGs, client secrets, and `Authorization` headers — are automatically redacted from the network history and log entries shown in the debugger UI. The live flow state retains the actual values so the exchange can complete; only the diagnostic display is redacted.
...
**Protected-resource metadata mismatch**
- The well-known document's `resource` field doesn't match your MCP server URL. Ensure your server publishes a `resource` value that exactly matches the URL you're testing, including any path, query string, and trailing slash.

**Authorization-server issuer mismatch**
- The AS metadata `issuer` doesn't match the candidate being probed. Check that the issuer URL in your protected-resource metadata's `authorization_servers` list exactly matches the `issuer` the AS publishes in its own metadata.
...
- The MCP server's `initialize` response negotiated a protocol version other than `2025-11-25`. Update your server to negotiate `2025-11-25` or check that it is running a compatible MCP SDK version.
