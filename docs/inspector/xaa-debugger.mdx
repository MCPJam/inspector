...
In the **Configure Server to Test** dialog, the **Authorization Server Issuer** field is optional. If you leave it blank, the debugger reads the issuer from the MCP server's OAuth discovery metadata (its `.well-known` endpoints) automatically. Enter a value only when you need to override the issuer advertised by the server.
