...
2. **Configure Target** - Provide your MCP server URL. Leave the issuer field blank to let MCPJam auto-discover the authorization server.
...
## Authorization Server Auto-Discovery

When you leave the issuer field blank, MCPJam resolves the authorization server automatically in this order:

1. **Configured issuer** — if you set an explicit issuer in Configure Server, it is used as-is.
2. **Protected-resource metadata** — MCPJam fetches the MCP server's `/.well-known/oauth-protected-resource` document (RFC 9728) and reads the `authorization_servers` field to find the issuer.
3. **Resource URL fallback** — if no protected-resource metadata is found, MCPJam treats the MCP server URL itself as the issuer (legacy behavior for servers that self-host authorization-server metadata at the resource origin).

Once the issuer is resolved, MCPJam sweeps the issuer's well-known endpoints to locate the token endpoint.
