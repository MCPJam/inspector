...
4. **Review Compatibility** - Check the green/amber/red banner for authorization server capability warnings
...
The debugger performs RFC 9728 protected-resource metadata discovery and RFC 8414 authorization-server metadata discovery only when the information isn't already available:

- **Authorization server issuer already configured** (via a registration or entered manually) — resource metadata discovery is skipped entirely. The flow advances directly to authentication with no probe request.
- **Token endpoint already known or resolved via registration** — AS metadata discovery is skipped.
- **Bare MCP URL with nothing configured** — both discovery steps run. Resource metadata discovery tries the path-insertion form (`/.well-known/oauth-protected-resource/<path>`) first, then falls back to the root form (`/.well-known/oauth-protected-resource`) if the first returns a 404.

You can see which steps were skipped or executed in the flow log and sequence diagram.

## Authorization Server Compatibility

After authorization server discovery (or when the issuer is already known), the debugger displays a compatibility banner:
