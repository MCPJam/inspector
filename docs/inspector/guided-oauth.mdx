...
### Security: outbound metadata fetch guard

Inspector validates every outbound OAuth metadata fetch — Protected Resource Metadata (PRM) pointers, authorization-server discovery endpoints, and Client ID Metadata Documents — before the request is sent. Destinations that resolve to private or reserved IP ranges (cloud metadata services such as `169.254.169.254`, RFC 1918 LAN ranges, link-local addresses, and similar) are blocked regardless of how the server advertises them.

**Local development servers** (`localhost`, `127.0.0.1`, `::1`) are handled automatically: when the server URL you configure is itself a loopback address, Inspector permits loopback metadata fetches for that server. When the server URL is a public or remote host, loopback destinations are blocked even if the server's metadata points at one.

If you see an error such as `Refusing outbound OAuth fetch to private/reserved host`, verify that the server URL you entered is correct and that the server's OAuth metadata endpoints are reachable from a public host.
...
**In the server detail panel**, the **Last OAuth Trace** section is collapsed by default. Click the chevron next to **Last OAuth Trace** to expand it. Inside, each step and the **HTTP History** log are also collapsed by default — error steps open automatically so failures are immediately visible. The section shows:
...
- **Step-by-step breakdown** - Each OAuth step collapsed by default (error steps open automatically), with its status conveyed by title color, a summary message, and optional HTTP request/response details
...
When a server has [multiple accounts](#multiple-oauth-accounts) connected, the **Reconnect** option in each account row's menu (in the server detail modal's Config tab) replaces only that row's credential. The other accounts are not affected. Using the top-level **Reconnect** button on the card replaces the default account's credential.
