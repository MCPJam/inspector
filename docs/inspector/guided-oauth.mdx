...
### Authorization-server metadata discovery errors

When every candidate authorization-server metadata URL returns a 4xx, the debugger lists each URL it tried and the HTTP status each one returned. This replaces the previous "Last error: null" message and makes it clear which discovery paths were attempted and why each failed.
