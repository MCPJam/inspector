...
<Steps>
<Step title="Navigate to the OAuth Debugger">
Click the OAuth Flow tab in MCPJam Inspector.
</Step>

<Step title="Configure a Target Server">
Click **Configure** to open the OAuth Profile Modal prefilled with the currently selected server, or click **Add** to open a blank form and save a new target without changing the current one.
...
Provide your server's base URL and select your protocol version.
</Step>

<Step title="Choose Registration Method">
Select CIMD, DCR, or pre-registered based on your implementation.

<Check>
Pre-registered needs a **Client ID** before the configuration can be saved — see [Pre-registered clients](#pre-registered-clients).
</Check>
</Step>
...
<Check>
The sequence diagram tracks your progress through the handshake. A failed step is named in the flow log — read it in [Inspecting a flow](#inspecting-a-flow).
</Check>
</Step>
</Steps>
...
Every real connection attempt is traced, so most diagnosis happens without opening the debugger at all.
...
The log row names the server, and the reconnect rules decide whether a new consent screen opens at all.
...
### Protocol and metadata edge cases

Cases where a server's metadata or callback does not match what the spec requires, and what Inspector does about it.

<AccordionGroup>
<Accordion title="Resource indicator warnings">
...
<Accordion title="Path-scoped authorization servers">
...
<Accordion title="Authorization-response `iss` mismatch warnings">
...
How each auth type is configured and stored, and what the **Auth** tab shows for it.
