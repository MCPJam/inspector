...
- **Reveal** — Fetches and temporarily displays the stored secret so you can copy it. The value is hidden again when you close the panel or navigate away.
...
<Note>
Client secrets are never written to your browser's local storage. They are stored securely and fetched on demand when needed for token exchanges.
</Note>
