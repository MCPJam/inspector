...
When a server has [multiple accounts](#multiple-oauth-accounts) connected, the **Reconnect** option in each account row's menu replaces only that row's credential. The other accounts are not affected. Using the top-level **Reconnect** button on the card replaces the default account's credential.
