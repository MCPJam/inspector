...
## Recovering from a rejected authorization code

When the authorization server rejects the token exchange with an `invalid_grant` error (for example, because the code expired or was already used), the debugger automatically resets to re-run the authorization request:

- The **Continue** button changes to **Authorize**.
- Clicking **Authorize** generates a fresh PKCE verifier, state, and authorization URL, then reopens the sign-in popup.

This is distinct from a network failure during the token request: if the request never reached the server, the code is still valid and **Continue** retries the exchange as normal.
