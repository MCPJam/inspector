...
Reach for a plugin when a set of skills and MCP servers should travel together as one versioned unit: you hand your project a single bundle instead of registering each server by hand, and every run records exactly which bundle it used.
...
```mermaid
flowchart LR
    Bundle["Bundle<br/>ZIP or folder"] --> Inspect["Inspect<br/>no code runs"]
    Inspect --> Preview["Import preview<br/>identity, census, warnings"]
    Preview --> Revision["Stored revision<br/>content-addressed"]
    Revision --> Skills["Skills"]
    Revision --> Servers["MCP servers"]
```

## The smallest valid plugin

A bundle needs one file: a `plugin.json` at the root with `$schema` and `name`.
...
That imports cleanly and gives you a plugin with no components. Add an `mcp.json` at the root, and the same bundle now projects a server:
...
Open the **Connect** tab (Servers), click the **⋯** actions menu in the top-right of the server list, and click **Add plugin**.
</Step>
...
Review any preflight warnings shown before the upload starts, then click **Continue** to upload and inspect the bundle.
</Step>

<Step title="Read the import preview">
The preview is your last look before anything is stored. Check the **Census** for the component counts you expect, and read **Warnings** — every skipped component is listed there with its reason.

<Check>
Identity shows your plugin name and a short revision ID, and the census counts match what your bundle declares.
</Check>
</Step>
...
<Check>
A **Plugins** section now appears above the server grid on the Connect tab, with your plugin as a group card. Expand the card to see the active revision's servers and skills.
</Check>
</Step>
</Steps>
...
The dialog stays open across close and reopen so an in-progress import is not lost.

| Failure | What to click |
|---|---|
| **Commit failed** (the bundle was already inspected and previewed) | **Retry install** to recommit the same import without re-uploading |
| **Inspection failed** (uploaded but not yet inspected) | **Resume inspection** to re-drive the inspect step |
| **Terminal failure** | **Start a new import** to upload the bundle again |
...
`version` is a free-form string — the format imposes no semver rule.
...
After installing a plugin, each MCP server component that requires credentials shows its readiness in the plugin card. You configure the actual values per server — MCPJam never reads or stores credential values from the bundle itself.
...
Once at least one plugin is installed, a **Plugins** section appears above the server grid on the Connect tab. Each plugin is shown as a group card with a rolled-up health badge:
