...
<Note>
Plugins are available on accounts where the feature has been enabled. The **Add plugin** entry and the Plugins section only appear when the feature is active for your account. Guest sessions can import plugins too — the same upload path and limits apply to every actor.
</Note>
...
A `plugin.json` or `mcp.json` anywhere else in the bundle is treated as an ordinary file, and a `SKILL.md` deeper than one directory under `skills/` is not a skill. Directories the format does not define (`hooks/`, vendored folders, and so on) are preserved as plain files — they are not components and not errors.
...
| Unsupported components | Anything preserved in the stored bundle that MCPJam does not run |
