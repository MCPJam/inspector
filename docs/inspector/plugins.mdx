...
A **skip** means the component was declared but did not conform, so it is absent from the imported revision. The import preview shows a **Skipped components** warning block — always visible before the commit buttons — listing each skipped component's type, key, and reason. A skipped server is a bundle problem to fix and re-import, not a runtime failure. This block is distinct from the Unsupported components section: unsupported components are preserved by design; skipped ones failed validation and are not installed at all.
...
|---------|---------------|
| Identity | Plugin name, version, schema version (when present), and a short content-addressed revision ID |
| Census | Counts of skills, MCP servers, app mappings, and assets |
| Skipped components | A prominent warning block listing every component that **failed validation** and will not be installed — shown before the commit buttons so a partial install is never a surprise. Absent when all components passed. |
| Warnings | Non-fatal issues such as `MCP_CONFIG_IGNORED` or secret-field omissions |
...
| Setup required | Environment variable and header names the bundle declares — you supply values per server after installing, not here. Names the bundle pre-configures itself (non-secret literals or runtime-resolved templates) are noted as pre-configured. |
| Skills | Declared skill model references and descriptions |
| MCP servers | Server keys, transport (including the specific HTTP variant — `streamable-http` or `sse` — when declared), and any OAuth or credential requirements |
...
| Needs configuration | At least one component requires environment variable or header values before it can run |
...
- **Schema version** — the Agent Plugins schema version the bundle targets (for example, `Agent Plugins 1.0.0`), shown when present.
- **MCP servers** — each server's key, transport (the specific HTTP variant — `streamable-http` or `sse` — when declared), placement, and readiness. A per-component readiness badge appears next to each server; hover it to see the raw state code when the badge label is generic.
- **Configure** — servers whose bundle declared environment variable or header requirements show a **Configure** button. Click it to open an inline editor where you can enter the required values. Bundle-declared non-secret literals (for example `MODE=production`) appear read-only with an **Override** affordance; runtime-resolved template values (for example `${PLUGIN_ROOT}/data`) appear as **Pre-configured** with no input — the runtime fills them at launch. See [Credential setup](#credential-setup) for details.
...
After installing a plugin, each MCP server component that requires credentials shows a **Needs configuration** readiness badge in the plugin card. Click **Configure** next to that server to open the inline setup editor.

The editor lists every declared environment variable and header name for that server:

- **Required values** — enter the value in the masked input field.
- **Optional values** — marked `optional`; you can leave them blank.
- **Bundle-declared literals** — non-secret values the bundle ships (for example `MODE=production`) appear read-only. Click **Override** to replace the bundle value with your own; overriding pre-fills the declared value so you can edit from it. An override that is unchanged from the bundle value has nothing to save.
- **Pre-configured values** — entries backed by a runtime-resolved template (for example `${PLUGIN_ROOT}/data`) show **Pre-configured** with no input and no override. The runtime fills these at launch; there is nothing to enter.

Click **Save values** to apply. The card's readiness badge updates once the backend confirms the new state.

<Note>
The setup editor sends the complete set of values for each group you edit in one save. Values you entered in an earlier session are not pre-filled — re-enter them in the same save to keep them. Leaving a group's editor without saving leaves that group's existing values unchanged.
</Note>
