...
The trash icon next to a server group is greyed out when that group is still referenced by a test suite. Hovering the icon shows "In use by a test suite." Detach the group from every suite that uses it before deleting.
