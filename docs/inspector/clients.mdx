...
- **Inject window.openai** — Whether the OpenAI Apps SDK compatibility runtime (`window.openai`) is injected into the widget iframe. Enable this when simulating a ChatGPT host; disable it when simulating a host (such as Claude) that does not provide the bridge. Widgets that call `window.openai.*` will fail gracefully when the bridge is absent, letting you verify your app's fallback behavior.
