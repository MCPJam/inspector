...
- **Require tool approval** — When on, the model pauses before each tool call so you can approve or deny it manually. The setting **raises** the bar and never lowers it: some actions pause whether it is on or off — anything driving a browser or a third-party web page, anything running on your own machine, loading a skill an MCP server provided, and destructive UI actions. Read-only actions never pause in either mode.
