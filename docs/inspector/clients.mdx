...
- **Client view** — Shows the full selector with two controls:
  - **Add client** pill — opens the Create client dialog. Quick-add buttons for Claude, ChatGPT, and Copilot appear alongside it so you can spin up a template client without filling in the form.
  - **Switcher pill** — same as the Servers view switcher, but switching also updates the canvas URL.
...
- **Scenarios** — Same as Chat, but persisted as a saved scenario configuration.
...
Standalone clients do not appear as options in the Scenarios client picker dropdown. If you need a client for both Scenarios and Swarms journeys, create a regular client from the Connect tab instead of from Swarms → Clients.
...
The **Servers** section on the scenario Publish tab shows which project servers the scenario connects to. Click **Edit servers** to open the attachment editor and update the selection.

- If no servers are picked, the scenario displays an amber warning. Chat sessions started from that scenario will have no tools until at least one server is selected.
...
- **Delete** — Removes the Client. The inspector blocks deletion if any scenario or eval still depends on it; resolve those first, or use force delete if you're sure.
