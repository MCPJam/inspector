...
3. Enter a name (e.g. `Production servers`) and check the servers to include. Each row shows a colored status dot when the server has a known connection status — a red dot means the last connection attempt failed. Servers in a failed state are offered in the list but arrive unticked; servers that are disconnected (the default on a fresh load) are pre-selected as normal.
