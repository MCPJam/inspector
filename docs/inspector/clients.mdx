---
title: "Hosts"
description: "Reusable MCP host configurations for testing servers and apps under realistic conditions"
icon: "user-cog"
---

<Note>
Open the **Connect** tab in the left sidebar to manage Hosts (labelled **Servers** in some builds). The page name "Hosts" refers to the configuration object, not the sidebar label.
</Note>

A **Host** is a named, reusable configuration that defines how MCPJam connects to and talks to your MCP servers when you test them in the Playground, Chatboxes, and Evals. It bundles everything that would normally be scattered across one-off settings — model, system prompt, attached servers, protocol-level knobs, and MCP App sandbox controls — behind a single picker.

Real MCP hosts (Claude Desktop, ChatGPT, Cursor, your own product) each advertise slightly different capabilities and apply different sandbox policies to MCP Apps. Hosts let you reproduce those environments in MCPJam so you can verify your server behaves correctly *before* shipping to a specific host.

## Why Hosts exist

Without Hosts, every Chat or Eval would be tied to whatever ambient settings were active when you ran it. That makes results hard to reproduce and hard to share. Hosts fix this by giving you:

- **Reproducibility** — A snapshot of every setting that affects a run, so a teammate hitting the same Host gets the same conditions.
...
A Host groups settings into two layers: the **protocol layer** (how the MCP connection itself behaves) and the **app layer** (what an MCP App running inside the host is allowed to do).
...
- **Supported protocol versions** — The ordered list of MCP protocol versions the host advertises. The first entry is sent as `protocolVersion`; the rest form the accept-set. Useful for testing how your server handles older or newer clients.
...
You can override the request timeout and headers **per server** from the project-wide server list. This lets a single project connect to one server with a long timeout and an auth header while keeping host defaults for the others.

### Agent behavior

These settings control how the model interacts with tools during a chat session. They appear in the **Agent** tab of the Host focus panel.
...
When your server exposes [MCP Apps](https://modelcontextprotocol.io/docs/extensions/apps) — interactive UIs delivered via `ui://` resources — the Host controls how the inspector renders and sandboxes them. These map to the fields a real host advertises in `HostCapabilities` and `HostContext`.
...
1. Open the **Host** tab in the Connect view.
2. Click **Add host**.
...
<Note>
  Server selection is project-wide, not per-host. Use the **Auto-connect**
...
The canvas on the left shows a live diagram of the Host — the model, attached servers, and any per-server overrides. Cards flash briefly when you switch between Hosts so you can see what changed.

## Using a Host

Once saved, a Host shows up in the picker at the top of every consumer:
...
- **Evals** — Every test run inside an Eval uses the selected Host, so results are reproducible across teammates.

Switching the active Host in any of these views swaps the entire configuration in place. There's no separate "apply" step.

## Server attachments

A **server attachment** is a named, reusable set of servers drawn from the project pool. Server attachments let you decouple *which servers are active* from the Host configuration itself, so you can swap the server set without touching the model or prompt settings.
...
- The server pick is sticky — editing the host's model or system prompt does not change it.

## Managing Hosts

From the Host tab in the Connect view:

- **Duplicate** — Copy a Host when you want a small variant (e.g. `Claude Desktop — strict CSP`) without rebuilding from scratch.
- **Delete** — Removes the Host. The inspector blocks deletion if any chatbox or eval still depends on it; resolve those first, or use force delete if you're sure.

<Tip>
  Keep one Host per real host you intend to ship to. Duplicate it to make scoped
...
Hosts are scoped to the project, like servers and views. Every member of a shared project sees the same Host list and any saves sync automatically. This means a teammate can reproduce your exact test conditions by selecting the same Host from the picker — no copy-pasting settings.
...
Compare is the third tab in the Connect section of the sidebar (`Servers | Host | Compare`). Click **Compare** from any Connect view to open it. The `/host-compare` route is preserved, so any existing bookmarks or links continue to work.
...
- **Mirror your target hosts.** Create one Host per real host you care about (`Claude Desktop`, `ChatGPT`, your own product) and keep its `clientInfo`, capabilities, and sandbox settings aligned with what that host actually advertises.
- **Test the negative paths.** Build a "minimal" Host with most capabilities disabled and a strict CSP — running your server against it surfaces assumptions that won't hold on more restrictive hosts.
- **Pin Evals to a Host.** Always select a Host before running an eval suite so the results are reproducible. Evals run against ambient settings drift the moment someone toggles something elsewhere.
- **Duplicate before debugging.** When you're chasing a bug, duplicate the Host first and tweak the copy. Your baseline stays intact for comparison.
