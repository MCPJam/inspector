...
- **Require tool approval** — The single gate for every tool the model calls to act. When on, every acting tool pauses before it runs — MCP server tools, browser verbs, page tools, the shell, and mutating UI actions. When off, all of those apply immediately. Either way, read-only lookups of your own project, the progressive-discovery meta-tools, and read-only UI actions never pause. Web search follows the switch — its query is your text leaving for a third party, and the call spends credits. One exception that does not follow the switch: loading a skill that a connected MCP server provides always asks for confirmation, because it brings that server's instructions into the conversation.
