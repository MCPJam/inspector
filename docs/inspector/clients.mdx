...
- **Allow app capabilities → appTools** — Host policy that controls whether the inspector calls `tools/list` and `tools/call` on the iframe app and surfaces those tools to the LLM agent. This is separate from the wire-level `hostCapabilities` — apps advertise tool support via `appCapabilities.tools` in `ui/initialize`, but the host must explicitly opt in before those tools reach the agent. Off by default on all presets except MCPJam (which enables it so developers testing app-tools widgets don't need to flip a toggle). Toggle it on when you want to test an app that registers its own tools with the agent.
