...
**Style tokens** — A read-only collapsible block below the two matrices listing every style variable the host sends to a view on `ui/initialize`. Variables are grouped by family (background, text, border, ring, type, shape, shadow) and show their light and dark values side by side. Color tokens include a split light/dark swatch over a checkerboard so transparent tokens (such as `…-ghost` variants) are distinguishable from opaque ones. Radius, shadow, and type tokens show a small preview of the property they drive. Click any row to copy `var(--token)` — the form you paste into a widget's own stylesheet to stay themed by the host. The block header shows the total token count; expand it to browse or search the full list. If the host sends no style variables, the block says so explicitly.
...
Style variable rows in the compare matrix normalize how each host encodes its values: hosts that send a single `light-dark(a, b)` string and hosts that send a resolved value per theme both render as the same stacked LIGHT / DARK pair, so you can read down a column without the notation getting in the way. The literal value the host actually sends is preserved and shown on hover. Color rows include a split light/dark swatch over a checkerboard so two hex values are comparable at a glance and a fully transparent token is distinguishable from an opaque one.
