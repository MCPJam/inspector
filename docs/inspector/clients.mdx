...
- **Paginated list traversal** — Dropdown in the **MCP Protocol** settings card. **Walk every page** (the default) follows `nextCursor` through every page of a paginated list, as a conforming client does. **First page only** simulates a host that stops after the first page and never follows the cursor — tools past page one become invisible to the model, and because the mirroring source is the page-one list the client cached, those tools' calls also lose their `Mcp-Param-*` headers. Use this to see what your server looks like through a non-paginating host.
- **Multi-round tool results (MRTR)** — Dropdown in the **MCP Protocol** settings card. **Supported** (the default) drives `input_required` rounds so a server can collect input mid-call. **Not supported** simulates a client that has not implemented the 2026 multi-round pattern — the inspector stops advertising elicitation on a modern connection, so a server that would have elicited answers returns `-32021` instead. Use this alongside the client capabilities elicitation toggle to test how your server behaves with clients that cannot handle mid-call input requests.
