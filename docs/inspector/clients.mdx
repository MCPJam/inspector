...
- **Client capabilities** — The capabilities object sent in `initialize` (sampling, roots, elicitation, experimental, etc.). Enable or disable each one to test what your server does when a capability is missing. This blob also gates widget rendering at display time: the inspector uses the same resolved capabilities to decide whether to render widget iframes in Chat, Playground, and the Tools tab — so what you configure here affects both the `initialize` handshake and what the user sees in the thread.
...
<Note>
  **SEP-1865 strictness:** for widget iframes to render, the client capabilities
  blob must advertise the MCP UI extension with a `mimeTypes` array that includes
  `"text/html;profile=mcp-app"` — for example:
  ...
  The SDK default ships this shape, so built-in host templates (Claude, ChatGPT,
  MCPJam, Copilot, Cursor) work without any changes. Hand-crafted capability
  blobs that include the extension key but omit `mimeTypes` (e.g.
  `{ "extensions": { "io.modelcontextprotocol/ui": {} } }`) no longer render
  widgets. Add the `mimeTypes` field to restore rendering.
</Note>
