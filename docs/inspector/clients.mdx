...
- **`mcpAppsOverrides`** — Sparse per-dimension override for the SEP-1865 `app.*` spec-bridge capability matrix. Set this in the Apps tab JSON editor to suppress specific capabilities (e.g. `serverResources`, `logging`) or restrict `availableDisplayModes` independently of the host style preset. Absent from the JSON means "use the host style preset". Combine with the `copilot` host preset to simulate the M365-published capability subset exactly.

<Tip>
  Until a structured UI for `mcpAppsOverrides` is available, configure it by hand-editing the JSON in the Apps tab. Paste a block like the following, then save — it persists to `mcpProfile.apps.mcpAppsOverrides` and survives round-trips:
  ...
  Boolean fields require actual booleans; `availableDisplayModes` accepts `"inline"`, `"fullscreen"`, and `"pip"`. Unrecognized fields are silently ignored. An entirely invalid block is dropped and the resolver falls back to the host style preset.
</Tip>
