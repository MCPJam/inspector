...
## When a custom Client is useful

You reach for a Client the moment "it works in MCPJam" stops being the same thing as "it works in the host I'm shipping to." A capability your server assumes is present, a stricter sandbox, an older protocol version: each of those is a real host difference, and a Client is how you reproduce it deliberately instead of discovering it in production.
...
- Create a Client from a host template.
- Point it at your servers and open it in the Playground.
- Confirm the connection is running under the settings you expect.
- Tune the optional protocol, agent, and MCP App settings afterwards.

## Create your first Client

<Steps>
<Step title="Create the Client">
The smallest working Client is a template. While the Connect canvas is open, the **Add client** pill sits at the top-left of the canvas with quick-add buttons for **Claude**, **ChatGPT**, and **Copilot**. Click one to spin up a template client without filling in the form.

To build one by hand instead:

1. Open the **Connect** tab in the inspector.
2. Click **New Client**.
...
<Check>
The new Client appears in the switcher pill at the top-left of the canvas, and the canvas shows its diagram: the model, attached servers, and any per-server overrides.
</Check>
</Step>

<Step title="Attach your servers">
<Note>
...
<Step title="Use it">
Select the Client from the picker at the top of the Playground (or Scenarios, Evals, or Swarms). Switching the active Client swaps the entire configuration in place, so there's no separate "apply" step.
</Step>

<Step title="Confirm it's working">
Send a prompt in the Playground and check that your server's tools are being called under the Client you selected.

<Check>
The Client picker shows the Client you created, its attached servers show **Connected**, and the tool calls in the thread come from those servers. If a widget doesn't render, check the MCP App settings below, because widget rendering is gated on the effective `clientCapabilities`.
</Check>
</Step>
</Steps>
...
Some clients are **standalone**, meaning they are owned by the Swarms surface and have no Scenario or publish surface. If you select a standalone client in the Scenarios tab, the tab shows a **"Managed by Swarms"** notice instead of a scenario. The client picker stays visible so you can switch to a publishable client.
...
<CardGroup cols={2}>
  <Card title="Playground" icon="joystick" href="/inspector/playground">
    Run your Client against a server and watch every tool call.
  </Card>
  <Card title="Evaluate" icon="check-check" href="/inspector/evals">
    Pin an eval suite to a Client so results stay reproducible.
  </Card>
  <Card title="Host Compatibility" icon="scale" href="/inspector/host-compat">
    See how your server scores against each host's expectations.
  </Card>
  <Card title="OAuth Debugger" icon="shield-check" href="/inspector/guided-oauth">
    Work through an auth flow before wiring it into a Client.
  </Card>
</CardGroup>
...
Who the client claims to be during the MCP handshake.

- **`clientInfo`**: The `name`, `version`, and any additional fields identifying the client. On the legacy revisions these are sent in `initialize.params.clientInfo`; on `2026-07-28` there is no `initialize` handshake, so they ride every request's `_meta` under `io.modelcontextprotocol/clientInfo`. Set this to match a real host's identifier when you want your server to take a host-specific code path.

### Protocol behavior

These map directly to fields the inspector sends when it identifies itself. On the legacy revisions that is the `initialize` handshake plus every subsequent request; on `2026-07-28` the handshake is gone and the same values travel in each request's `_meta` (`io.modelcontextprotocol/protocolVersion`, `.../clientCapabilities`, `.../clientInfo`).
...
- **Client capabilities**: The capabilities object the client advertises (sampling, roots, elicitation, experimental, etc.) — in `initialize` on the legacy revisions, in per-request `_meta` on `2026-07-28`. Enable or disable each one to test what your server does when a capability is missing.
...
- **Elicitation**: Toggle in the **MCP Protocol** settings card that controls the top-level `clientCapabilities.elicitation` key. When off, the key is absent. When on, the inspector advertises `{ form: {} }`, signalling that the client can handle elicitation requests from the server. An optional **URL mode** sub-checkbox (visible only when Elicitation is on) adds `{ url: {} }` to the elicitation object, indicating the client can also handle requests to open a URL. Enabling this before server-side elicitation support is active is safe, because it is a no-op rather than a break.
- **Mirror tool params into `Mcp-Param-*` headers (SEP-2243)**: Dropdown in the **MCP Protocol** settings card. **Mirror** (the default) copies a tool argument annotated with `x-mcp-header` into an `Mcp-Param-{Name}` request header on `tools/call`, as MCP `2026-07-28` requires. **Omit** deliberately simulates a client that has *not* implemented the mirroring (real-world support is uneven, and browser clients never send these headers), so you can see what your server does when they don't arrive. A conforming server should answer `-32020 HeaderMismatch`; MCPJam surfaces that failure as-is rather than silently retrying. The setting applies to the whole client, not per server, because conformance is a property of the client.
...
When your server exposes [MCP Apps](https://modelcontextprotocol.io/docs/extensions/apps), which are interactive UIs delivered via `ui://` resources, the Client controls how the inspector renders and sandboxes them. These map to the fields a real host advertises in `HostCapabilities` and `HostContext`.
...
- The server pick is sticky, so editing the client's model or system prompt does not change it.
...
Clients are scoped to the project, like servers and views. Every member of a shared project sees the same Client list and any saves sync automatically. This means a teammate can reproduce your exact test conditions by selecting the same Client from the picker, with no copy-pasting settings.
