...
<Note>
  Server selection is project-wide, not per-client. The **Auto-connect**
  switch in the Servers tab header connects every project server for you in
  this browser; it is a personal setting, not a project one. Per-server
  overrides (headers, timeout) are configured on the Servers tab too.
</Note>
