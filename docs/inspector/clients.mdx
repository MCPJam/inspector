...
The **Tools** tab of the Client focus panel lists the first-party built-in tools the host makes available to the model — for example, **Web Search**. The tab appears only when the deployment exposes at least one built-in tool.

Each tool is shown as a toggle. Tools that require a personal computer (such as Bash) are shown in a blocked state until a computer is attached; the blocked row points you to the **Computer** tab to attach one first.

### Personal computer

The **Computer** tab lets you attach or detach a per-member cloud workstation (a persistent Linux sandbox) to the host. Attaching a computer is required before you can enable computer-backed tools like Bash in the Tools tab. Detaching automatically removes any computer-backed tool IDs from the host's tool selection.

The Computer tab is shown when the `computers-enabled` feature is on, or when a computer is already attached (so an existing attachment always stays detachable even if the feature is later turned off).

<Note>
  Clicking the **Built-in tools** or **Computer** island on the host canvas opens the corresponding tab directly, so you can jump straight to the right editor from the visual diagram.
</Note>
