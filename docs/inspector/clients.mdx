...
While the Connect canvas is open, a **client selector** is pinned to the top-left of the canvas. It shows two controls:
...
- **Switcher pill** — shows the active client's logo, name, and its position in the list (e.g. `2 / 3`). Click to open a dropdown listing all clients; select one to switch the canvas to it. A delete button appears on hover for each client (disabled on the last remaining client).

The header client bar hides itself while the canvas is open to avoid duplicating the control.
