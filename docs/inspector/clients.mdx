...
- **MCP App support** — Master switch that controls whether the inspector advertises the MCP UI extension (`io.modelcontextprotocol/ui`) in `clientCapabilities`. Turn it off to simulate a host that doesn't render MCP Apps at all (e.g. a CLI client). Turning the switch off does **not** clear your per-dimension overrides — they remain dormant and are restored when you turn advertising back on. Use the **Reset** button (shown when overrides are active) to explicitly revert all per-dimension overrides to the host style preset.
