...
- **Chatboxes / Swarms** — Each chatbox is bound 1:1 to a Client. On the Chatbox and Swarms pages, a **Client picker pill** in the publish bar (and in the header on other sub-tabs) lets you switch the active Client directly on the page — the global host bar is hidden on these routes. The gear icon on the pill opens Connect so you can edit that Client's identity (model, system prompt, sandbox settings). Sessions and Clusters follow whichever Client is currently selected.
