...
- **Browser** (local inspector) — Enable or disable the local Browser tool for this client. This setting is independent of the hosted Browser configuration. When the feature is not yet rolled out to your account, the toggle is disabled unless the client already has Browser enabled (so existing configurations remain inspectable).
- **Browser** (hosted) — Enable or disable the hosted browser built-in for this client. Hosted Browser settings are independent of the local Browser setting.
...
- **Enable for all clients** — After granting device permission, this button opens a setup flow that enables the local Browser tool for all clients in projects you manage, including shared clients. New clients inherit the setting; clients you have explicitly disabled stay disabled. Teammates must allow browser access on their own machines separately.
