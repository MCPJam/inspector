...
- **User Testing** — Same as Chat, but persisted as a saved scenario configuration.
...
Some clients are **standalone** — they are owned by the Swarms surface and have no User Testing scenario or publish surface. If you select a standalone client in the User Testing tab, the tab shows a **"Managed by Swarms"** notice instead of a scenario. The client picker stays visible so you can switch to a publishable client.

Standalone clients do not appear as options in the User Testing client picker dropdown. If you need a client for both User Testing scenarios and Swarms journeys, create a regular client from the Connect tab instead of from Swarms → Clients.
...
In the Eval Playground, the **Clients** row next to **+ Attach client** includes a **Compare** button. When two or more clients are attached to the suite, clicking **Compare** opens the Compare view with exactly those clients pre-selected. The button is disabled (with a tooltip) when fewer than two clients are attached.
