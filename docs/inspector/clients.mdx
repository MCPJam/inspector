...
The Apps Extension tab contains **two independent capability matrices**, one for each host bridge. They never cross-gate — toggling a row in one matrix has no effect on the other.

#### `window.openai` matrix (ChatGPT compat shim)

Controls the `window.openai.*` surface advertised to apps that use the ChatGPT compatibility shim.

#### `app.*` matrix (SEP-1865 spec bridge)

Controls the `app.*` surface advertised to apps that use the primary MCP Apps protocol. The matrix is always active (there is no master inject toggle). Dimensions are split into two groups:

- **Main dimensions** (always visible) — `toolInputPartial`, `toolCancelled`, `hostContextChanged`, `resourceTeardown`, `serverResources`, `logging`. These are the rows that vary most across published host tables (e.g. Microsoft 365 Copilot vs. Claude).
- **Advanced dimensions** (collapsed by default) — `toolInfo`, `openLinks`, `serverTools`, `updateModelContext`, `message`, `sandbox.permissions`, `sandbox.csp.frameDomains`, `sandbox.csp.baseUriDomains`, `_meta.ui.prefersBorder`. Rarely differ across hosts but surfaced so legacy `hostCapabilitiesOverride` migrations are visible.
- **`availableDisplayModes` cluster** — Multi-checkbox at the top of the matrix for the `inline`, `fullscreen`, and `pip` allowlist. The matrix enforces a non-empty invariant: if you uncheck the last mode, `inline` is force-enabled.
- **Overridden badge** — Appears on any row where your override diverges from the host style preset, so you can see at a glance what you've changed.
- **Match host preset chip** — Clears the entire `app.*` matrix in one click and reverts every dimension to the selected host style's defaults. Disabled when no overrides are set.

Each row shows a **Preset: \<value\>** hint so you always know what the selected host style would advertise without an override.
