...
- The **Tracing** tab shows a per-header verdict for each mirrored `Mcp-*` header — `✓ matches body`, `✕ body says … → -32020`, `not required here`, and so on — so you can see at a glance whether the headers agree with the body before a `-32020 HeaderMismatch` surfaces. See [MCP header tracing](/inspector/debugging#mcp-header-tracing-2026-07-28) for the full verdict reference.
