...
The inspector can speak more than one version of the MCP protocol. That matters because a server that works today can break for a client on a different revision: the 2026-07-28 RC drops the `initialize` handshake entirely, discovers capabilities through `server/discover`, and carries protocol metadata on every request. Pinning the inspector to an exact version is how you find out what your server does under each revision before a real client does.

By default (**Automatic**), unpinned connections probe `server/discover` for 2026 support and fall back to the legacy `initialize` negotiation — on both HTTP and stdio. You can also pin a specific version when you want to test an exact revision — including the **2026-07-28 stateless RC** or any earlier stable release.
...
<Accordion title="Not yet supported in the RC client">
