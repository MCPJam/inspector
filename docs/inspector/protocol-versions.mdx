...
The inspector can speak more than one version of the MCP protocol. By default (**Automatic**), unpinned HTTP connections probe `server/discover` for 2026 support and fall back to the legacy `initialize` negotiation. You can also pin a specific version when you want to test an exact revision — including the **2026-07-28 stateless RC** or any earlier stable release.
...
| ------------------------- | ------------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Automatic**             | none               | HTTP: `server/discover`, then legacy fallback   | Detect 2026 support from the server; otherwise use the version negotiated by `initialize`. This is the default.                             |
...
- Resumption tokens
- Automatic HTTP detection falls back to the legacy `initialize` handshake. An explicit 2026 pin remains exact and does not fall back.
