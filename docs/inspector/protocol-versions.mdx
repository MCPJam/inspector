...
The version dropdown offers the union of the client's `initialize` accept-list and the eras the host's catalog row advertises. This means `2026-07-28` is selectable on clients like Claude and ChatGPT that speak it, even when their `initialize` list only carries legacy revisions. A catalog row with no accept-list is a legacy row and stays fully editable — the catalog only widens an existing list, never creates one.
...
- A **`ConflictingProtocolVersionPin`** warning appears when a stateful pin (any `2025-*` revision) is not present in the client's `initialize` `supportedProtocolVersions` list. The warning does not fire for stateless pins (`2026-07-28`) — a stateless pin skips the `initialize` handshake entirely and is exempt from the accept-list check.
