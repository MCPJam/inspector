...
- **`logging/setLevel` is deprecated.** The `2026-07-28` revision deprecates the `logging/setLevel` RPC (SEP-2577). The inspector's Logs panel reflects this: modern servers show an opt-in toggle instead of a level dropdown, and the server info panel displays a deprecation note next to the negotiated protocol version. Log levels are now sent per-request via `_meta` rather than as a one-time RPC.
