...
If you run authenticated HTTP MCP evals through the SDK, MCPJam can also securely persist replay credentials for reruns and debugging. The recommended setup is to attach the connected `MCPClientManager` to your `TestAgent`, or pass `agent` / `mcpClientManager` to the manual reporting APIs.
