...
Once a server is connected, a **compat strip** appears on its card: a row of host logos (Claude, Claude Desktop, ChatGPT, Cursor, Copilot, Codex), each with a colored status dot, and a one-line summary such as "works in 4 · degraded in 1."
...
1. Create a new host pre-configured from that host's template (Claude, Claude Desktop, ChatGPT, Cursor, Copilot, or Codex).
