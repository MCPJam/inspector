...
Once a server is connected, a **support pill** appears at the bottom of its card. It shows a supporter count ("Supported by N") alongside up to three client logos — ChatGPT and Claude lead the stack when they support the server, since they're what most people are checking for first.

- **Supported** — the pill shows the count and up to three client logos.
- **No supporters** — once analysis has settled, the pill reads "No clients support this server" in a red-tinted style. Mid-analysis ("Checking compatibility…") and failed checks ("Compatibility checks unavailable") are shown instead when the result isn't known yet.

A client counts as a supporter when its compat status is green — meaning the host profile covers all of the server's relevant requirements. A `works` verdict that carries a blocking or protocol-mismatch finding is not counted.

Click the pill to open the **Clients** tab in the server detail modal for the full per-host breakdown.
...
- The tool list hasn't loaded yet (the pill shows "Checking compatibility…" briefly after connect).
...
| Probe | Captured from a live client session (e.g. Cursor 3.14.27) | High, but versioned |
...
After updating your server, reconnect and the support pill will re-evaluate.

### Test in an emulated host

Click **Test** next to any host in the Clients tab to:
