...
Any HTTP server has an **Auth** tab in the server detail modal. The tab shows connected OAuth accounts and stored tokens. It remains accessible even after OAuth is switched off, so you can still reach stored or invalid auth data.

### Multiple OAuth accounts

Personal OAuth servers support connecting more than one account. When a server uses OAuth, the **Auth** tab shows a **Connected accounts** section that lists every connected credential.

Each row shows the account's email address (or name) on the first line, and a caption on the second line — the name you gave it, plus **Default** when it is the default account, or **Needs reconnect** when the tokens are expired or revoked.

When two accounts share the same email address, the caption adds a qualifier — workspace name, nickname, or a short account ID — so the rows are always distinguishable.
...
- **Set as default** — promotes the row to the default. The default account is used by everything that is not account-aware in chat: evals, the Tools tab, resource reads, and the CLI. In chat, the model picks an account per call; the default is the fallback when no account is specified. This option is hidden on the row that is already the default.
...
When two or more accounts are live, chat exposes an account selector so you (or the model) can choose which credential each tool call uses. Changing the default mid-conversation does not redirect tool calls that are already in flight.
