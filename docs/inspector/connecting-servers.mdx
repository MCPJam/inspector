...
Personal OAuth servers support connecting more than one account. When a server uses OAuth, the server card and detail modal show an **accounts** section that lists every connected credential.

From the accounts section you can:

- **Connect another account** — starts a new OAuth flow and adds the credential as a second (or third, etc.) row without touching the existing connection. Up to 8 accounts per server.
- **Reconnect** — re-runs the OAuth flow for an existing row, replacing only that credential. A row that shows **Needs reconnect** has expired or revoked tokens.
- **Relabel** — type a name into the label field and click away to save it. Labels appear in chat when the model picks an account.
- **Use in chat / Set default** — promotes a row to the default. The default account is used by non-chat consumers and as the fallback when no account is specified.
