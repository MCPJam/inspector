...
The agent panel uses the same connected MCP servers that are active in your Playground and Chat sessions. When you have servers connected to your project, the agent can call their tools directly from the Home tab — no extra configuration needed.

The agent uses whichever servers you have selected in the multi-server selector. If no servers are explicitly selected, it falls back to all currently connected servers. The panel shows a **Loading MCP servers…** message while it resolves your server connections on first load.
