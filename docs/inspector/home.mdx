...
Actions that mutate the UI (such as running a tool call) pause for your explicit approval before they run when the **Tool approval** toggle is on. When the toggle is off, those actions apply immediately.
...
- **Toggle off (default)** — Tool calls apply immediately, including browser actions, page tools, and shell commands. Read-only lookups and an open app's own actions never pause regardless of this setting.
- **Toggle on (strict)** — Every tool call that acts pauses before it runs: MCP server tools, browser verbs, page tools, the shell, and mutating UI actions.
