...
| **Registry catalog** | Installs and connects servers from the public registry catalog, disconnects them, and stars or unstars them — available while the Registry screen is open |
...
Registry catalog actions are available only while the Registry screen is open. If you ask the agent to install a registry server from another screen, it will navigate to the Registry first. OAuth-protected registry servers follow the same rule as project servers — the agent reports `authorization_required` and leaves the Authorize click to you.
