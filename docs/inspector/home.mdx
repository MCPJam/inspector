...
The composer footer includes a **Tool approval** toggle (off by default). When enabled, any mutating `ui_*` action — such as navigating to a tab or executing a tool — pauses and shows an Approve / Deny pill before it runs. Read-only actions like taking a snapshot are never gated. The preference is saved to your browser's local storage and persists across reloads.
