...
The case editor is a two-pane workspace. The left column is where you write the case; the right column shows the selected iteration — a live run in progress, a persisted iteration from history, a recording session, or a spec preview.
...
- **Evaluators** — everything that grades this case, grouped by the link of the user-value chain it measures, in the same vocabulary the suite's Grading tab uses.
...
A row's **role** says what a miss does: **Gate** fails the iteration, **Warn** highlights the miss without failing it, **Report** only records it. The control appears when your deployment supports assertion policy; otherwise the role still shows, read-only.

Because a failing gate halts the run, an assertion written as a step can leave later assertions unrun. The iteration column marks those **Skipped**.
...
- **Any route** — the case names no route and is graded by its evaluators. This is a normal positive case, and it is what most CLI- and SDK-authored cases are.
...
**Steps** (top right of the column) swaps the form for the flat, ordered step list, in the same workspace with the same iteration on the right. **Back to form** returns.

**Right column — the selected iteration**

The iteration opens on its **Scorecard**: the same evaluators the left column lists, in the same order, each with what happened to it on this iteration. A row shows its result, its provenance, its role, and — where there is one — the reason, which for a step is the message the runner recorded.

The summary line counts **gates only**: "2 of 2 gates passed · 1 warn". A Warn miss is shown on its row and never counted as a failure, because it did not fail the iteration. An evaluator with no result reads **Not measured** rather than passing by default. The verdict word itself stays in the header above.

The judge's row hosts the judge panel, so labelling an iteration works where the judge sits. Under the rows, **Score rows** expands the raw score contract, including any row the backend could not join.
...
1. **Explicit history pick** — an iteration you selected from History.
...
6. **Empty** — no iteration yet; start typing a prompt.

<Note>
  A **negative case** is one you marked **No tool should be called**. Meta questions ("what params does `search` take?"), conversational drift, and ambiguous prompts are the usual shape. A case that simply names no tool is *not* negative — it is graded by its evaluators.
</Note>

### History inspect

Click **History** in the iteration header to open a list of past iterations. Selecting one freezes the left form on that iteration's snapshot — the exact steps, assertions, and settings that were in effect when it ran — so you can compare what you see in the trace against what was authored at the time.

While an iteration is frozen, an **Inspecting** banner appears at the top of the left column. If the current draft differs from the frozen snapshot, a **Changed** indicator appears. Click **Edit case** to return to the live draft; the selected iteration stays visible on the right.

### Next run sheet

Click the **Model**, **Iterations**, or **Host** chip above the form to open the **Next run** sheet. It separates two kinds of settings:

- **Saved with this case** — *Iterations* and *Model* are written to the case before every quick run and on Save. Changes here persist.
...
<Note>
**A per-iteration token ceiling is not a spend control.** It is set without seeing
...
- **Minimum iterations** — Set a floor on how many iterations each case runs. Every case runs at least this many times, regardless of its own iteration count. Use `mcpjam cloud eval update --suite <name> --min-iterations <1-10>` to set this from the CLI; `off` removes the floor.
...
priced when the iteration finished and stamped with the rates used — so a run's
cost does not move later when prices change. It rolls up onto cases, runs and
the metric strip.

A dash (`—`) means **no cost was observed**, never that the iteration was free:

| Why | What you see |
|---|---|
| Your own API key (BYOK) | MCPJam did not bill it, so there is no cost to report. |
| A harness run (Claude Code, Codex, Cursor) | A harness turn mixes models — sub-agents on cheaper models under a main one — and the iteration's totals carry no per-model split, so no honest single figure exists. Harness cost arrives with billed attribution. |
...
When only some of a run's iterations were priced, the total says so. That
...
- **Per-iteration row** with case, model, pass/fail, tokens, cost, duration, and the tool calls that actually happened. Each row also shows a one-line summary of where that iteration's user-value chain stopped — for example `broke at Tool response`, `Request satisfied`, or `chain withheld` — so you can scan the table without opening each row.
...
- **User-value chain** — when you expand an iteration, six numbered stage cards show how far value travelled: Session connected → Tools discovered → Tool selected → Tool called → Response returned → Request satisfied. The card for the first stage that failed opens automatically. Click any card to read what happened at that stage and why; click it again to close. This is shown for every iteration — passing and failing alike. An iteration that delivered end to end reads as the full delivery story; a failing iteration opens on the break. During blind review (before you label an iteration whose User value stage was decided by the judge), the rail still shows all six stages; the User value card opens automatically but its state, reason, and evidence are hidden until you submit your label. The other five stages — decided by the runner, not the judge — remain fully readable.
- **Full trace** — every turn, every tool call, every token. This is the thing you couldn't see before; spend time here. When a turn ran through a harness (e.g. Claude Code), the trace detail pane shows an **Engine** badge — "Claude Code" for harness turns, "Emulated" for turns that ran through MCPJam's built-in chat engine — so you can tell at a glance which runtime handled each turn. The user-value chain also appears above the transcript in the trace pane, so the answer to "where did this iteration stop" is on the same screen as the evidence.
...
- **Assertion Gate** — when `successPredicates` are configured on a case, an expandable Assertion Gate section in the iteration detail lists each assertion with a PASS/FAIL verdict, a one-line summary, and the evaluator's reason. Hidden when no assertions are configured.
