...
- **Outcome achieved** — the goal sentence, and what the judge will actually do with it.
...
The trial opens on its **Scorecard**: the same scorers the left column lists, in the same order, each with what happened to it on this trial. Rows are titled by what they evaluate — standard checks use catalog names (for example **Tool errors (isError)**) and the judge row is titled **Outcome achieved**. Each row shows its result, its provenance, its role, an **Expected** field (the configured rule), and — where there is one — an **Actual** field with the reason, which for a step is the message the runner recorded.
...
Click a **test case name** to open its definition — the user prompt, checks, and expected outcome. Click a **result cell** (pass/fail) to open a **case drawer** with per-pairing averages (passed count, P50, P95, tokens, calls) and a flat iteration list. Click an iteration row to open its full detail view — scorecard, transcript, tool calls, and trace — without leaving the run page. A **Back** breadcrumb returns to the iteration list.

In the scorecard, each scorer row is titled by what it evaluates — standard checks use catalog names such as **Tool errors (isError)** and **Tool hops before the right tool**; the judge row is titled **Outcome achieved**. Each row shows an **Expected** field (the configured rule) and an **Actual** field (what the run recorded).

When a stage fails because the server reported a tool error, the **Actual** field quotes the server's own error message directly from the downloaded trace — for example, `` `create_journey` returned an error: VALIDATION_ERROR: A journey must target at least one host `` — at no model cost. When an AI analysis has run for the iteration, its explanation supersedes the recorded quote for that stage.

The scorecard also shows the current state of the AI analysis:

- **No analysis yet** — "Analyze this run to add AI explanations to these rows." appears above the chain. Click **Analyze** on the run to start it.
- **Analysis in progress** — "Reading iterations N of M…" updates as the read phase works through the population.
- **Analysis failed** — a specific reason is shown, such as "This iteration's trace was too large to analyze." The recorded EXPECTED, ACTUAL, and verdict remain on every row regardless.
- **Analysis ready** — AI-generated explanations appear on each row with a cited trace evidence disclosure. If the narrative predates the latest grade it is marked stale and the recorded evidence is shown instead.

During blind review, none of these status lines are shown — a reviewer labelling a trial is not told whether an explanation is coming.
...
While generation is open, the breadcrumb reads **Evaluate / &lt;suite&gt; / Generate test cases**. Click the suite name in the breadcrumb to exit the generation view and return to the suite overview at any time.
