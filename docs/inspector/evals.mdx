...
- **Route facts** — when the `evaluate-route-facts-enabled` flag is on, each case row shows a one-line route summary (which tool sequence each trial took) and two collapsed expanders: **Routes** (per-route trial counts, no-tool and looping rates) and **Expected vs observed** (which expected tools were never called, which unexpected tools appeared, and any one-to-one substitutions). Counts are by tool name — a call with the wrong arguments still counts as called. The page prefers a persisted route-facts document from the backend when one exists and falls back to computing locally from the loaded iterations when it does not. A run that exists but has no persisted document reads as unmeasured; there is no backfill.
- **Description experiment** — when the `description-experiments-enabled` flag is on, a failing case whose missing tool is in the run's catalog shows a **Propose a description rewrite** button. Clicking it drafts a rewrite (spends a small model budget) and opens a collapsed card on the run page showing the two-arm result — original description versus the proposed rewrite — as a pass-rate difference in points with a confidence interval. The card states whether the evidence was **Controlled** (frozen model, host, catalog, and grader) or only **Reproducible** (one or more variables differed). Harness-engine runs show the button as disabled. The card is not mounted when the flag is off.
- **Failure groups** — when the `evaluate-failure-groups-enabled` flag is on, a completed run grows a collapsed **Failure groups** card that draws one ribbon per failed trial across a Case → Route → Reason flow. When clustering did not split the failures into distinct groups, the card shows the flat list instead. The card is not mounted when the flag is off.
