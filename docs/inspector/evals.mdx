...
- **User-value chain** — when the run has stage funnel data, a funnel panel appears in the insight rail showing how users progressed through the measured stages. The panel is only shown when there is data to display; runs with no derived chain produce no panel.
