...
- **Scorers** — everything that grades this case, grouped by the link of the user-value chain it measures, in the same vocabulary the suite's **Test Suite Evaluators** section uses.
...
- **Suite** — a default the suite applies to every case. Read-only here, with a link to the suite's **Test Suite Evaluators** settings.

The list reads as a checklist with progressive disclosure. An off row shows only its name. Once on, the **Required / Advisory** control appears on the row; an assertion that carries a number (such as a latency budget) shows it under the name. Assertions that have their own fields — thresholds, match criteria — show a chevron next to the name; clicking the name opens the editor in place. Assertions that are nothing but their kind (such as **No tool returns an error**) have no editor to open: the role control is inline and the checkbox removes them.
...
|-------|-------|-----------|
| Suite default | Suite settings → **Test Suite Evaluators** → Default validators | Yes |
...
<Warning>
  **These are not two units of one number.** Take ten equally sized cases, nine
  ...
  under `settings.policy`. The API refuses the field a suite does not use, with
  a message naming the one that would work.
</Warning>

Changing which criterion decides a suite re-decides every case in it, so it is
a separate, explicit operation rather than a side effect of editing a
threshold. There is no toggle beside the threshold field.
