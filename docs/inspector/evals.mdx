...
- **Actionable findings** — after a completed run, an **Actionable findings** panel appears above the existing triage card. It lists repair tasks ordered by ownership: MCP server fixes that are ready to make come first, then suspected server issues that need more investigation, then agent/prompt work, test-design changes, and finally environment or informational rows. Each finding shows a deterministic observed sentence (counts and identifiers, not model prose) that is always visible without expanding. Expanding a row reveals the likely cause, the recommended change, acceptance criteria, and evidence excerpts with links to the sessions where the failures occurred. For findings the backend has promoted to "ready", the pinned tool contract (description, input schema, and output schema when present) is shown alongside a **Copy server fix prompt** button that builds a prompt safe to paste into a coding agent. Unproven server findings show an investigation prompt instead; environment and informational rows offer no prompt. The panel only appears on completed runs that have server-quality data; pending, failed, and unavailable states are shown explicitly rather than as an empty list.
