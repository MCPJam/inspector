...
- **Full trace** — every turn, every tool call, every token. This is the thing you couldn't see before; spend time here. When a tool call fails at the MCP protocol level, the detail pane shows an **MCP error** line with the JSON-RPC code and its name (e.g. `-32602 · Invalid params`). Negative codes are MCP/JSON-RPC errors; the label distinguishes server-returned errors from client-side transport failures like `-32001 · Request timeout`.
