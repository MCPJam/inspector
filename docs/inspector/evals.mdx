...
<Warning>
  If a server in the snapshot is no longer connected when you rerun, the eval fails immediately with a message like **"Could not start eval because &quot;Server Name&quot; is not connected. Reconnect the server and try again."** Reconnect the server (or update the snapshot to remove it) before retrying.
</Warning>
