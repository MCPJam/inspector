...
A completed run carries one of three verdicts:

| Verdict | Badge | Meaning |
|---------|-------|---------|
| **Passed** | Green | The run cleared its pass-criteria threshold |
| **Failed** | Red | The run did not clear its threshold |
| **Inconclusive** | Amber | The run finished but could not be measured well enough to decide either way — too few gradeable trials, too many evaluator errors, or the validity phase withheld a verdict. This is not a failure: no defect was observed |

An inconclusive run does not quote a pass-criteria threshold in its badge, because the threshold was never applied to it.
