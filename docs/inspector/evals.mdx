...
- **Full trace** — every turn, every tool call, every token. This is the thing you couldn't see before; spend time here. Click an LLM span row to expand its detail pane, which shows **Provider**, **Finish** (the model's stop reason), **TTFC** (time to first chunk), **Throughput** (output tokens/s), and **Response ID**.
