...
When you run a suite against multiple execution contexts at once — multiple host configurations or multiple project environments — the Runs list groups them into a single collapsible **run group** row showing mean accuracy and longest duration. The group badge shows the number of distinct contexts: **N environments** for environment-backed runs, or **N clients** for host-backed runs. Expand the group to inspect each context's individual run.

For environment-backed run groups, the group header shows the environment name and a revision range when the group spans multiple edits (for example, **Staging · rev 2–4**). The exact revision each run pinned appears on the individual run row. Two environments that resolve to the same host are always kept as separate groups.
...
When a run was launched from a project environment, the run detail header shows an **Environment** row (name and pinned revision) above the **Client** row. The environment is the run's identity; the client shows how the environment resolved. For legacy host-backed runs, only the **Client** row appears.
