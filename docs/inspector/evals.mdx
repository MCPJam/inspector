...
The **Runs** mode (switch to it with the **Suites | Runs** control in the Evaluate header, or navigate directly to `/evals/runs`) shows CI-produced results keyed by commit. It shows only CI-active suites — suites created by the SDK or suites that CI has reported into at least once. Suites that have only ever been run from the UI stay in **Suites** mode. If a project has no CI evals yet, Runs mode shows a quickstart prompt.

The commit rail in Runs mode groups runs by commit SHA and only includes CI (SDK-sourced) runs. Playground runs on mixed suites are excluded from the rail so they don't appear as spurious manual-commit groups.
...
### Legacy `/ci-evals` URLs

Any bookmarks or CI log links pointing to `/ci-evals/*` automatically redirect to the equivalent `/evals/runs/*` path. Query strings, commit SHAs, and suite IDs are preserved exactly as encoded.
