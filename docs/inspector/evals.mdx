...
The judge model picker offers only MCPJam-hosted models that are eligible to run as judges (models with verified zero data retention). BYOK, org-provider, and local-key models are not offered. If a previously saved judge model is no longer eligible, it still appears in the picker but is shown disabled and tagged **Not eligible** — pick another model to replace it.
