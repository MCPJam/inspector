...
Each step row has a remove button (trash icon). Extra prompt steps in a multi-turn case can be removed as long as at least one prompt remains — the last prompt cannot be removed, because a case must always have something to ask. Non-prompt steps (pinned tool calls, interact steps) can always be removed. Removing a prompt that has checks nested under it shows a confirmation before deleting.
