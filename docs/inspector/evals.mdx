...
The **Groundedness** judge answers a different question from goal completion: *are the factual claims in each case's final answer actually supported by what the tools returned?* It is advisory — it never changes a run's deterministic pass/fail verdict.

Open a completed run and find the **Groundedness** card in the insight rail (below the goal-completion panel). Click **Run judge** to start grading. The judge only runs on an explicit click and never runs automatically.

Results are evidence-first: each failing case leads with the specific **unsupported claims** — the exact phrases in the answer that the tool trajectory does not support — followed by a per-case score and reason. A summary headline (e.g. `1/2 grounded`) appears at the top of the card.

| State | What you see |
|-------|-------------|
| Idle | **Run judge** button with a description of what the judge checks. Uses credits. |
| Pending | Spinner with "Checking claims against tool evidence…" |
| Results | Headline count, per-case scores, unsupported claims for failures, and a **Re-run judge** button to force a fresh grade. |
| Failed generation | The error summary and a **Retry** button. |

The card is only shown for completed runs and hides automatically when the backend does not yet support the judge.
