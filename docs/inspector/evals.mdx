...
- **User-value chain** — when you expand a trial, the scorecard shows one section per measured stage in chain order: Session connected → Tools discovered → Tool selected → Tool called → Response returned → Request satisfied. Each section heading carries the stage's state word (passed, failed, or a neutral label such as "never ran"). A failed stage whose evaluator rows do not already explain the failure shows the chain's own reason under the heading. This is shown for every trial — passing and failing alike. A trial that delivered end to end reads as the full delivery story; a failing trial shows the break in its stage section. During blind review (before you label a trial whose User value stage was decided by the judge), all six stage sections are shown; the User value section omits its state word, stage sentence, and tally line until you submit your label. The other five stages — decided by the runner, not the judge — remain fully readable.
