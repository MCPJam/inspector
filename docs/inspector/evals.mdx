...
Each Swarm and User Testing session detail also shows a **Checks** block listing every deterministic check that ran against that session — its name, PASS/FAIL verdict, and the evaluator's reason string. The block is separate from the judge section: a check is a rule that held or did not, while the judge is an LLM opinion, and the two are kept visually distinct. The block is hidden for sessions that have no check rows (eval sessions store their verdicts differently and do not show this block).
...
- **Checks** — when checks are configured on a case, an expandable Checks section in the iteration detail lists each check with a PASS/FAIL verdict, a one-line summary, and the evaluator's reason. Hidden when no checks are configured.
