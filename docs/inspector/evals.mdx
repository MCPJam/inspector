...
- **Suite** — a default the suite applies to every case. Standard check families inherited from the suite can be unchecked here to turn them off for this case only; other suite-level evaluators link to the suite's **Checks by stage** settings.
...
An advisory miss is shown on its row and never counted as a failure, because it did not fail the trial. A scorer with no result reads **Not measured** rather than passing by default. The verdict word itself stays in the header above.
