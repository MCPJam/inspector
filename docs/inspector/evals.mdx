...
**Analysis provider retention:** platform-key analysis calls require zero data retention and no training on every call. The pre-run disclosure (shown in the run tooltip in the app and in `mcpjam eval run` CLI output) includes an **Analysis providers** line that states this policy and names what it does not cover — for example, customer Playground chat. The line is absent on older backends that do not send the fact.
