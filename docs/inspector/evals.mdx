...
The simple form organizes authoring into three sections:
...
Steps are stored and executed in the order you author them. Adding a new tool check splices it after the last existing tool check, so the grading order matches what you see.
...
|-------|-------|-----------|
| Suite default | Suite settings → Grading tab → Default validators | Yes |
...
In suite settings under the **Where it runs** tab, the **Computer environment** select lets you pin a built Docker environment so every eval iteration boots a fresh sandbox from the same image. This keeps results comparable across runs and edits — each iteration gets an identical starting state.
...
The suite settings sheet is organized into tabs. Each tab is a vertical chain of always-expanded sections — no collapsing required.

| Tab | What lives here |
|-----|-----------------|
| **Grading** | Pass/fail policy, the user-value stage chain (Connection → Discovery → Selection → Call → Response → User value), checks, and judge config. |
| **Where it runs** | Environments (or Clients on deployments without project environments) and computer environment. |
| **Limits** | Token and turn budgets. |
| **Triggers** | Schedule and GitHub Checks. |
| **Delete suite** | Permanent deletion. |
| **History** | Committed settings revisions. |

Edits across all tabs accumulate in a draft. The **commit bar** at the bottom names the revision a save will create (for example, `Saving creates r13`) and links to the first blocking error. Click **Review and save** to commit, or navigate away to discard.

### Grading tab

The Grading tab shows the **user-value chain** — six stage cards from Connection through User value. Each card describes what decides that stage:

- **Connection** and **Discovery** are measured by the runner. Their cards list the effective configuration for every run target the suite fans out to: client, server, transport, auth, header names, protocol version, request timeout (Connection); tool visibility, progressive discovery, pagination traversal, and which servers get their `tools/list` taken (Discovery). These values are read-only here — they are shared with every other suite pointing at the same client or server — and each line links to the Hosts, Servers, or Environments page that owns it.
- **Selection**, **Call**, **Response**, and **User value** are graded by checks and the judge. Stages with no grader yet are highlighted so you can see the gap.

### Where it runs tab

The **Environments** row (labelled **Clients** on deployments without project environments) controls which client × model cells the suite runs against. The **Computer environment** row pins a Docker image for every iteration's sandbox.

### Triggers tab

The **Schedule** row sets a recurring run interval and which environment a scheduled run uses. The **GitHub Checks** row connects the suite to a repository so a run fires on every pull request.

### Suite execution config

The model, system prompt, temperature, tool approval, connection settings, capabilities, and host context are configured on the **Grading** tab under the stage chain.
...
- **Server selection** — Servers are not configured here. They come from the suite's environment.
