...
When authoring cases via a suite file or the API, each case also accepts a `kind` field (`"capability"` or `"regression"`). When absent, the kind is derived from the case's `matchOptions`: `strict` order with zero extra calls is `regression`; everything else is `capability`. Setting it explicitly pins the label regardless of `matchOptions`. Send `null` on a PATCH to clear a previously set value.
