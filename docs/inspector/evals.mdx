...
The run timeline below the case form lists past iterations. Click any row to open that trial's evidence in a drawer — the Scorecard, Chat, Tool Calls, Trace, and Steps tabs are all available there. The case form stays editable while the drawer is open; close it to return to the workspace.

When you open a trial from history, the drawer shows the trial's snapshot — the exact steps, checks, and settings that were in effect when it ran — so you can compare what you see in the trace against what was authored at the time.

### Setup Run sheet (case workspace)

Click **Setup Run** above the case form to open the **Setup Run** sheet. It owns the trial count, client, and model choice for the next run:
...
Click **Run test case** inside the sheet to launch. Click **Suite settings** to configure hosts and other suite-level defaults.
...
Click **Setup Run** in the suite header or on the **Suites** landing list to open the **Setup Run** sheet. It owns the trial count, client, and model choice for the next run, so you can confirm or adjust before committing. The run only starts when you click **Run test case** (or **Run all**) inside the sheet; closing it leaves the suite unchanged.
...
- **Minimum iterations** — Set a floor on how many times each case runs. Every case runs at least this many times, regardless of its own iteration count. New suites default to **5 runs per test**. Use `mcpjam cloud eval update --suite <name> --min-iterations <1-10>` to set this from the CLI; `off` removes the floor.
...
- **Runs tab** — every run with its summary metrics. Click in for the iteration-level breakdown. The table shows **Date** first, then **Run**; individual run rows are labelled **#1**, **#2**, and so on. Client/model pairings are shown inline per run row; when there are more than two, a **+N** badge appears — hover it to see all pairings in a tooltip.
...
## Importing cases from Markdown

If you have existing documentation, runbooks, or specification files, you can extract test cases from them directly. Click **Import from Markdown** in the suite header, paste or upload your Markdown content, and an AI extractor proposes cases based on the passages it finds.

Each proposed case opens in a review editor before it is saved. Cases flagged as unsupported remain blocked with the reason shown — review the note and either edit the case to fit a supported shape or discard it. Cases you approve are saved with a source provenance record that traces each case back to the file, its SHA-256 hash, and the line range the extractor read.
