...
Click **Export** at the top of the run detail page to download traces as OTLP JSON (OpenInference + MCP Apps conventions) for ingestion into Arize Phoenix, Datadog, or any OTLP-compatible backend.

The export modal lets you choose:

- **This run** — exports only the sessions recorded for the selected run. Disabled if the run has no recorded sessions.
- **Whole project** — exports all traces across the project, paginated automatically.

**Include content & artifacts** is off by default. Enable it to include prompts, outputs, tool arguments and results, and screenshots. Leave it off when traces may contain secrets.

The file downloads as a timestamped `.json` file.
