...
**From the Swarms tab** — Open a completed run, click a session row to select it, then click **Promote to test case** above the session viewer. The button is enabled only for sessions whose run attempt reached **succeeded**. Sessions from failed, rate-limited, or still-running attempts show the button as disabled with a reason on hover; sessions with an unknown run outcome are also blocked.
