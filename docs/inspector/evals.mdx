...
<Note>
  **Browser tools in eval suites get their own per-iteration desktop box.** When the suite's host config includes the `browser` built-in tool and a `browserToolPolicy` is declared, each iteration automatically provisions a dedicated desktop sandbox — not the member's personal computer. This means N parallel iterations never share a browser profile, cookie jar, or daemon.

  Two requirements must both be met for the desktop box to be provisioned: the `browser` tool must be in the host config's built-in tool list, and a `browserToolPolicy` must be declared. A `browser` tool with no policy advertises no browser tools to the model (fail-closed), so no desktop is booked.

  Combining a pinned computer environment with browser tools is not supported — the run fails with a `desktop_pin_conflict` error. Use the stock desktop image (no pinned environment) for browser eval runs.
</Note>
