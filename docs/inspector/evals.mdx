...
Every case opens in this form, however it was authored — in the app, through the CLI, or from an SDK suite file. It organizes authoring into three sections:

- **User asks** — the prompt the model receives.
- **In the app** — interact steps (clicks, typing) and widget assertions recorded against a live widget, shown in the order they run.
- **Check the result** — tool-call assertions and other checks that decide pass/fail.

Steps are stored and executed in the order you author them. Adding a new tool check splices it into the first turn, after the last tool check already there, so the grading order matches what you see.

Under **More checks**, a case shows both kinds of check it can carry, without converting between them:

- **Checks written as steps** (what the CLI and the SDK produce) are graded in place, at their position in the run. They are labelled by that position — a step-level "no tool errors" reads **No tool errors so far**, because it only sees the transcript up to that point. Editing one rewrites that step; it never becomes a case-level check.
- **Case-level checks** you add here are graded once, over the finished transcript.

Because a failing step halts the run, a check written as a step can leave later checks unrun. The trial column marks those **Skipped**.

**Which tool should handle it?**

The answer to this question is what marks a case negative — never an inference from its steps:
...
- **No specific tool is required** — the case names no route and is graded by its checks. This is a normal positive case, and it is what most CLI- and SDK-authored cases are.

A brand-new case that asserts nothing at all must answer this question before it can be saved or run: without a check, a rubric, or a named tool, it would pass vacuously.
