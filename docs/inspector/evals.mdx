...
When a case expects tool calls and `argumentMatching` is not `"ignore"`, a locked **Arguments match** row also appears under Tool call. It is graded by its own scorer (`toolCalls:arguments`) and reports whether every expected call was made with the expected arguments — naming the tool and the argument key, never the value. The **Route** row at Selection continues to report which tools were called. Runs graded before this split show no Arguments match row and render as before.
