...
- **Execution provenance** — above the tabs in the iteration detail, a "Ran on \<model\> via \<rail/connection\>, \<harness vX\>, effort/temperature, max output" line shows what the iteration actually ran on. When the run deviated from the requested model selection — for example, a provider fallback routed the call through a different rail — a visible **Deviation** banner appears so the fact is not missed. Iterations recorded before this feature was deployed show nothing; nothing is inferred from the case's model field.
