...
<Note>
  GitHub Checks is a beta feature available to organizations with the integration enabled. The section appears in suite settings only when your organization has access.
</Note>

The **GitHub Checks** section in suite settings lets you connect the current suite to a GitHub repository so it runs as a check on every pull request. This is the point-of-intent entry: you're already looking at the suite you want enforced, so the connection lives here.

### Connecting a repository

1. Open the suite's **Settings** tab.
2. Scroll to **GitHub Checks**.
3. Pick a repository from the **Repository** dropdown — only repositories not already bound to another suite are listed.
4. Click **Connect**.

Each repository runs exactly one suite. If a repository you expect to see is missing, it may already be connected to a different suite. Use **Settings → Integrations → GitHub Checks** to see all connected repositories and manage them.

### What the section shows

- **Connected repositories** — repositories currently running this suite. A **(paused)** label appears next to any repository where checks are paused.
- **No repositories run this suite yet** — shown when no repositories are connected.

The section intentionally omits pause, disconnect, and retarget controls. Those are repository-level decisions that belong in the full management surface (**Settings → Integrations → GitHub Checks**), where every repository is visible at once.

Click **Manage in Settings → Integrations** to go there directly.
