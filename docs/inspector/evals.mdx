...
Click **Add case** in the suite header (or in the empty-state screen when no cases exist yet) to open a menu with four options:

- **Add manually** — opens the case workspace so you can write the prompt, steps, and assertions by hand.
- **Generate** — reads your attached servers' tool catalog and drafts cases automatically (see [Generating cases from your tools](#generating-cases-from-your-tools)).
- **Import** — opens the Markdown import dialog to extract cases from a document (up to 100 KB).
- **Describe** — opens a prompt where you describe the scenario and the editor fills in the case.

The case is a draft until you press **Save** — backing out without saving leaves your suite unchanged.
...
One list, one **Add assertion or action** menu. Each row says what it checks, who wrote it, and what a miss does:
...
The trial opens on its **Scorecard**: the same scorers the left column lists, in the same order, each with what happened to it on this trial. Each row is an expanded report showing:

- **Looks for** — what the evaluator checks.
- **Observed** — what the runner actually saw on this trial.
- **Why it passed / failed** — the evaluator's reason, with blind-review masking applied until you submit your label for judge-decided rows.

A row also shows its result, its provenance, and its role. A scorer with no result reads **Not measured** rather than passing by default. The verdict word itself stays in the header above.

The summary line counts **required rows only**: "2 of 2 required passed · 1 advisory". An advisory miss is shown on its row and never counted as a failure, because it did not fail the trial.
...
Opening the Evaluate tab lands on the **Overview** by default, showing a flat table of all runs across suites — suite name, client, model, result, pass rate, platform, commit, date, latency, tokens, and tool calls — together with Suite Health bars above the table. Switch to **Suites** using the header tabs to see the suites overview. To drill into a suite, click its name; the **suite switcher** in the breadcrumb lets you jump between suites, create a new one, or delete the current one. If no suites exist yet, the same empty-state hero appears on both the Overview and Suites tabs — click **Eval my server: &lt;server name&gt;** to open the create-suite form with that server and a name already filled in, or click **Create suite** to start blank.
...
- **Runs tab** — every run with its summary metrics. Click in for the iteration-level breakdown. The table shows **Run** first, then **Client** and **Model** in separate columns; the **Client** column shows the host logo and name, and the **Model** column lists the model(s) used. When there are more models than fit, a **+N** badge appears — hover it to see the rest in a tooltip.
...
The run header shows a **SHIP** pill for each client/model pairing that passed. Pairings that did not pass are not shown as a HOLD pill — only passing pairings appear. Intermediate states (**Running**, **Grading**, **Inconclusive**) each get their own pill. The **Compare runs** and **Run again** buttons are visible directly in the header.
...
**Generate** (available in the **Add case** menu) reads your attached servers' tool catalog and drafts realistic cases — a mix of positive ("call `search` with a query") and negative ("don't call `delete_user` on a meta-question"). Treat it as a draft: skim, edit the prompts to match how your users actually talk, tighten the assertions, then save.
