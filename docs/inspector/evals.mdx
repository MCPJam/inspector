...
### Pinning a sandbox image at the environment level

When the **computers** feature flag is enabled and your deployment uses project environments, the project environment editor includes a **Sandbox image** section. Pinning an image there applies to all eval runs in that environment — each run boots a fresh, isolated sandbox from the chosen image.

- Images that have not finished building are labelled **(not built)**; selecting one will cause runs to fail at launch. Build the image first under **Computer → Images**.
- Personal draft images are listed but cannot be selected — promote the image to project-shared first.
- Selecting **None (default image)** uses the provider's default base image.

<Note>
  The sandbox image pin applies to eval runs only. Playground, chatboxes, and swarms do not use the pinned image yet.
</Note>
