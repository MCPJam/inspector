...
- **Browser tab** — appears on iterations where the eval ran MCP App widgets through the headless-browser harness. It has two sections:
  - **Render observations** — one card per widget showing the latest render outcome (status badge, screenshot, turn number, and timing). Possible statuses include `Rendered`, `Render error`, `Bridge timeout`, `Blank screenshot`, `Browser unavailable`, and others. Console errors captured during rendering are shown in a collapsible list on the card.
  - **Computer Use timeline** — the sequence of browser interactions the harness performed, grouped by widget. Each step shows the action (`Left click`, `Type`, `Scroll`, `Screenshot`, …), elapsed time, a step screenshot when available, and any tool calls the widget made during that step.
