...
The judge grades recorded traces — the full conversation, tool definitions, runtime context, and any captured media — not just the final answer text.

### Grading instructions

In suite settings, the **Grading instructions** field lets you add free-text guidance the judge applies to every case alongside each case's own task and expected outcome. Use it to express requirements that are hard to capture as structured criteria — for example, "only count success when a tool result confirms it; saying 'done' is not enough."

Structured rubric criteria (authored through the API) remain supported and appear read-only in the suite settings panel. Instructions and criteria can be used together.

### Per-iteration grading status

Each graded row carries a `status`: `scored` (a verdict was produced), `error` (the judge could not grade this iteration), or `skipped` (grading was disabled for this case). Interpret `score` and `passed` only for `scored` rows.

A failed grading job retains the measurements it completed — job failure does not mean every iteration failed. The **Retry grading** button appears when any iterations errored, and retries only those rows without regrading ones that already have a scored result. Use `mcpjam cloud eval judge --run <id> --scope failed` for the same operation from the CLI.

### Per-case judge override

Set `enabled: false` on an individual case (via the API or a suite file) to skip goal-completion grading for that case. The suite setting applies to all other cases. Set `judge: null` to clear a per-case override and restore the suite default.
...
Click a **test case name** to open its definition — the user prompt, checks, and expected outcome. Click a **result cell** (pass/fail) to open a **case drawer** with per-pairing averages (passed count, P50, P95, tokens, calls) and a flat iteration list. Click an iteration row to open its full detail view — scorecard, transcript, tool calls, and trace — without leaving the run page. A **Back** breadcrumb returns to the iteration list. In the scorecard, each scorer row shows an **Expected** field (the configured criterion) and an **Actual** field (what the run recorded). When the findings pipeline has produced a narrative for the iteration, the **Actual** field shows the AI-generated explanation with a cited trace evidence disclosure; if the narrative predates the latest grade it is marked stale and the recorded evidence is shown instead.
