...
#### Goal completion trend in swarms

The swarm Overview tab shows a **Goal completion** strip above the runs list when the overview window has a graded pass rate and at least two graded days. The strip headlines the window pass rate with its passed/graded session denominators and a daily sparkline. Days with no graded sessions are omitted from the sparkline — a gap in grading is not the same as a 0% day. The strip does not appear for a single graded day (one day is a number, not a trend) or when the window pass rate is unavailable.
...
When at least one run in the window has been judged, a **Judge score · advisory** card appears beside the accuracy chart. It shows the latest judged run's score and a sparkline across all judged runs. Unjudged runs are dropped from the trend rather than plotted as zero — a run that was never graded is not a run that scored 0%.

The card is deliberately separate from the deterministic pass-rate chart: a judge score and a failed check are different kinds of signal and should not be read as the same thing.

If a run was graded under a per-run judge override (a judge config that differs from the suite's default), its dot on the sparkline and its tooltip are marked with a **⚙** glyph so the divergence is visible rather than silent.
