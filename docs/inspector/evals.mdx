...
In both cases the dialog asks for a **Test case name** (prefilled from the session) and a destination suite.

**Adding to an existing suite** — the default when the project already has suites. A suite picker appears with the first suite pre-selected. The suite's client and server are shown read-only underneath; you are not asked to choose them again.

**Creating a new suite** — shown automatically when the project has no suites, or when you switch to **New suite**. A suite name field appears (seeded from the session's servers), followed by **Client** and **Server** pickers side by side, also seeded from the session.

Client and server belong to the suite, not the individual case. Once a suite exists, promoting further sessions into it reuses its configuration without re-asking.
