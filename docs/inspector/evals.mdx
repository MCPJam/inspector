...
- **Prompt turns** — one or more user messages. Multi-turn is supported; expected tools and checks attach per turn, so you can model "ask, get a result, follow up." Each expanded turn also has its own **Checks** section for assertions scoped to that turn only (see [Per-turn checks](#per-turn-checks)).
...
In addition to case-level checks, you can add checks scoped to a single turn. Expand any prompt turn in the case editor and use the **Checks** section that appears beneath it. These checks are evaluated against only that turn's slice of the transcript, not the full iteration.

Per-turn checks are useful when you want to assert on an intermediate step — for example, "turn 2's response contains the word 'confirmed'" — without that assertion applying to every turn.

<Note>
  The **Add check** menu inside a turn only shows check kinds that can be scoped to a single turn. Check kinds that require the full iteration context are not available here.
</Note>
...
- **Predicate Gate** — when `successPredicates` are configured on a case, an expandable Predicate Gate section in the iteration detail lists each case-level predicate with a PASS/FAIL verdict, a one-line summary, and the evaluator's reason. Hidden when no case-level predicates are configured. Per-turn check verdicts appear under their respective turn in the **Turn Breakdown** list instead.
