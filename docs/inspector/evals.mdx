...
Opening the Evaluate tab lands on the **Runs** view by default, showing run history across all suites with grouped runs, pass-rate changes, and filters. Switch to **Suites** using the header tabs to see the suites overview. To drill into a suite, click its name; the **suite switcher** in the breadcrumb lets you jump between suites, create a new one, or delete the current one. If no suites exist yet, the empty-state screen appears instead.
...
- **Friction signals** — report-only observations about observable patterns in a trial's tool calls. When signals are present, a collapsible row appears in the case body headed by the most consequential pattern found: **Possible detour** (an identifier a result surfaced that no later call used, or the same search repeated after one), **Retry** (identical or changed arguments), or **Pagination** (only pagination keys changed). Expanding the row shows the specific calls involved. These are signals, not verdicts — every pattern has a benign reading, and none of them affects the trial's pass/fail result. A trial whose tool results were not retained still reports retry signals; the identifier half says "not measured" rather than guessing. An absent friction section means the trial predates the measurement, not that nothing fired.
- **Suspected condition** — when the backend judge has run on a flagged trial, a line under the friction detail names which server-controlled condition is *suspected* of contributing, plus one remediation naming a server lever. `unclear` renders as "could not attribute" with no next step; an unverified verdict renders as "not available". Trials the judge never reached show nothing — "we never looked" is never rendered as "we looked and found nothing". This is advisory only and never a claim about cause.
...
2. On the **Suites** landing, click **Eval my server: &lt;server name&gt;** to open the first-run workspace. MCPJam reads your server's tool catalog and generates a draft suite of cases for you to review.
