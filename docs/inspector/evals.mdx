...
### Prerequisite setup for swarm journeys

New journeys created in the UI default to **Create prerequisite data before each run** enabled. The confirm step has a switch to disable it. Journeys created through the API or SDK have setup off unless you pass `setupWrites: true`.

When setup is on, each target attempts to create prerequisite state before its first session. The run and detail views show a **grounding disclosure** — what the personas knew, which entities were created, readiness status, and any residue — separately from the graded session matrix. Setup events do not create session cells and are not included in Findings.

<Warning>
  Setup can invoke real write tools against your connected servers. Created data is **not cleaned up**. Use a test account.
</Warning>

**API and SDK:** pass `setupWrites` on create or update. On `PATCH`, `setupWrites` must be sent together with `sessionsPerTarget` and `maxTurns` — they are one execution config object. Replacing the pair without `setupWrites` clears it; send its current value to preserve it.
