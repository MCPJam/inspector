...
## SDK/CI-created suites

Suites created by the MCPJam SDK (for example, from a CI pipeline using `EvalSuite` or `reportEvalResults`) appear in the Evaluate tab alongside suites you author in the UI. They are labelled with a **CI** badge in the suite switcher so you can tell them apart at a glance.

A few things work differently for CI suites:

- **Delete is disabled in the switcher.** CI suites represent your pipeline's historical record; the next CI run would recreate the suite anyway. To remove a CI suite, delete it from the suite settings page.
- **They are fully runnable and editable in Evaluate.** You can add cases, attach servers, and trigger runs just like any other suite. If no servers are attached, you'll see the usual "attach a host" prompt.

When you open the Evaluate tab, it lands on the suite with the most recent completed run — whether that run came from the UI or from CI.
