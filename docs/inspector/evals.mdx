...
- **Findings** — the insights rail groups two panels together and stays visible for both completed and failed runs:
  - **Suggested fixes** — ranks tool-quality and workflow issues by impact. Each issue has a **Copy** button that copies a ready-to-use fix prompt (tool description + input schema) for a coding agent; **Copy top N** copies the top issues combined.
  - **Goal judge** — the LLM-as-judge scorecard showing how many cases meet the goal, judge disagreements with the deterministic pass/fail, and per-case scores. Click **Run judge** to grade the run.
