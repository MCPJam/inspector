...
The trial opens on its **Scorecard**: the same scorers the left column lists, in the same order, each with what happened to it on this trial. The scorecard is one column of stage sections, in user-value chain order. Each section is headed by the stage name and a state chip (passed, failed, or a neutral label); under the heading sits at most one sentence about the stage as a whole — the AI note when one exists, else the recorded floor, else the chain's own reason for a failure no row explains. Then every evaluator that grades that stage appears as an **Expected / Actual** pair separated by hairlines. **Expected** is the configured criterion; **Actual** is the AI-generated narrative when one is current, else the recorded reason and evidence, else the formatted value, else "No observation recorded."

Connection, Discovery, Tool call, and Response each lead with a **Built-in** runner check row — a locked row that reports what the stage analysis decided, in the same Expected / Actual form as your evaluators. It never gates a trial and is not counted in the summary line. Tool call and Response runner checks appear only when the case gives the runner a call or response to measure.
...
**Data sent for rubric checks:** when a suite has grading criteria or rubric-check questions, every trial the goal-completion judge grades is also sent to TypeSafe AI's Jev classifier through Vercel AI Gateway: the request, the expected outcome, the final answer and a clipped trajectory, with credentials redacted. If Jev can't be reached, the same questions go to a fallback model through Vercel AI Gateway or OpenRouter. Turn rubric checks off in suite settings if that isn't appropriate for your data.

Rubric checks ask each of the suite's grading criteria on its own, as a yes or no question, and record the classifier's probability of yes. A suite can add up to ten questions of its own, each either a **choice** (pick one of several options; you mark which ones pass) or a **score** (an ordered scale; you pick the lowest level that passes).

- **Advisory, always.** A rubric check never fails a trial, never gates a run, and never decides the User value link. There's no model, threshold or role to pick.
- **Runs with the goal judge.** Rubric checks grade exactly the trials the goal-completion judge grades, in the same job. While that judge is off, they're paused.
- **Uncertain is not a pass.** A criterion answered with a probability of yes from 0.4 up to 0.6 shows as **Uncertain** on the trial scorecard, beside its value. The stored row still passes at 0.5 or more.
- **Wording is identity.** Editing a criterion's or question's wording, options, levels or pass line starts a new rubric-check row, so a baseline comparison shows the old row removed and the new one added.
- **App only, for now.** Rubric-check settings are edited in the app, and the API refuses `settings.judge.rubricChecks`. The criteria they ask come from `settings.judge.rubric`, which the API does accept.

Each answer appears on the trial scorecard under User value, after the goal judge, and under **Score rows** as `judge:rubricChecks:c:<criterion id>` or `judge:rubricChecks:q:<question id>`.
...
1. **Servers** — for environment suites, servers come from the server group selected on the environment (plus any explicit plugin pins). If no group is picked, the suite bar shows a notice and the run starts with zero servers — cases that expect tool calls will fail. For non-environment suites, one or more servers must be attached directly to the suite.
...
<Tip>
  If **Run all** is disabled, no models are selected or a run for that suite is already in progress. For non-environment suites, a missing connected server also disables it. The header pickers will tell you which condition applies.
</Tip>
...
- **Server selection** — Servers are not configured here. They come from the suite's environment. Specifically, an eval run connects the **server group selected on the environment** plus any explicit plugin pins — not the client's own stored servers. If the environment has no server group and no plugin pins, the run starts with zero servers and cases that expect tool calls will fail. The server picker is intentionally hidden in this editor.
...
compare runs, waive a gate, read every setting — and delete the suite.
...
A CI-managed suite can be deleted, from the suite switcher, the suite card and
the suite page, by anyone whose role lets them delete a suite they can see.
Deleting is not editing: it says what this workspace keeps, and it makes no
claim about what CI runs.

The confirmation names the two consequences, because neither is visible from
the button: the run history stored here goes with the suite, and the next CI
run or SDK report creates the suite again. A suite the SDK reports into is
identified by **name**, so renaming `suiteName` in your code does not rename
the suite — it creates a second one and leaves the first behind. Deleting the
one you no longer report into is how you clean that up.
...
- **Run insights** — an AI-written diff against your previous completed run: which cases moved, which tools changed behavior, which models diverged. Skim this first; it usually points at the right rabbit hole. The model pass is **included with MCPJam and consumes no credits**; it counts against a daily insight quota shared with swarm wave and user-testing insights. Guests browsing without a signed-in account will see a **Sign in** prompt in the insights band — signing in unlocks the feature without losing your place.
...
- **Declared.** The launching process says which of those it is. The CLI declares
  `CLI` (or `GitHub` inside an Actions job, with the commit, PR, and branch
  filling the GitHub provenance columns); the hosted MCP worker declares `MCP`
  and names the calling agent. Hovering a declared badge says so.
...
Click a **test case name** to open its definition — the user prompt, checks, and expected outcome. Click a **result cell** (pass/fail) to open a **case drawer** with per-pairing averages (passed count, P50, P95, tokens, calls) and a flat iteration list. Click an iteration row to open its full detail view — scorecard, transcript, tool calls, and trace — without leaving the run page. A **Back** breadcrumb returns to the iteration list. In the scorecard, each evaluator row shows an **Expected** field (the configured criterion) and an **Actual** field (what the run recorded). When the findings pipeline has produced a narrative for the iteration, the **Actual** field shows the AI-generated explanation; if the narrative predates the latest grade it is marked stale and the recorded evidence is shown instead. Evaluators are grouped by stage rather than split into default and added-assertion sections.
...
- **User-value chain** — when you expand a trial with a verified decision chain, the scorecard shows a two-column layout: a stage rail on the left lists all six user-value stages (Session connected → Tools discovered → Tool selected → Tool called → Response returned → Request satisfied) as a column of clickable links, and the selected stage's detail opens on the right. The scorecard opens on the stage where the chain broke; click any other stage in the rail to read it. Each open section's heading carries the stage's state word (passed, failed, or a neutral label such as "never ran"). A failed stage whose evaluator rows do not already explain the failure shows the chain's own reason under the heading. When no verified chain exists (for example, before a run has completed), the sections stack in chain order instead. During blind review (before you label a trial whose User value stage was decided by the judge), the rail opens on User value — where the label control lives — so you always label the same section regardless of which stage the judge decided. The User value section omits its state word, stage sentence, and tally line until you submit your label; the other five stages remain fully readable.
