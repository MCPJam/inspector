...
### Configuring the case mix

**Generate** is a split button. Clicking the label generates immediately using the suite's last-saved settings (defaults: 2 simple, 2 multi-tool, 1 multi-turn, 1 complex, 2 negative). Clicking the **chevron** opens a config popover where you can adjust the mix before generating.

The popover lets you set a per-bucket count (0–10 each, 20 total) and toggle **Vary user styles**:

| Bucket | What it generates |
|--------|-------------------|
| **Simple** | Easy, single-tool cases |
| **Multi-tool** | Medium cases that call 2 or more tools |
| **Multi-turn** | Follow-up workflow cases |
| **Complex** | Hard or cross-server cases |
| **Negative** | Cases that should not trigger any tools |

**Vary user styles** conditions the generated queries on a range of realistic user phrasings, so the cases read like different users wrote them. Off by default.

Settings persist per suite in your browser. The one-click **Generate** always uses the last-saved config for that suite; an all-zero mix falls back to the default 8-case mix.
