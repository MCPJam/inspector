...
### Completing an interact step

When you add an `interact` step manually (rather than recording it), the editor seeds placeholder values — an empty view tool and an empty element target — that must be filled before the case can be saved or run. **Save** and **Run** stay disabled until every interact step has:

- A **view (tool)** selected — the MCP tool that renders the widget the interaction targets.
- An **element target** with at least one non-empty reference point: `testId`, ARIA role, visible text, or CSS selector.

The tooltip on the disabled button names the specific field to fill and, when the case has more than one turn, identifies which turn the incomplete step belongs to. Fields that still need a value are outlined in red.
