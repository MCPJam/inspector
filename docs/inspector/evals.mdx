...
The list reads as a checklist. Each row shows its name and, when applicable, a one-line description beneath it. Click **Edit evaluators** (top right of the evaluators column) to open every check's settings at once — including checks that are still off. Checking a box also enters edit mode. While settings are open, each row shows its full criterion, the **Required / Advisory** role control, and any editable fields (thresholds, match criteria); a configured number appears only in its field, not repeated in the row header. Click **Close evaluators** to collapse all settings. An off check the user can toggle stays in foreground text; a locked or deployment-unsupported check is muted.
