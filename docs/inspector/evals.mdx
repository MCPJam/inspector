...
- **Model and prompt** — Set the default model ID, system prompt, and temperature for all runs in the suite. When running evals against MCP Apps that render widgets, the eval harness automatically enables browser interaction (screenshot, click, type) for any model that supports both vision and tool calling — not just Claude. Models that lack vision or tool calling run without browser interaction. The model picker uses the eval workload policy: a model the catalog has confirmed does not support tool calling is disabled, and a model whose tool support is unverified is also disabled and tagged **Not verified** — because a run that cannot call tools produces an invalid result. Models from older catalog snapshots or BYOK rows without catalog observations are unaffected and remain selectable.
