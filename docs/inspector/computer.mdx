...
The browser panel includes a full navigation bar with Back, Forward, Reload, and an address field. Type a URL to navigate directly, or type any plain text to search Google. The tab strip is always visible; you can open new tabs with the **+** button and close any tab — including the last one, which returns to a fresh start page without ending the session.

Page-opened popups (for example, OAuth sign-in windows) are adopted into the tab strip rather than opening in a separate window. The browser keeps up to eight tabs at a time. Closing a tab returns focus to its opener, then the tab to the right, then the tab to the left.
