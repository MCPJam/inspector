...
The browser panel streams the live page as a series of JPEG frames. The pane scales frames down to fit the available space but never enlarges them, so text stays sharp even at high display resolutions.

When the stream stats overlay is open it shows:

- **Encoded size** — the pixel dimensions the capture produced.
- **Display size** — the pixel dimensions the pane is actually drawing at.
- **JPEG quality** — the effective quality and the quality that was requested, plus the reason for any difference (for example, `jpeg_frame_limit` when a frame was too large to send at the requested quality).
- **Delivery rate** — the maximum frames-per-second the relay is sending to this viewer, and whether delivery has been slowed due to congestion.

The pane automatically switches to a lower-bandwidth **Data saver** tier when it detects sustained frame loss. It recovers to the normal tier after the loss clears. Round-trip latency alone does not trigger a tier change.
