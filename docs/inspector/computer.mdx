...
<Check>
The terminal opens, and **Hibernate now** appears in the Computer tab toolbar — it is only visible when the computer is running.
</Check>
</Step>

<Step title="Run something on it">
Use the web terminal directly, or let an agent call the `bash` tool. Files you attach in the chat composer also land on the machine — see [Chat attachments](#chat-attachments).
</Step>

<Step title="Leave it alone">
The computer sleeps after about 30 minutes idle, or shortly after you close the terminal, and wakes on next use. Sleeping is non-destructive.

<Check>
Your files and installed tools are still there on wake — the disk and memory are captured in a pause snapshot. What does wipe them is listed under [What wipes it](#what-wipes-it).
</Check>
</Step>
</Steps>
...
<Note>
  ...
  any one computer's disk — see [Data & persistence](#data--persistence).
</Note>
