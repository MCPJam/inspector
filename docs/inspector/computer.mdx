...
The first time you use a local browser session, the Browser panel requires a one-time Allow action before the agent can browse. If permission hasn't been granted yet, the panel shows:

> Allow Browser below, then retry your request.

Click **Allow** in the Browser panel to grant permission. The message clears automatically once permission is granted. After allowing, retry the request that was blocked — the agent does not automatically replay a turn that ran before permission was granted.

If the agent responds that it cannot browse, check the Browser panel for this message and click Allow before retrying.
