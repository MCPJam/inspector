...
- **Cache provenance** - A badge appears when the tool list was served from the local client cache rather than a live server round-trip; click **Refresh** to force a live re-fetch
...
When the inspector serves a tool list from its local client cache instead of sending a request to the server, a badge appears below the search bar showing **"served from client cache · {age}"** (for example, "served from client cache · 4s"). The age reflects how long ago the cached entry was originally fetched.

The badge is only shown on an actual cache hit — its absence means the most recent list came directly from the server. To force a live re-fetch at any time, click the **Refresh** button; this bypasses the cache and always contacts the server.

<Note>
The cache provenance badge is not shown in hosted mode. Hosted connections always fetch live data directly from the server.
</Note>
...
- **Cache provenance** - A badge appears when the resource or template list was served from the local client cache; click **Refresh** to force a live re-fetch
...
- **Cache provenance** - A badge appears when the prompt list was served from the local client cache; click **Refresh** to force a live re-fetch
