...
Click any log entry to expand it and see the full JSON payload. To copy a value, use the copy button next to any object or array brace (`{…}`, `[…]`) — these buttons are always visible. Copy buttons on primitive leaf values appear on hover.
