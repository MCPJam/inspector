...
This behavior applies to both local and hosted connections. On local connections, the inspector must be connected to the server before the tool is invoked; the `elicitation` capability is advertised at connect time. On hosted connections, the operation suspends to a durable continuation — the stream ends while the round is pending, and your answer re-drives the turn automatically. A page reload abandons any pending hosted round; the server expires the continuation on its own timeout.
