...
The four work together on a single failure: invoke the tool from **Tools**, and when the result is wrong, expand that frame in the traffic log — the **HTTP headers** row names the first disagreement (for example, `mcp-name disagrees with the body`) before you open anything else.
