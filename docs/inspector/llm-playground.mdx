...
The playground displays real-time token usage and context window information to help you understand how much of the model's capacity you're using.

<Frame>
  <img
    className=\"block\"
    src=\"/images/playground/context-window.png\"
    alt=\"Context window indicator\"
    width=\"300\"
  />
</Frame>
...
Click the context indicator (circular progress icon with percentage) to view detailed token usage:

- **Input tokens**: Tokens used for user messages and tool responses
- **Output tokens**: Tokens generated by the model
- **System prompt**: Tokens used by your configured system prompt
- **MCP Tools**: Token count for each connected MCP server's tool definitions

Each section shows the token count and estimated cost based on the selected model's pricing.

<Info>
  Token counts are calculated using model-specific tokenizers when available. For unsupported models, a character-based estimation is used (approximately 1 token per 4 characters).
</Info>

<Warning>
  Keep an eye on your context usage. When approaching 100%, the model may start dropping earlier messages or fail to process new requests.
</Warning>
