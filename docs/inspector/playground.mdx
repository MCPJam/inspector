...
When you reopen a conversation that ran in a Project Environment, the Playground restores it onto the environment it actually ran in rather than defaulting to your current browser selection. Conversations that predate this recording — or that ran in host mode rather than a Project Environment — show a notice that the original execution target could not be confirmed.
