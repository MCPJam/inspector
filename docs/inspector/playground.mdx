...
- **Off (default)** — Destructive agent actions (such as running an arbitrary MCP tool via the agent's `ui_execute_tool` capability) still pause for your approval. Non-destructive agent actions run immediately.
- **On** — Every mutating agent action pauses for your approval before it runs. Read-only actions (such as taking a snapshot of the current app state) never require approval in either mode.

A denial is final — the agent explains what it wanted to do rather than retrying the call.
