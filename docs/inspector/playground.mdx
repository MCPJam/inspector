...
- **Raw** — The full JSON payload sent to the model (system prompt, tool definitions, conversation history). Available for both live sessions and previously-saved sessions you reopen. For reopened sessions, the synthesized payload includes the host's browser tool definitions when the host has the browser capability attached, so the tool list reflects what would actually be sent rather than showing an empty object.
...
When the previewed host has the browser capability attached, a **Browser** section appears in the Tools rail with two groups:

- **MCPJam browser tools** — the six `browser_*` tools the model is given, with their descriptions and JSON input schemas exactly as the model sees them.
- **Page tools** — the WebMCP tools the page currently open in the browser offers. These are reached by the model through `browser_webmcp_invoke`, not called directly by name. Click **Refresh page tools** to re-read the current page (the list does not update automatically as the model navigates).

The section is labeled **this machine** when the browser runs locally, or **your computer** when it is a hosted cloud browser.

Browser tools have no **Run** button — driving a browser goes through the approval flow that exists to put you in front of each action.

If the browser has not yet loaded a page, the Page tools group shows a "hasn't loaded a page yet" notice rather than an error. If someone else has taken control of the browser, the group shows a "taken control" notice until the lease is released.
