...
- **Shell** (shown when the previewed host has a Project Computer) — A live terminal on the computer the harness runs on. Click **Open terminal** to connect. For cloud computers, this provisions or wakes the machine; for local computers, it opens a shell on your own machine. Switching between Logs and Shell keeps both the terminal WebSocket and the log stream alive so neither is dropped.
...
When the previewed host has a Project Computer, a **Shell** tab appears next to **Logs**. Click **Open terminal** to open a live terminal session:

- **Cloud computer** — provisions or wakes the cloud machine and connects. The computer status chip in the Shell header shows the current state (starting, ready, error, etc.).
- **Local computer** — opens a shell on your own machine in the project workspace. The terminal stays idle until you click **Open terminal**, so no shell is spawned on Playground load.
