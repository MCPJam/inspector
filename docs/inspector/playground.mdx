...
When a browser is active, the Tools rail organizes tools into collapsible groups:

- **WebMCP** — Tools declared by the page currently open in the conversation's browser. Each row shows the page's own tool name first, with the name the model calls and the page's host beneath it. This group is hidden when no page is open. Tools change as the browser navigates.
- **Browser** — The `browser_*` verbs the host gives the model (collapsed by default). These drive the open browser; every call still asks the agent first.
- **Servers** — Tools from your connected MCP servers.
- **Built-in tools** — Host-managed built-ins such as `web_search`.

### Running WebMCP page tools

Selecting a WebMCP page tool and clicking **Run** invokes it directly on the page — the same path the WebMCP tab uses — without going through the agent. This works even while the agent is driving. The result appears inline below the parameter form.

Selecting a `browser_*` verb and clicking **Run** asks the agent to call it instead of driving the browser directly from the form.
...
The right rail has two tabs. The **Logs** tab is always present; the **Shell** tab appears when the previewed host has a Project Computer attached. When a browser is active, a **Browser** tab also appears.

When the agent starts a live browser tool call, the right rail opens automatically and switches to the **Browser** tab so you can follow along. If you switch to **Logs** (or **Shell**) while browsing is in progress, the rail respects that choice and will not switch back — it only auto-opens once per browse session. Switching to a different conversation resets this behavior for the new conversation.
