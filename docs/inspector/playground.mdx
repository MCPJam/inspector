...
| Claude Desktop | Omitted (`undefined`) | `["inline"]` |
| Copilot | Omitted (`undefined`) | `["inline", "fullscreen"]` |
| ChatGPT | Included | `["inline", "fullscreen", "pip"]` |

**Claude Desktop** is the Electron app; the **Claude** preset is claude.ai in a browser. The two share the same style variables and fonts but differ in display modes and `toolInfo`: the desktop app advertises only `inline` and does not deliver `toolInfo`, so a widget that calls `app.getHostContext()?.toolInfo` on the Claude Desktop preset correctly sees `undefined`.
