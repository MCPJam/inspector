...
When you open a Playground conversation that was started through the [session API](/reference/public-api#browser-tools-on-chat-sessions) (for example, via `cloud sessions send` or `POST /chat-sessions/messages`), the Playground shows the transcript in read-only mode. The composer is disabled and a notice reads **"This conversation is driven by an agent. Continue it through the session API."**

This is intentional: API sessions have a single writer (the agent), and the Playground is a viewer. You can still:

- Read the full transcript, trace, and raw views.
- Take over the session's live browser — if the session has a browser attached, the notice adds **"You can take over its browser here."** The browser panel opens normally and you can interact with the page. Select **Resume agent** in the browser panel when you are done to hand control back.

To open an API session in the Playground, use the **Open in Playground** link printed by `cloud sessions send`, or navigate to `/playground?conversation=<chatSessionId>`. If the session has browser artifacts, append `&browser=open` to open the browser panel automatically.

## Temporary Browser override

When Browser is available in your deployment, a **Browser** selector appears above the Playground layout. It lets you enable or disable Browser for the current Playground session without changing the selected client's saved configuration.

The selector has three options:

- **Client default** — Uses whatever the selected client has configured. The label shows the current default in parentheses, for example `Client default (Off)`.
- **On for this Playground** — Enables Browser for this session, making the browser panel available even if the client has it off.
- **Off for this Playground** — Disables Browser for this session, hiding the browser panel even if the client has it on.

The override applies across chat, multi-host comparison columns, the workspace panel, and the fallback right rail. New chats started within the same Playground session keep the selection. The override resets when you change project, client, or environment mode, or when you leave and return to Playground.

Enabling Browser makes the browser panel available and lets you choose a location and grant permission inside that panel. Enabling the override does not grant browser permission on its own. Disabling the override does not revoke device permission or terminate a browser you are currently viewing.

When an environment is active, the selector is replaced by a notice that Browser is controlled by the environment's client and cannot be overridden.

A **Browser settings** link appears in the browser panel header when a client is selected. Clicking it opens the client's [Browser tab](/inspector/clients#browser-settings) where you can change the saved enablement, profile, and personal preferences. Changes made there do not affect the current Playground session's temporary override.
