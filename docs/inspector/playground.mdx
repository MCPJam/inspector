...
Each column shows a compact identity row at the top with the host's logo and name, so you can tell columns apart at a glance. If no logo is available for a host, the row shows a two-letter initials badge instead. The Trace/Chat/Raw tab strip remains available in every column; per-column Latency/Tokens metrics are hidden because the same model runs in all columns.
