...
On hosted connections, `input_required` rounds use a durable continuation: the stream ends while the round is pending, and the dialog stays open. Submitting your answer automatically re-drives the chat turn so the agent loop continues. Stopping the turn or resetting the conversation withdraws any pending round. A page reload abandons a pending hosted round; the server expires the continuation on its own timeout.
