...
The panel also has a **Policy Diff** tab with three columns:

- **Requested** — The allowlists your widget declared in its CSP metadata.
- **Effective** — The policy the sandbox proxy actually injected for this mount. When the proxy has reported the applied policy, the subtitle reads **proxy applied** and the column reflects what the browser is enforcing — including tokens like `'unsafe-inline'`, `data:`, and `blob:` that `buildCSP` adds, and the full permissive wildcard in permissive mode. When no applied policy has been reported yet (an offline replay from a cached URL, a saved eval trace, or the window between mount and message arrival), the column falls back to the declared allowlists and is labeled **UNCONFIRMED** in amber to make clear it is a guess, not a reading.
- **Observed** — Requests the browser actually blocked, from CSP violation reports.
