...
### Tools rail in environment mode

When a project environment is selected in the Playground, the Tools rail switches to a read-only view that lists the tools exposed by that environment's servers. In environment mode the backend connects to MCP servers on every chat turn — the browser never holds those connections directly — so the ordinary tool list (which reads browser-held connections) would show "No tools found" even while tools execute successfully in chat. The environment tools view fixes this by fetching the tool list over the same connections a chat turn uses.

- The list is **read-only**: there is no Run button. To execute a tool, send a message in chat and let the model call it, or ask the model to call a specific tool.
- Tools are **searchable** by name and description. Expand a row to see the tool's description and input schema.
- When two servers expose a tool with the same name, a server badge appears on each row so you can tell them apart.
- If one server fails to connect or list its tools, that server shows an error row with the failure reason. Other servers' tools still appear — one unreachable server does not blank the panel.
- Click **Refresh** (the icon in the panel header) to re-fetch the tool list at any time.
