...
### WebMCP page tools in the Tools pane

When the agent browser is open and the current page declares [WebMCP](https://webmcp.dev) tools, a **Declared by the page** section appears below the browser's built-in tools. Each entry shows:

- The name the **model calls** — a `webmcp_*` tool the model can invoke directly on its next step, with the page's own name and origin shown beneath it.
- A **"page says read-only"** badge when the page's registration carries that annotation. This is the page's own claim and is shown as such — the approval classifier does not rely on it.
- A **"Not offered to the model"** notice for any tool the server could not advertise (for example, a schema no provider can express as tool arguments, or tools past the per-turn cap).

A **live** badge appears on the section header when a browser stream is open and the list is following the page in real time — it updates automatically as the model navigates, with no polling and no manual refresh needed. The badge is absent when no stream is open; the list shown is the last one read.

Tool cards in the chat thread show a **from `origin`** chip for page tool calls, identifying which page's tool produced the result. The chip reads from the result itself, so it stays accurate even after the browser has navigated elsewhere.
