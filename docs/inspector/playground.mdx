...
The host selector lists your created hosts first, followed by **static reference profiles** for Claude, ChatGPT, Cursor, Copilot, Codex, and other supported hosts. These preset columns are opt-in — they are not selected by default — and require no host creation or connection. Add one to compare your real host configuration against a reference profile without any setup.

Preset selections are preserved across reloads and in shared links, so a comparison you set up is exactly what a teammate sees when they open the same URL.
