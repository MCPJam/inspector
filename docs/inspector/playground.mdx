...
- The result appears in the center pane with the same row of debugging icons and widget rendering. A compact collapsible card above the assistant message shows the tool name, the arguments you supplied, and the output the model received — expand it to inspect what was sent. This card has no edit action and is not included in the composer's Up/Down message history.
...
Context cards (injected skills, manually run tools, MCP prompt examples) are not included in the history — only messages you typed are recalled.
