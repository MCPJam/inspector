...
When you expand an entry whose payload exceeded the 256 KB per-message log limit, a notice reads **Payload not recorded — over the 256 KB log limit.** The envelope fields (`id`, `method`, `result`/`error`) are still shown so the row stays identifiable; only the large body (for example, a base64 image in a tool result) is replaced.
