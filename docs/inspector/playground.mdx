...
If the out-of-credits dialog appears, the actions available depend on your plan and role:

- **Free plan (owner)** — The dialog shows **Compare plans** (opens your organization's Plans settings) and **Learn more about BYOK** (opens the BYOK and credits explainer under Usage & billing).
...
### Frontier models and sign-in

Higher-priced models are greyed out in the picker for guests, with a "Sign in to use this model" tooltip. Signing in removes this restriction — signed-in users on any plan can use these models. If you select a greyed-out model as a guest, a sign-in prompt appears showing the supported providers. You can sign in to get access, or choose **Not now** to continue with a standard model. This prompt does not affect your credit balance.
...
<Note>
  Host Capabilities configures what the mock host *advertises*. Enforcement —
  rejecting widget requests for capabilities not in the blob — is a planned
  follow-up. Until then, the inspector will still service all widget requests
  regardless of what the handshake advertises.
</Note>
...
<Note>The timezone control is only available for MCP Apps (SEP-1865).</Note>
...
**Tool Approval**
When on, pauses the model before tool calls so you can approve or deny each one. Each approval answers one call and is valid for 15 minutes; answered later, the call does not run, the tool card says the approval expired, and the model can ask again.
...
- local shell commands (`bash`);
- destructive UI actions;
- [workspace tools](#workspace-tools) that change something in your project.

Local Browser (`browser_*`) and WebMCP page tools follow the chat’s Tool Approval setting. With it off, calls run without per-call approval. Browser device permission remains a separate prerequisite.
...
**Message history**
Press **Up** in the composer to recall your last sent message. Press **Up** again to step further back through your previous messages, and **Down** to walk forward. Past the newest entry, **Down** returns the draft you had started before the walk began.

The history is the conversation itself — the user messages already on screen — so it cannot drift from what you see, and a session reopened from the Sessions rail brings its history along automatically. A fresh thread starts with empty history.

Arrow keys only reach history at the edge of the draft: inside a multi-line message they move the caret as usual. Modifier keys (`Shift`, `Alt`, `Ctrl`, `Cmd`) are never intercepted — `Shift+Up` still selects text.
...
When your Playground session runs against a [Project Environment](/inspector/projects) that pins plugin versions, a **Plugins** row appears in the environment header above the chat. Each pinned plugin is shown as a chip.

### Narrowing plugins per turn

By default, every plugin the environment pins is active. You can toggle individual plugins off for a single turn by unchecking their chip. The selection is ephemeral — it is never saved, and reloading the page returns to the environment's full set. The server rejects any plugin version the environment does not pin, so the toggle can only narrow, never expand.

When you have deselected at least one plugin, a **Reset to environment** link appears next to the **Plugins** label. Click it to restore the environment's full set.

### Pinned plugins that can't run

If a plugin the environment pins is currently disabled, uninstalled, or otherwise unavailable, it appears in a collapsible **can't run right now** section below the chips rather than as a toggleable chip. Each entry shows the plugin name (or its short ID when the name can't be resolved) and the reason:

| Reason | Meaning |
|--------|---------|
| Disabled | The plugin is disabled for this project. Re-enable it in Connect → Plugins. |
| Uninstalled | The plugin was removed from this project. Re-import it, or unpin the version from the environment. |
| Not ready | This revision never finished importing and cannot run. |
| Not found | This revision no longer exists in this project. |

This section also appears when the environment itself fails to resolve — the most common cause is a pinned plugin that can no longer run, and the failed preview cannot name the culprit on its own.
...
- **Explicit** — Type `/` in the input and pick a skill from the **SKILLS** section of the picker. The skill content is injected into the turn before your message.
...
Workspace tools that change something — `call_server_tool`, `start_claude_readiness_run`, `start_openai_readiness_run`, `cancel_readiness_run`, and every tool that creates, updates, deletes, starts, cancels or dismisses — always pause so you can approve or deny each call, whatever the **Tool Approval** toggle says. The reads that open a connection to one of your servers (`diagnose_server`, `list_server_tools`, `read_server_resource`, and the other prompt, resource and skill reads) follow the **Require tool approval** setting saved on the client or project — on when nothing is saved. The composer toggle can add these pauses for a conversation but cannot remove them. Read-only listing and polling tools never require approval, on any model.

<Note>
  These tools are only available when you are signed in as a project member.
  They are not shown to guests or in scenario sessions.
</Note>
...
When you open Playground on a local installation where the browser is available but not yet set up, a dialog appears asking **"Let agents use your browser?"** — even over a restored conversation and without requiring a saved client or connected MCP server.

- **Allow browser access** — grants device permission and enables browser tools for eligible clients. The dialog stays open until setup finishes; closing or pressing Escape while setup is in progress is blocked.
- **Not now** / **Close** / **Escape** — dismisses the dialog and remembers that choice across tabs and remounts. The dialog does not reappear for that installation.

If device permission was already granted on a previous visit but client setup was never completed, the dialog reappears once with the title **"Finish enabling browser tools"** and a **Finish setup** button instead of Allow.

If setup fails partway through, the error stays visible and a **Retry setup** button appears so you can try again without re-granting device permission.

After dismissing the dialog, you can still enable browser tools from the **Browser tab** in the right rail or from a client's **Browser settings**.

Playground follows the selected client's saved Browser enablement. Turn Browser on or off in the client's [Browser tab](/inspector/clients#browser-settings). A **Browser settings** link in the browser panel header opens that tab when a client is selected.

Browser tools are visible in the Tools rail as soon as the project's browser setting is on — a saved default client configuration is not required. If a specific client has browser tools explicitly disabled, the Tools rail shows a note explaining that and links to that client's Browser settings rather than showing another Allow prompt.
...
- [Plugins](/inspector/plugins) — Import and manage plugin bundles whose versions environments can pin.
- [Projects](/inspector/projects) — Organize servers, sessions, and views into shared workspaces.

### Local Browser and WebMCP security

Local Browser and WebMCP inspection require **Allow Browser** on the current device. Shell permission is separate. Tool Approval keeps its existing chat setting: when off, the model can act without asking before each browser or page-tool call. Enabling page tools gives you and the chat shared control of the inspection page. Ordinary websites, local development servers and other local-network apps remain accessible; MCPJam’s own controller endpoints and local-file navigation are blocked.

Page text, screenshots and tool results used in chat may be sent to your selected model provider. Signed-in websites and local-network services can contain sensitive information. Exports may include page contents, tool inputs/results and screenshots; review them before sharing.

Electron inspection keeps website sign-ins in a separate profile for each account and project, including a separate standalone profile. The upgrade requires signing in to websites again: the old shared inspection profile is never imported. **Clear inspection site data** closes sessions for the selected profile and deletes its website storage/cache. **Delete old shared profile** removes the unused legacy profile. Neither action deletes previously saved transcripts, screenshots, exported files or cloud profile copies. Node inspection is temporary and does not retain sign-ins after closing.

Electron website downloads require your decision in a native save dialog. They are limited to 1 GiB and never opened automatically. Node website downloads are disabled. Camera, microphone, geolocation, device access and clipboard reads remain denied; explicit human paste and file selection remain available.

Local profiles are files on this machine protected by your OS account and filesystem permissions. Some browser secrets may use OS credential protection; this is not a promise that all profile data, screenshots or transcripts are encrypted. Uploading a Playground browser profile to a cloud computer is a separate action that copies its data outside this machine.
