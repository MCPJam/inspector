...
If an App reports a CSP violation and never finishes loading, an inline notice replaces the empty widget area after a short delay. The notice names the blocked directive and the first blocked origin so you can identify the missing declaration without opening the debug panel. Once you add the domain to your `_meta.ui.csp` and the App loads successfully, the notice disappears.
