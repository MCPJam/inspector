...
If the Browser tool catalog fails to load, the Tools rail shows a **"Couldn't load Browser tools."** message with a **Retry** button instead of silently hiding the Browser section. Clicking Retry re-fetches the catalog; once it succeeds the tools appear and subsequent refreshes use the cached definitions.

Share-link guests can see Browser and WebMCP tools in the Tools pane. Browser execution and page reads remain independently gated — guests still need device permission for local browsing.
