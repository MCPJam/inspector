...
- workspace operations the platform gates behind a human click (for example, `call_server_tool` and running an eval suite).
...
`call_server_tool`, `start_claude_readiness_run`, and `start_openai_readiness_run` always pause for approval regardless of the Tool Approval switch — these open a connection to one of your servers and the platform requires a human click before they run. (The readiness starts consume no credits; the approval is about dialling somebody else's server, not about money.) When **Tool Approval** is on, `diagnose_server` and `read_server_resource` also pause. Read-only listing and polling tools never require approval, on any model. `cancel_readiness_run` is also direct — it stops traffic rather than starting it.
