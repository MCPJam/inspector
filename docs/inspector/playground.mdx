...
- **Raw** — The full JSON payload sent to the model (system prompt, tool definitions, conversation history). For live sessions this reflects the most recent outgoing request; for saved sessions (including reopened Playground sessions, swarm runs, and eval iterations) it shows the exact per-step request that was recorded at run time.
