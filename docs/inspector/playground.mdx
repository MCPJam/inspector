...
- **Search** the buffer
- **Filter** by source — includes **WebMCP** as a dedicated source option. Selecting WebMCP shows only tool calls that arrived through the WebMCP Inspector, with direction labels (`← webmcp` / `webmcp →`) and error styling when a tool call returns an error result.
- **Log level** filter
...
When a multi-model or multi-host compare session is running and at least one client has an active browser, Playground replaces the single browser panel with a **comparison browser**: one shared tab strip over isolated per-client browser sessions. Only the selected client's session gets a renderer at any time; switching tabs or clicking a different client's tab activates that session without affecting the others.

- Tabs are labeled with the client name when more than one client is in the lineup (for example, `Cursor · Nike`).
- The navigation bar shows `<client> is driving` when an agent holds the lease on the selected session.
- Sessions in the comparison browser are transient — they are never saved as a project default.
- Closing the comparison browser or ending the multi-client session discards the comparison state.
...
Local Browser and WebMCP inspection require device consent — click **Allow** in the **Enable local Browser for all clients** prompt in Browser settings. Consent is device-scoped: granting it on one machine does not affect other machines, and teammates must allow browser access on their own devices. Tool Approval keeps its existing chat setting: when off, the model can act without asking before each browser or page-tool call. Ordinary websites, local development servers and other local-network apps remain accessible; MCPJam's own controller endpoints and local-file navigation are blocked.
