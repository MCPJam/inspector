...
- **Multi-host compare** — Render the same tool call under multiple host configurations (ChatGPT vs Claude, different device sizes, different capability sets) at the same time. Client and model selection are unified into one run picker in the chat input toolbar.
...
Client and model selection are unified into a single **run picker** in the chat input toolbar. The toolbar shows two pills side by side — a **client pill** (host logo + name) and a **model pill** (provider logo + name). Click either pill to open its dropdown.

To compare across hosts, open the client pill and toggle **Multiple hosts** on. Select up to 3 hosts from the list; each selected host gets its own column. To compare across models instead, open the model pill and toggle **Multiple models** on.
...
<Note>
  The **Multiple hosts** toggle and the client pill are hidden when the active
