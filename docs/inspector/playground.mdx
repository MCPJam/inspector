...
Within each provider, models are listed newest first. Models without a known release date appear after dated ones, in their original catalog order.

### Row tags

The picker annotates rows with small tags when the catalog has relevant information:

- **Retiring \<date\>** — The provider has announced a retirement date for this model. The model remains selectable until it is removed from the catalog.
- **Not verified** — The catalog has not confirmed that this model supports tool calling. In chat with MCP servers attached, the model is selectable but a tooltip warns that tool calls may fail. In eval and persona runs the model is disabled, because a run that cannot call tools produces an invalid result.

### Capability locks

When MCP servers are attached to the chat, the picker uses the `mcpChat` workload policy. A model the catalog has confirmed does **not** support tool calling is disabled with a tooltip explaining why. A model whose tool support is unverified is allowed but tagged **Not verified**.

When no servers are attached, the picker uses the `chat` policy and no capability locks apply.
...
On the free daily allowance with no purchased credits to fall back on, some hosted models are locked in the picker. Only models the catalog marks as eligible for the free tier remain selectable. Upgrade your plan, add credits, or use your own API key to access locked models. Models you access with your own API key (BYOK) are never locked by this rule.
