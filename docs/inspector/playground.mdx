...
- **Multi-host compare** — Render the same tool call under multiple host configurations (ChatGPT vs Claude, different device sizes, different capability sets) at the same time. New sessions start with Claude, ChatGPT, and Cursor pre-selected so the comparison is immediate.
...
New sessions start with **Claude, ChatGPT, and Cursor** pre-selected so you land directly in a 3-way compare. To change the lineup, open the host picker and check or uncheck rows — checking a second row immediately adds it to the compare grid; unchecking a row removes it. Your selection is saved per project, so returning users keep whatever lineup they last configured.
