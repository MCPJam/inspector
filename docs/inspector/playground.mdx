...
Playground follows the selected client's saved Browser enablement. Turn Browser on or off in the client's [Browser tab](/inspector/clients#browser-settings). A **Browser settings** button (Globe icon) in the browser pane's nav bar opens that tab when a client is selected — the button travels with the browser pane wherever it appears.
