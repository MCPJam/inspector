...
If you are an org owner or admin, a **Manage organization models** footer appears at the bottom of the **Your providers** tab. Clicking it closes the picker and navigates directly to your organization's model providers page, so you can add or update BYOK keys without leaving the composer. The footer is not shown to members who cannot open org settings. The **Your providers** tab stays reachable even when no provider keys are configured yet, so admins can use the footer to add their first key.
