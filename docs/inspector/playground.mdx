...
If the out-of-credits dialog appears, the actions available depend on your plan:

- **Free plan** — The dialog shows **Explore plans** (opens your organization's Plans settings) and **Learn more about BYOK** (opens the BYOK and credits explainer under Usage & billing).
- **Paid plan (owner or admin)** — The dialog shows **Buy credits** to top up directly, and **Learn more about BYOK** to open the BYOK explainer.
- **Members who cannot manage billing** — The dialog shows a link to email an organization owner.

### Frontier models and sign-in

Some frontier models require a signed-in account. If you select a frontier model as a guest, a sign-in prompt appears showing the supported providers. You can sign in to get access, or choose **Not now** to continue with a standard model. This prompt does not affect your credit balance.
...
When **Tool Approval** is on, the model pauses before `diagnose_server`, `call_server_tool`, `read_server_resource`, `start_claude_readiness_run`, and `start_openai_readiness_run` so you can approve or deny each one — these open a connection to one of your servers. (The readiness starts consume no credits; the approval is about dialling somebody else's server, not about money.) Read-only listing and polling tools never require approval, on any model. `cancel_readiness_run` is also direct — it stops traffic rather than starting it.
