...
<Tip>
  When mirroring a host that doesn't render widgets at all, replace
  `clientCapabilities` instead of spreading — a spread leaks the SDK-default
  MCP UI extension back in and misrepresents the host as UI-capable.
</Tip>
