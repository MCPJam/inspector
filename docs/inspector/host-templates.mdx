...
<Tip>
  When mirroring a host that doesn't render widgets at all (e.g. Codex CLI or
  Cline), replace `clientCapabilities` instead of spreading — a spread leaks
  the SDK-default MCP UI extension back in and misrepresents the host as
  UI-capable. See the Codex or Cline preset for the pattern.
</Tip>
