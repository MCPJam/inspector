...
<Info>
  When using auto-generate, the AI generates realistic test scenarios from your tool definitions, including both positive cases (tools that should be called) and negative cases (tools that should NOT be called).

  If your suite has a saved server attachment with two or more servers, the generator automatically produces coverage for each server individually **and** at least one cross-server case that chains tools across both servers. Suites without a saved attachment fall back to the previous behavior (all available servers treated as a single pool).
</Info>
