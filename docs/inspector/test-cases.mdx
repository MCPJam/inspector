...
- **Cross-Host Comparison** - When two or more MCP hosts are attached to a suite, a matrix view automatically shows pass rate, p50/p95 latency, and token usage per test case per host
...
4. **Configure models** - Open a test case and set the model(s) directly on the case. For a compare run across multiple models, add them to the case's model list and click **Run compare**
5. **Run Tests** - Click Run to execute a single-model test, or Run compare to run across all models configured on the case
...
- **Suite accuracy** - Large accuracy percentage for the latest run, with a trend delta vs the previous run and a sparkline of recent runs
