...
Use the **Runs / Cases** segmented control to switch views:
...
- **By case / By host toggle** - When your suite has two or more host attachments, a toggle appears that lets you switch between the per-case list and a cross-host matrix showing how each test case performs across different hosts

### Comparing runs

You can compare any two completed runs side by side:

1. In the Runs view, select the checkboxes on two completed runs
2. Click **Compare** in the action bar that appears
3. The diff view opens, showing how pass/fail results changed between the two runs

You can also compare a run against the previous completed run directly from the run detail view.
...
- **Run Summary Sidebar** - Click "View run summary" to see:
  ...
  - Performance by Model (only shown when more than one model was used)
