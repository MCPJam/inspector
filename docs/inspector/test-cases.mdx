...
- **Expanded Details** - Click an iteration to see:
  ...
  - **Predicate Gate** - When `successPredicates` are configured on a test case, a Predicate Gate section lists each predicate with its PASS/FAIL verdict, a one-line summary of what was asserted (e.g. `responseContains needle "refund issued"`, `tokens < 500`), and the evaluator's reason string explaining why the gate decided as it did. The section is hidden when no predicates are configured.
...
- Predicate Gate verdicts (when predicates are configured — each row shows PASS/FAIL and the evaluator's reason string)
