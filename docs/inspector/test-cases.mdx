...
When you run a suite against multiple clients at once, the runs list groups them into a single collapsible **run group** row. The parent row shows the mean accuracy and longest duration across all hosts. Click the chevron to expand and inspect each host's individual run. Single-host and legacy runs render as flat rows unchanged.
