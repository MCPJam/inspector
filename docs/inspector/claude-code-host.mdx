...
Each harness honors a different subset of the host config. Read the table for the harness you selected before you tune anything on the host page — a setting listed as not honored is disabled in the UI rather than silently dropped.
...
Read this before you treat a Codex trace as evidence about a connector — what the model received is not what a native MCP client would have received.
