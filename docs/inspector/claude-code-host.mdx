...
**Setting up for the first time.** Type a prompt and press Send. A dialog
appears — *Run Claude Code in ~/code/your-project?* — with one decision to make:

- **Install & allow** (cold path) — the runtime is not yet on this machine.
  MCPJam starts the ~200 MB download in the background, closes the dialog, and
  leaves your draft in the composer. A progress notice appears below the input.
  When setup finishes, press Send again to run your turn. Nothing fires
  automatically after a download.
- **Allow** (warm path) — the runtime is already installed and verified. MCPJam
  mints the authorization and continues your original send immediately.

The dialog shows the folder the agent will start in, and a **Details** section
with the runtime version, digest, permission profile, and expiry. The
authorization is scoped to this project and this machine, and expires after
12 hours. Changing the runtime, the folder, or the permission profile means
authorizing again — the old authorization does not carry over, by design.

**Choosing a folder.** In the desktop app the dialog opens your OS folder
picker. In `npx`, type the path directly into the dialog's folder field. The
folder is where the agent starts, not a boundary it cannot leave. Filesystem
roots (`/`, drive roots) are refused.

**The execution target chip.** A small chip in the composer toolbar shows where
the current turn will run — *This machine* or *Cloud*. On a machine with only
one option it is an indicator; on a machine where both are available it is a
picker.
...
**When it is not offered.** Your platform is not supported, the runtime is not
installed, you are signed in as a guest, the session is a shared scenario or
swarm run, or the feature is not enabled for your account. The chip and the
dialog both say which.
...
| Condition | What you see |
|---|---|
| The local runtime is not installed | The trust dialog opens with **Install & allow**. Setup starts on approval; the draft is preserved; press Send again when the progress notice clears. |
...
| Your platform cannot prove whole-tree cleanup | Local execution is not offered, and the chip and dialog both say why: MCPJam could not guarantee that stopping a session stops everything it started. |
| The authorization expired, or the runtime, folder or permissions changed | Refused — the trust dialog reopens. The old authorization does not cover the new terms. |
| Setup failed or was interrupted | A notice appears in the composer with a **Retry** button. Retry reopens the dialog rather than silently re-downloading. |
| Guest, shared scenario session, or swarm run | Local execution is not offered; the turn runs hosted. |
