...
**When it is not offered.** Your platform is not supported,
...
| Model not supported by the selected harness | Pre-flight error asking you to pick an eligible model. This covers two cases: **unsupported** (the harness's runtime cannot run this model at all — hard fail everywhere) and **unverified** (the model has not been tested against the harness's current runtime version — hard fail for evals and swarms; Playground chat runs it with a warning). The model picker disables ineligible models and shows the reason inline. |
...
| A case or environment selects a model the harness has not been verified to run at its current runtime version | Pre-flight error naming the harness and version — pick a supported model, or switch to the emulated engine. When creating or editing a suite, a warning toast lists any client × model pairs that were skipped for this reason. |
