...
## Can I Use comparison matrix

The **Can I Use** view (accessible from the Compatibility page) shows a per-field breakdown across all hosts. A few things to know about how values are displayed:

- **Apps · Styles** — A subsection under Apps shows the actual CSS custom-property value each host sends for every MCP Apps style variable (colors, fonts, radii, shadows). For hosts that resolve their tokens per theme, the cell shows separate **LIGHT** and **DARK** values when they differ, or a single value when they agree. This lets you compare `light-dark(rgba(255,255,255,1), …)` against `#fff` directly.
- **Client version** — Shown at the top of the Protocol section. Every protocol row below it describes facts captured from that specific build of the client.
- **Not supported vs. —** — A cell showing an **unsupported** chip means a probe ran and the host sent nothing for that field. An em dash (—) means no probe has been run for that host yet. The distinction matters: absence of evidence is not evidence of absence.
...
- The host to support MCP Apps rendering (Claude, ChatGPT, Cursor, Copilot, and Codex all do — Codex runs the same ChatGPT app runtime).
