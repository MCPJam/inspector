...
Projects with no skills receive no skill tools and no catalog stanza, so the model's tool-selection observations are not distorted by phantom tools.
...
When skills are available, the LLM has access to the following tools:
...
The skill catalog (names and descriptions) is inlined in the system prompt rather than exposed as a discovery tool. `loadSkill` is only advertised when the project has at least one skill — empty projects receive no skill tools.

`listSkills` is available only when a connected MCP server provides skills via the SEP-2640 extension; it lists those server-provided skills only.
