...
[Skills](https://agentskills.io/what-are-skills) are an open format that provide instructions on how to use MCP tools and complete workflows. MCPJam lets you discover skills from your filesystem or your project's Computer, upload new ones, and use them in the Playground — either automatically based on the user's prompt or deterministically with the `/` command.
...
When running locally, MCPJam scans the following directories for skills. Each subdirectory is expected to contain a `SKILL.md` file. When duplicate skill names are found, the first match in search order wins.
...
When the Computers feature is enabled, skills can also be stored on your project's **Computer** (a persistent E2B sandbox). Cloud skills are stored at `~/.claude/skills/` on the Computer, which is the same directory Claude Code scans natively inside the sandbox.

In hosted mode, the Skills tab uses cloud skills exclusively. When running locally with Computers enabled, a **Local / Cloud** toggle appears in the Skills tab header so you can switch between your local filesystem and the Computer.
...
- **Local mode**: uploaded skills are stored in `~/.mcpjam/skills/`.
- **Cloud mode**: uploaded skills are stored in `~/.claude/skills/` on your personal Computer.
...
When you have skills available, the LLM discovers and loads them on demand:

- **Local mode**: MCPJam injects a lightweight list of all skill names and descriptions into the LLM's system prompt. The LLM calls `loadSkill` to load only the instructions it needs.
- **Cloud mode**: The LLM calls `listSkills` first to discover what's available on the Computer, then `loadSkill` to load a skill's instructions. This lazy approach avoids waking the sandbox on every turn.

Once a skill is loaded, the LLM can browse and read its supporting files using `listSkillFiles` and `readSkillFile`.
...
The tools available to the LLM depend on the skills source:

**Local mode** — three tools:
...
**Cloud mode** — four tools (adds discovery):

| Tool | Description |
|------|-------------|
| `listSkills()` | Discover skills installed on the Computer |
| `loadSkill(name)` | Load the full skill instructions by name |
| `listSkillFiles(name)` | List all files in the skill directory |
| `readSkillFile(name, path)` | Read the content of a specific supporting file |
