...
## Pinning a specific skill version

By default, a pinned skill runs its current revision — "Latest" — resolved when the run starts. To hold an environment or composed stack at a known revision, add `versionPins` to the `skillSelection`:

```json
{
  "mode": "explicit",
  "skillIds": ["sk_abc", "sk_def"],
  "versionPins": [
    { "skillId": "sk_abc", "versionId": "psv_xyz" }
  ]
}
```

`sk_def` above runs Latest; `sk_abc` runs the pinned version. This is the shape accepted by environment create/update, `ensure_adhoc_environment`, and the `compose.skills` field on `run_eval_suite` / `run_eval_case`.

Before offering a version picker in your own tooling, probe the `skillVersionPins` flag from `GET /projects/{projectId}/environments/capabilities`. Older deployments return `false` and reject the field outright.
