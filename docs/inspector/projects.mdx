...
4. **Auto-connect** - The **Auto-connect** switch in the Connect tab header (on by default) connects every project server for you in the Playground and debugger tabs. It is a personal setting for your browser — anyone can flip it, and it does not change the project
...
Members get access to every server in the project with the same configuration. Per-server overrides (headers, timeout, protocol version) set from the Servers tab apply across every host in the project. Auto-connect is not shared: each member decides for their own browser whether servers connect automatically.
...
### Changing a server's destination

Two edits cause the backend to clear a server's stored credentials: changing an HTTP server's **origin**, or changing a stdio server's **command**.

**HTTP servers — origin changes**

Editing the path portion of a server's URL (for example, `/mcp` to `/mcp/v2`) keeps all saved credentials intact.

Changing the **origin** — the scheme, host, or port — clears every credential saved against that server: request headers, environment variables, the bearer token, and any OAuth access token, refresh token, and client secret. The edit form warns you before this happens and names both the old and new origin. You must check the acknowledgement box before Save becomes available.

**Stdio servers — command changes**

Changing the **command** (or its arguments) on a stdio server that has stored environment variables clears those variables. The edit form warns you before this happens and shows both the old and new invocation. You must check the acknowledgement box before Save becomes available.

**Both cases**

The warning applies to credentials that other project members may have added and that you may not be able to see. After the save, each member who had credentials saved for that server needs to re-enter them.

### Destination history

The server detail panel includes a **Destination history** section that shows every time the server's destination was changed, who made the change, and whether saved credentials were cleared as a result. This covers HTTP origin changes, stdio command swaps, and switches between transport types. It is visible to all project members regardless of plan, so anyone whose credentials were removed can see what happened and who made the edit.
...
Organization admins can set an org-wide ceiling on how resources are shared. The **Sharing** card appears on the organization overview, next to the Audit Log card.

### Maximum share access

The **Maximum share access** select controls the highest share mode any member of the organization can grant:

| Option | Effect |
|---|---|
| Anyone with the link | No ceiling — members can share with anyone who has the link (default) |
| Invited users only | Shares can only be granted to explicitly invited users |
| Project members | Shares are limited to existing project members |

Tightening the ceiling takes effect immediately. Existing share rows are not rewritten — they are enforced at access time.

### Organization members only

When the **Organization members only** toggle is on, invites can only go to people who are already members of the organization. Turning it off allows inviting anyone by email.

<Note>
Non-admins see these controls in a disabled state. Only organization admins can change the sharing policy.
</Note>
