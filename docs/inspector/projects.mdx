...
### Organization share-policy ceiling

An org admin can set a maximum share mode for the organization. When a ceiling is active, share dialogs grey out any access options that exceed it and show a note such as "Your organization limits sharing to invited users only." You cannot select a greyed-out option.

The same ceiling applies when creating a new scenario: if your current access selection is above the ceiling, it is automatically snapped down to the highest allowed option before the scenario is saved.
