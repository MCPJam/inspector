...
The **Computer** tab of the host config editor includes a **Browser profile** field. Use it to pin a specific [saved browser profile](/inspector/computer#saved-browser-profiles) to this host. When a pin is set, browser sessions started by this host load from that profile's saved state (cookies, local storage, etc.) instead of your personal default. Leave the field on **Default profile for my chats** to use whichever profile you have set as your default in the Computer tab.
