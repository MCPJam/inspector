...
**Browser** can be enabled from the **Tools** tab without attaching a Computer. When Browser is selected (or a saved browser profile is already pinned to the host), a **Browser** section appears in the Tools tab where you can optionally pin a saved browser profile. Without a pin, new chats use your selected default profile.

For local installations, the first time you use the Browser panel the inspector shows a device-consent dialog inline. Granting permission covers both commands and browser control on this machine; it does not enable the Bash tool on the host. Existing grants remain valid and do not require re-authorization.
