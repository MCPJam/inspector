...
For detailed HTTP request logging, start the inspector with the `--verbose` flag:

```bash
npx @mcpjam/inspector@latest --verbose
```

This enables detailed logging of all HTTP requests, which is helpful when troubleshooting connection issues or debugging server communication. See [Installation](/installation#start-with-verbose-logging) for more details.
