...
- **From the Okta dashboard** — click the MCPJam tile. Okta posts the SAML
  assertion to WorkOS, which redirects the browser to MCPJam to complete
  sign-in automatically (IdP-initiated SSO).
- **From the MCPJam sign-in page** — enter a work email address and MCPJam
  redirects to Okta (SP-initiated SSO).
