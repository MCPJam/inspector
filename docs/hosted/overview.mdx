...
**Shared free allowance temporarily unavailable** — MCPJam maintains a platform-wide shared free budget. If that budget is exhausted or paused, the credit balance card shows "Free allowance temporarily unavailable" and the model picker tab is relabeled "MCPJam models". A notice also appears above the chat input and as a toast. During this time, new requests automatically draw from your organization's purchased credits if you have any. Adding your own API key under **Settings → LLM Providers** lets you continue chatting without using the shared allowance or your credits.
