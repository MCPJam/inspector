...
When you connect an OAuth server in the hosted app, your OAuth credentials are stored securely in Vault. You can inspect those tokens on demand from the server's **Auth** tab without them ever being written to `localStorage` or shared with other users.
...
When you connect to such a server, the app shows a warning: "This server's sign-in service is on a private network. MCPJam's hosted web app can't renew this connection automatically." You can continue using the server until the access token expires, but you'll need to sign in again when the connection expires. For fully-local setups, run `npx @mcpjam/inspector@latest` on your computer or use the MCPJam desktop app.

### No resource template browsing

Browsing resource templates is not available in the hosted app. To use resource templates, run `npx @mcpjam/inspector@latest` on your computer or use the MCPJam desktop app.

### No elicitation responses

Responding to server input requests (elicitation) is not available in the hosted app. To use elicitation, run `npx @mcpjam/inspector@latest` on your computer or use the MCPJam desktop app.
...
<Note>
  **Free plan organizations cannot purchase credits directly.** If your organization is on the Free plan, the Top up dialog shows a message that only an organization owner can upgrade the plan, with a **Compare plans** button. Clicking it closes the dialog and opens your organization's Plans settings so you can upgrade.
</Note>

<Note>
  **Desktop app:** When you confirm a top-up in the desktop app, checkout opens in your default system browser and the dialog closes automatically. Your credit balance updates in the app once the purchase completes — no redirect back into the app is required.
</Note>
