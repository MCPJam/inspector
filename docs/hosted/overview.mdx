...
- **Guests** get a free daily usage allowance and can use a subset of the free hosted models. The sidebar shows a progress bar with how much of your daily limit you've used and when it resets. When you reach the limit, you'll be prompted to sign in. Guests can build and use scenarios privately, but making a scenario publicly accessible (sharing with anyone or rotating a share link) requires an account — a **Sign up to share** prompt appears when you try.
