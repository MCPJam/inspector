...
The credit balance card shows your current balance and, when applicable, additional line items:

- **Outstanding credit debt** — credits owed from usage that exceeded your balance. Shown only when the amount is positive.
- **Carried credits (included in monthly balance)** — rollover credits from a previous period that are counted inside your monthly allowance. Shown only when the amount is positive.

**Free plan:** If your organization is on the Free plan and is not eligible to purchase credits, the Buy credits and Auto-reload sections are replaced by an **Upgrade to Pro to buy credits** link. Locked wallets (under payment review) show a spending-paused notice instead of the upgrade prompt.
