...
When you connect to such a server, the app shows a warning toast. You can continue using the server until the access token expires, but you'll need to re-run the OAuth flow manually each time.

For fully-local setups, use the [local inspector](/installation) instead. The local inspector refreshes tokens directly against your private authorization server and stores the result automatically, so sessions keep working after token expiry without any manual re-authentication.
