...
Turns in a User Testing scenario are billed to the scenario owner's organization, not to the tester. If the owner's organization runs out of credits while a tester is active, the tester sees a **"This test is paused"** notice explaining that the owner is out of MCPJam credits. The tester cannot lift this limit by signing in, buying credits, or upgrading — only the scenario owner can resolve it by topping up their organization's credits.
