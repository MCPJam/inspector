...
- **Guests** get a free daily usage allowance. Higher-priced models are greyed out in the model picker with a "Sign in to use this model" tooltip — sign in to unlock them. The sidebar shows a progress bar with how much of your daily limit you've used and when it resets. When you reach the limit, you'll be prompted to sign in.
...
**Exception — plugin `stdio` components with a Project Computer.** If your project has a Computer and the feature is enabled on your account, plugin `stdio` components run inside your own project computer rather than being refused. MCPJam verifies the bundle and connects to the child process as a remote server; no local install is required. See [Where plugin servers run](/inspector/plugins#where-plugin-servers-run).
