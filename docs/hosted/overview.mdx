...
The hosted app supports MCP tasks via polling. Each poll opens a fresh connection to your server, reads the task state, and disconnects — no persistent session is required. Task handles are stored in your browser's local storage and survive page reloads.

Progress notifications are not available in hosted mode because they require a live notification stream that an ephemeral connection cannot provide. Use the [local inspector](/installation) if your workflow depends on real-time progress events.
