...
const result = await manager.executeTool('local', 'search', { query: 'hello world' });
console.log(result);
```
...
```ts
const resource = await manager.readResource('local', { uri: 'file://README.md' });
```
...
manager.addNotificationHandler('local', ResourceListChangedNotificationSchema, n => {
...
manager.setElicitationHandler('local', async params => {
  ...
  return {
    content: [
      { type: 'text', text: `You asked: ${JSON.stringify(params)}` },
    ],
  };
});
