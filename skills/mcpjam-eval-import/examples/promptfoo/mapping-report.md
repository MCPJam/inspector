...
| looks up the invoice before answering | `c_looks_up_invoice` | `unresolved` | The `javascript` assertion asserts a tool call by name. `toolCalledAtLeastOnce` has those semantics, but `lookup_invoice` was never resolved against the target server's live discovery — `eval validate` does not resolve tool names. Disabled until discovery confirms the tool exists. |
