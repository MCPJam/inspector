...
/**
 * The fourth code, and the only one a LOCAL run can never produce.
 *
 *   5  ready, but the AI observations the caller asked for were not produced
 *
 * Observations exist only on the hosted path — they need a broker holding the
 * provider key, a lease to bill against and an organization to bill to — so a
 * local run has no way to reach this and no reason to.
 *
 * REACHABLE ONLY OVER A PASS, and that is the whole design. Observations are
 * non-dispositive by construction: they can never make a server not-ready. So
 * a credit refusal must not turn a passing server into a failing one for a
 * BILLING reason, and it must never mask a real verdict — a violation (`1`) or
 * an unestablished run (`3`) always wins. What it must not do either is exit
 * `0` in silence: a caller that asked for commentary and did not get it did
 * not get what it asked for.
 *
 * `4` is deliberately skipped. A cancelled or failed run is an INFRASTRUCTURE
 * condition, and the rule above is that those are `3` — none of them is a
 * statement about the target, and giving them a code of their own would invite
 * the next reader to treat one as a verdict.
 */
export const READINESS_EXIT_OBSERVATIONS_UNAVAILABLE = 5;

export function hostedReadinessExitCode(run: {
  status?: string;
  overallStatus?: string | null;
  includeLlmObservations?: boolean;
  llmObservations?: { status?: string };
}): number {
  // A run that did not reach a terminal state, or reached a non-completed one,
  // never established anything about the target. Same `3` a timeout gets, for
  // the same reason.
  if (run.status !== "completed") return 3;

  const verdict = directoryReadinessExitCode({
    status: run.overallStatus ?? undefined,
  });
  if (verdict !== 0) return verdict;

  // ABSENT counts as not-produced, not as fine. A row written by a backend
  // that predates this field carries no observation state at all, and reading
  // that silence as success would exit 0 for a caller who asked for
  // commentary and received none — the exact case this code exists for.
  if (
    run.includeLlmObservations === true &&
    run.llmObservations?.status !== "completed"
  ) {
