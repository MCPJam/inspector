...
Close the two hosted doctor reflection paths the transport-detail redaction did
not cover.
...
`error.message` and `oauth.discoveryError` — name no attempt and so pass through
only when every recorded attempt received a response; a run that recorded no
attempt at all is redacted too.
