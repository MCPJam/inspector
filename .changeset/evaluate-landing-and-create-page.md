...
- **The empty hero offers your connected servers directly** — up to four, since more wraps badly under the subtitle. Picking one prefills both the suite name and the server group, resolved by `pickServerAttachmentIdForServer`: an exact single-server group if one exists, otherwise the *smallest* group containing that server, and `null` when nothing matches so a card can never silently attach a different server's group. Those cards are an addition, not a replacement: **Create suite and Try sample suite stay on the hero** whether or not any servers are connected, demoted to ghost buttons under the cards. The quickstart has no other entry point in the product, and "has servers, has no suites" is the exact state it exists for.

Suite rows on the landing keep their **delete**, gated per suite by `canDeleteArtifact` and confirmed through the same `ConfirmationDialogs` every other eval delete uses. Deleting from the row is the only path that does not require opening the suite first — the in-suite path (Edit → settings → Delete) still exists, but it cannot help with a suite you never want to look at again.
...
**Generate sits on the Test Cases card as well as in the empty state.** Offering it only in the empty hero would take it away at the moment a suite gets its first case, which is when "generate more from live discovery" is most useful. It carries the same capability gate and disabled-reason tooltip as the hero action. The card's other button now says **Add case** — it always created one; it never opened an editor.
