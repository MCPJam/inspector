...
The input's `max` is the PLATFORM ceiling. An organization that lowered its own ceiling is enforced by the server, which refuses the save naming the field and the bound — the same refuse-don't-clamp rule the resolver follows. Pre-empting that in the form would need the effective ceiling, which suite capabilities do not carry yet.
