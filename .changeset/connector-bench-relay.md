...
Only an address the deployment can **vouch for** gets a budget of its own — `cf-connecting-ip` on the hosted edge, or whatever header an operator names in the new `MCPJAM_TRUSTED_CLIENT_IP_HEADER`. Everything else shares one pooled bucket at a looser ceiling. A key minted from a header the caller writes is a key the caller can rotate, and keying per value would let one actor fill the window table and then refuse every genuinely new caller until the entries expired.

`POST /preflight` is the one route that does more than relay. It asks the backend whether benchmark runs exist at all **before** it opens anyone's server — until the flag flips, a preflight costs the target nothing, and a target that happens to be down can no longer mask the real answer with its own connection failure. It then resolves a **saved** `projectId + serverId` — never a URL, never caller-supplied auth headers — dials it through the ordinary hosted authorize-and-connect path, captures the full live tool surface, and asks the backend to upsert the stable target and return a classification receipt plus the runnable categories, tracks and per-actor prefill. The snapshot is bounded in UTF-8 bytes (the target chooses how many tools it lists, not us) and drops server-controlled `_meta`, which classification does not read and which would otherwise become durable backend state. `preferences` is bounded too — key count, key length, nesting depth and serialized bytes — because it is forwarded verbatim into state the backend may persist and price from.
