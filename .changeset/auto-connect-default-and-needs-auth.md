...
- A transport failure is retried three times over about fifteen seconds before the card goes red, and the row stays on "connecting" throughout, so a server that is still booting lands green without ever having looked broken. Failures a retry cannot fix — a protocol-version pin the server does not offer, a server waiting on authorization — are not retried. `Failed (N)` finally means something; it used to always read `(0)` because nothing incremented the counter.
