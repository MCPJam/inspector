...
The second PR (#4896) made Node WebMCP a facade over the in-process browserd daemon, put inspection on the shared `BrowserShell` for tabs and navigation, and moved Electron WebMCP to a main-owned `WebContentsView` — retiring the Playwright provider and the `<webview>` entirely.

The third PR replaces the inspection frame envelope and blob-URL adapter with the daemon frame reader, which `browser-navigation-tabs.md` had named as the deliberate follow-up:

- The inspection socket speaks the daemon's own record format — it already did, byte for byte — so `createFrameWireReader` consumes it unchanged. One decoder, one framing contract, one place that bug can live. `encodeWebMcpBinaryFrame`/`decodeWebMcpBinaryFrame`, `WebMcpBinaryFrame` and `PaneFrame.src` are deleted.
- Frames leave the product store for a narrow channel (`lib/browser-pane/frame-channel.ts`) that only the viewport subscribes to. Isolation stops depending on every future panel remembering to use a selector.
- Frames leave the event hub for a one-slot channel on the runtime, so no consumer of the timeline has to filter pixels — and the SSE route loses its `frames=off` switch, its frame backpressure branch and its one-slot buffer.
- The WS → SSE-frames → screenshot-poll ladder is gone, with its badge. The client and server ship in one package and the hosted deployment never mounts the route, so the skew those rungs guarded against cannot occur. A spent retry budget now says "Live view is unavailable" and points at the Screenshot button.
- Decoded bitmaps are owned by the connection that produced them: it closes the one each new frame replaces, and the last one on teardown.

Still open in the roadmap: extract the Local/Hosted/inspection stream lifecycle into `useFrameStream`; fold the three server frame-socket routes into one factory with a single negotiation message (`hello` vs `capabilities`); give each viewer its own diagnostics instance rather than a module singleton; move hosted inspection off the VNC `BrowserPanel` onto `HostedBrowserBody`; complete the full UI as-shipped comparison and physical-trackpad/platform matrix.
