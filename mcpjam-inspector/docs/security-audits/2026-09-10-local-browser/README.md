...
- [ ] Configure local rollout to require request property `local_browser_security_version = "1"`. The local server, the local frontend and the Convex control plane all send this property; do not persist it as a person profile attribute or older clients could inherit eligibility. The control plane sends a constant because it reads the same flag for the location-neutral **authoring** gate (`lib/featureGates.ts`, `browser`, `onFlagUnavailable: 'deny'`) and knows nothing about the version of the client that will execute; without it that gate cannot evaluate the condition and refuses every local-only actor. Execution admission stays version-accurate because it is decided separately by the local server's own rollout check. Verify `MCPJam/mcpjam-backend` carries that forwarding before adding the condition.
