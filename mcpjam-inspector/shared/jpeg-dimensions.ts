...
export function readJpegDimensions(
  ...
  while (offset + 1 < bytes.length) {
    ...
    if (isStartOfFrame(marker)) {
      ...
      // The declared length has to COVER that payload, not merely be followed
      // by bytes that look like it: a segment declaring less is malformed, and
      // reading past its end would take the next segment's bytes for a picture
      // size — which the caller would then trust over its own fallback. Eight
      // is the minimum a real SOF declares (2 length + 1 + 2 + 2 + 1).
      if (length < 8 || offset + 8 >= bytes.length) return undefined;
