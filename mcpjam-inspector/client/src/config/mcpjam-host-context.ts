 ...
 *
 * Values are mirrored verbatim from `design-system/src/tokens.css` (the
 * `@mcpjam/design-system` package consumed by `client/src/index.css`).
 * The mapping keeps `getChatboxShellStyle("mcpjam", theme)` producing the
 * SAME `--background` / `--card` / `--primary` / etc. that the design
 * system ships, so when a user previews an MCPJam-styled host the
 * Servers/Host chrome cascade matches prod — no "Claude white" leak.
 *
 * Translation table (`getChatboxShellStyle` consumer → MCP Apps key):
 *   --background          ←  resolveChatBackground(theme)  (not a variable)
 *   --foreground          ←  --color-text-primary
 *   --card / --popover    ←  --color-background-primary
 *   --secondary / --muted ←  --color-background-secondary
 ...
 *   --primary (claude     ←  --color-border-warning
 *     family)
 *
 * MCPJam's brand orange is the design-system `--primary` / `--ring`;
 * it's piped through `--color-border-warning` here so the chatbox shell
 * picks it up as `--primary` (the claude-family branch in
...
export function getMcpJamStyleVariables(
  ...
  return { ...resolved, ...MCPJAM_STATIC_VARS } as McpUiStyles;
}
