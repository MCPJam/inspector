...
import type {
  ...
  PlatformUnifiedFindingsExperiment,
...
// ── unified-findings experiment (additive, default-off) ─────────────────────
...
export type UnifiedFindingsExperiment = PlatformUnifiedFindingsExperiment;
...
/**
 * The experiment payload, or `null` when the backend does not serve it.
 *
 * A client on the experiment branch talking to a backend that is NOT must
 * keep working and must SAY the pairing is incomplete — never retry a missing
 * function, and never imply the experiment ran.
 */
export function unifiedFindingsOf(
  envelope: InsightsEnvelope | null | undefined,
): UnifiedFindingsExperiment | null {
  const payload = envelope?.unifiedFindings;
  return payload?.capability === "unified_findings_v1" ? payload : null;
}

/** Provenance for one finding, by the candidate id it was built under. */
export function provenanceFor(
  experiment: UnifiedFindingsExperiment | null,
  findingId: string,
): InsightsFindingProvenance | null {
  return (
    experiment?.snapshot?.provenance.find((p) => p.candidateId === findingId) ??
    null
  );
}
