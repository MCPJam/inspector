/**
 * Bridge integration with the REAL scenarios group (no groups mock): the sixth
...
const SCENARIO_TOOL_NAMES = [
  "ui_publish_scenario",
  "ui_delete_scenario",
];

describe("scenarios group through useSurfaceAgentBridge", () => {
  beforeEach(() => {
  ...
  it("listSurfaceGroupToolNames knows the scenarios group", () => {
    expect(listSurfaceGroupToolNames("scenarios")).toEqual(SCENARIO_TOOL_NAMES);
  });
  ...
  it("mount registers both tools surface-scoped; unmount removes them", () => {
    ...
    for (const name of SCENARIO_TOOL_NAMES) {
    ...
    expect(hasSurfaceSnapshotProvider("scenarios")).toBe(true);

    unmount();
    for (const name of SCENARIO_TOOL_NAMES) {
      expect(useUiToolsRegistry.getState().resolve(name), name).toBeNull();
    }
    expect(hasSurfaceSnapshotProvider("scenarios")).toBe(false);
  });
  ...
  it("waitForUiToolNames resolves once the just-mounted group is live (same-turn discovery)", async () => {
    ...
    // ordering when the same turn navigates to the Scenarios screen and then
    // waits for the group's tools.
    const pending = waitForUiToolNames(
      listSurfaceGroupToolNames("scenarios"),
      1_500,
    );

    const { unmount } = renderHook(() =>
      useSurfaceAgentBridge({ surfaceId: "scenarios" }),
    );
    ...
    unmount();
    await expect(
      waitForUiToolNames(listSurfaceGroupToolNames("scenarios"), 50),
    ).resolves.toBe(false);
  });
});
