...
describe("classifyChatboxAccessError", () => {
  it("classifies the stale and denied codes off a top-level `code`", () => {
    ...
    expect(
      classifyChatboxAccessError(403, {
  ...
  it("prefers the domain code in `details.code` over the transport code", () => {
    ...
    const info = classifyChatboxAccessError(409, {
      code: "CONFLICT",
      details: { code: "CHATBOX_ACCESS_STALE" },
      message: "stale",
    });
    expect(info).toMatchObject({ kind: "stale", code: "CHATBOX_ACCESS_STALE" });
  });
  ...
  it("reads the message from `error` when the envelope has no `message`", () => {
    ...
    expect(
      classifyChatboxAccessError(403, {
        error: "Couldn't load this chatbox's settings, so the turn was stopped.",
        code: "CHATBOX_ACCESS_DENIED",
      })?.message
    ).toContain("Couldn't load this chatbox's settings");
  });
  ...
  it("falls back to the legacy 403 INTERNAL_ERROR fail-closed envelope", () => {
    ...
    expect(
      classifyChatboxAccessError(403, {
        code: "INTERNAL_ERROR",
        message:
          "Couldn't load this chatbox's settings, so the turn was stopped to avoid running with the wrong configuration. Chatbox not found or access denied",
      })?.kind
    ).toBe("denied");
  });
  ...
  it("does NOT claim a bare 403 — an OAuth insufficient_scope challenge is not an access verdict", () => {
    ...
    expect(
      classifyChatboxAccessError(500, {
        code: "INTERNAL_ERROR",
        message: "Couldn't load this chatbox's settings, so the turn ...",
      })
    ).toBeNull();
  ...
  it("returns null for non-object bodies", () => {
    expect(classifyChatboxAccessError(403, null)).toBeNull();
    expect(classifyChatboxAccessError(403, "denied")).toBeNull();
    expect(classifyChatboxAccessError(403, undefined)).toBeNull();
  });
});

describe("classifyChatboxAccessResponse", () => {
  it("classifies without consuming the response body", async () => {
    const response = new Response(
      JSON.stringify({ code: "CHATBOX_ACCESS_STALE", message: "stale" }),
      { status: 409, headers: { "Content-Type": "application/json" } }
    );

    expect((await classifyChatboxAccessResponse(response))?.kind).toBe("stale");
  ...
  it("returns null for a non-JSON body instead of throwing", async () => {
    ...
    await expect(classifyChatboxAccessResponse(response)).resolves.toBeNull();
  });
});
...
describe("patchBodyAccessVersion", () => {
  it("replaces accessVersion and preserves every other field", () => {
    ...
    expect(patched.chatboxId).toBe("cbx_1");
