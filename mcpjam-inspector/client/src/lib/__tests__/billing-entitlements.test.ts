...
describe("getBillingErrorMessage", () => {
  ...
  it("formats billing_feature_not_included using the current plan name", () => {
    ...
    expect(message).toBe(
      "Chatboxes is not included in the Free plan. Upgrade to Pro to continue.",
    );
  });
  ...
  it("formats billing_feature_not_included for non-billing admins with explicit upgrade guidance", () => {
    ...
    expect(message).toBe(
      "Chatboxes is not included in the Free plan. Ask an organization owner to upgrade to Pro.",
    );
  });
...
describe("getDisplayPriceCentsForPlan", () => {
  it("returns catalog cents for Pro", () => {
    const drifted = minimalCatalogEntry({
      monthly: 6100,
      annual: 29000,
    });
    expect(getDisplayPriceCentsForPlan("pro", "annual", drifted)).toBe(
      29000,
    );
    expect(getDisplayPriceCentsForPlan("pro", "monthly", drifted)).toBe(6100);
  });
