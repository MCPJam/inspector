...
describe("getBillingErrorMessage", () => {
  ...
  it.each([
    new ConvexError("Private backend validation detail"),
    new ConvexError({ message: "Private backend validation detail" }),
    new Error(JSON.stringify({ message: "Private backend validation detail" })),
    "Private backend validation detail",
    new Error("[Request ID: abc] Private backend detail"),
    null,
    new Error(""),
  ])("uses catalog fallback for non-billing failures", (error) => {
    expect(getBillingErrorMessage(error, ERROR_MESSAGES.failedToCreateSuite))
      .toBe(ERROR_MESSAGES.failedToCreateSuite);
  });
  ...
  it("does not expose an unrecognized ConvexError payload", () => {
