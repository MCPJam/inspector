...
function getEntry(planCatalog: PlanCatalog, plan: OrganizationPlan): PlanCatalogEntry {
  return planCatalog.plans[plan];
}

function formatSeatLimit(plan: OrganizationPlan, entry: PlanCatalogEntry): ComparePlanCell {
  if (plan === "enterprise") {
  ...
  const value =
    plan === "starter" ? entry.includedSeats ?? entry.limits.maxMembers : entry.limits.maxMembers;
  if (value == null) {
...
function formatLimitValue(value: number | null, emphasize?: boolean): ComparePlanCell {
  if (value == null) {
...
function formatEvalRuns(plan: OrganizationPlan, entry: PlanCatalogEntry): ComparePlanCell {
  if (plan === "enterprise") {
    return t("Custom", true);
  }
...
function formatDeployments(plan: OrganizationPlan, entry: PlanCatalogEntry): ComparePlanCell {
  if (plan === "enterprise") {
    return t("Custom", true);
  }
      ...
      switch (row.label) {
        case "Seat limit":
          return {
            ...
            starter: formatSeatLimit("starter", getEntry(planCatalog, "starter")),
            team: formatSeatLimit("team", getEntry(planCatalog, "team")),
            enterprise: formatSeatLimit("enterprise", getEntry(planCatalog, "enterprise")),
          };
        case "Workspaces":
          return {
            ...row,
            free: formatLimitValue(getEntry(planCatalog, "free").limits.maxWorkspaces),
            starter: formatLimitValue(getEntry(planCatalog, "starter").limits.maxWorkspaces),
            team: formatLimitValue(getEntry(planCatalog, "team").limits.maxWorkspaces, true),
            enterprise: t("Custom", true),
          };
        case "Servers per workspace":
          return {
            ...row,
            free: formatLimitValue(getEntry(planCatalog, "free").limits.maxServersPerWorkspace),
        ...
        case "Evals CI/CD runs":
          return {
            ...
            starter: formatEvalRuns("starter", getEntry(planCatalog, "starter")),
        ...
        case "Deployments":
          return {
            ...
            starter: formatDeployments("starter", getEntry(planCatalog, "starter")),
