...
/**
 * Connections: the workspaces this org uses, its org-wide default project, and
 * its channel→project bindings.
 *
 * THE #1 FRICTION REMOVER. Today every member has to pick a default project in
 * the Slack App Home before the bot is useful to them; the two controls here
 * remove that step for a whole workspace (org default) or for a whole channel
 * (binding).
 *
...
export function SlackConnectionsTab({
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection title="Connected workspaces">
        {isLoading ? (
          ...
          <div className="space-y-2 px-4 py-8 text-sm text-muted-foreground">
            <p>
              No Slack workspaces yet. A workspace appears here once someone in
              this organization connects their MCPJam account from the MCPJam
              Slack app.
            </p>
          </div>
        ) : (
          workspaces.map((workspace) => (
            <div
              ...
              data-testid={`slack-workspace-${workspace.surfaceTenantId}`}
            >
                ...
                <span className="text-xs text-muted-foreground">
                  {workspace.installed
                    ...
                    : // Worth saying out loud: the org's members are using a
                      // workspace where the app is not installed, which is why
                      // the bot does not answer them.
                      "MCPJam is not installed in this workspace"}
                </span>
              </div>
      ...
      {isAdmin ? (
        <SettingsSection title="Bind a channel">
          ...
          <p className="px-4 pb-3 text-xs text-muted-foreground">
            A channel can be bound to one project. Copy the channel ID from
            Slack (channel name → About → the ID at the bottom).
          </p>
        </SettingsSection>
      ) : null}
    </div>
