...
export function RecordedToolDetails({
  trace,
  metadata,
  error,
}: {
  ...
  const spans = new Map(
    (Array.isArray(envelope.spans) ? envelope.spans : [])
      .map(record)
      .map((span) => [span.toolCallId, span]),
  );
  const completeness = metadata?.captureCompleteness;
  const recorded =
    metadata?.sdkRecorderVersion === 1 ||
    record(envelope.raw).sdkRecorderVersion === 1;
  return (
    <div className="space-y-3" aria-label="Recorded tool details">
      {error && (
      ...
      {completeness === "unavailable" ? (
        <p role="status" className="text-sm text-muted-foreground">
          Tool details could not be captured.{" "}
          {typeof metadata?.captureError === "string"
        ...
        <>
          {completeness === "incomplete" && (
            <p role="status" className="text-sm text-muted-foreground">
              Tool details are incomplete: the test ended before all tool calls
              finished.
            </p>
          )}
          {calls.length === 0 && (
            <p role="status" className="text-sm text-muted-foreground">
              {recorded
                ...
                : "Tool details unavailable for this older run."}
            </p>
          )}
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  >
                    {!result ? "Incomplete" : failed ? "Tool error" : "Success"}
                  </span>
                  {duration !== undefined && (
                    <span className="text-muted-foreground">
                ...
                <details>
                  <summary className="cursor-pointer text-sm">Inputs</summary>
                  <pre className="mt-2 overflow-auto rounded bg-muted p-3 text-xs">
                    {JSON.stringify(call.input ?? call.args ?? {}, null, 2)}
                  </pre>
                </details>
                <details>
                  ...
                  {result ? (
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      No response recorded before the test ended.
                    </p>
                  )}
                </details>
              </section>
