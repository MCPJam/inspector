...
export type CreateSuitePayload = {
  ...
  /**
   * Legacy single-host seed payload — when the suite is created without
   * attachments, the dialog still accepts a one-shot hostConfig snapshot
   * for back-compat with the old "Seed from host" flow. New host-bound
   * runs use `hostAttachments` instead.
   */
  namedHostId?: string;
  hostConfigInput?: ReturnType<typeof hostConfigDtoToInput>;
  /**
   * Multi-host fan-out. Each attachment is one host the suite can run
   * against. Runs are made one-per-attachment from "Run all hosts".
   */
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  out into its own run when you click "Run all hosts". Leave
                  empty to manage the suite's servers directly below.
                </p>
              </div>
              <HostAttachmentsEditor
                projectId={projectId}
