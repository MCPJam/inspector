...
/**
 * The Required / Advisory control, in one place.
 *
 ...
 * rather than a hard-coded "Required". A suite file or the CLI can author
 * `role: "advisory"` today; rendering that as "Required" tells a reader the
 * check will fail their iteration when it cannot. Not being able to EDIT a
      ...
      <TooltipTrigger asChild>
        <span
          ...
          className={cn(
            ...
            // Required reads as ordinary foreground because it is the default
            // and the consequential one; Advisory is muted because "shown,
            // changes nothing" is exactly what muted means.
            role === "required"
              ? "text-foreground"
...
/**
 * The segmented control. `ariaLabel` names what is being roled ("Assertion
 * role", "Judge role") so two groups on one page stay distinguishable to a
 * screen reader and to a test.
 */
export function RoleSegmentGroup({
  value,
  roles = ["required", "advisory"],
  disabledRoles,
  ariaLabel,
  onChange,
}: {
