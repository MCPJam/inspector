...
import { STANDARD_ASSERTION_CHECKS, type StandardCheckDraft } from "../standard-checks-model";
import type { SuiteCapabilities } from "@/hooks/use-suite-capabilities";
afterEach(cleanup);
const check=STANDARD_ASSERTION_CHECKS.find(c=>c.id==="response.performance")!;
const capabilities={scorers:{predicateKinds:STANDARD_ASSERTION_CHECKS.map(c=>c.preset.type),suppressedSuiteStandardCheckIds:true}} as SuiteCapabilities;
it("saves a family override, reloads it off, and restores the customized inherited rule",()=>{
  const save=vi.fn();let stored: StandardCheckDraft={};
  function Editor(){
    const [draft,setDraft]=useState(stored);
    return <CaseChecksPage title="Catalog" suitePredicates={[{type:"toolLatencyUnder",ms:1234,role:"advisory"}]} {...draft} capabilities={capabilities} onChecksChange={setDraft} judgeSkipped={false} onJudgeSkippedChange={()=>{}} onSave={()=>{stored=draft;save(draft);}} saveDisabled={false}/>;
  }
...
it("keeps inherited toggles disabled until suppression is supported",()=>{
  render(<CaseChecksPage title="Catalog" suitePredicates={[check.preset]} onChecksChange={()=>{}} capabilities={{scorers:{predicateKinds:[check.preset.type]}} as SuiteCapabilities} judgeSkipped={false} onJudgeSkippedChange={()=>{}} onSave={()=>{}} saveDisabled={false}/>);
  expect(screen.getByRole("checkbox",{name:check.label})).toBeDisabled();
});
