...
const CLAUDE_LOADING_MARK_PATH =
...
export function LoadingIndicatorContent({
  ...
  if (variant === "claude-mark") {
    return (
      <span className={cn("inline-flex min-h-8 items-center", className)}>
        <span className="sr-only">Thinking</span>
        <span
          aria-hidden="true"
          data-testid="loading-indicator-claude"
          className="inline-flex h-8 w-8 items-center justify-center"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 100 100"
            className="-translate-x-px h-7 w-7 fill-current text-[#d97757]"
          >
            <path d={CLAUDE_LOADING_MARK_PATH} />
          </svg>
        </span>
      </span>
    );
