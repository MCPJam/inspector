...
import { useSandboxHostIntroGate } from "../useSandboxHostIntroGate";

describe("useSandboxHostIntroGate", () => {
  afterEach(() => {
  ...
  it("dismisses intro and hides welcome after dismissIntro", () => {
    ...
    expect(result.current.showWelcome).toBe(false);
    expect(result.current.showAuthPanel).toBe(true);
    expect(sessionStorage.getItem("sandbox-intro-dismissed-sbx_2")).toBe("1");
  });
  ...
  it("auto-persists intro dismissal when OAuth completes (not a non-OAuth first visit)", () => {
    ...
    expect(sessionStorage.getItem("sandbox-intro-dismissed-sbx_3")).toBe("1");
  });
});
