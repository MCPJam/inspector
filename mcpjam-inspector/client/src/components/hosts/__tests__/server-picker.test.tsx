...
describe("ServerPicker — the clear control beside a full-width field", () => {
  ...
  function column(props: Record<string, unknown>) {
    ...
    const { container } = render(
      // The shape that broke it: a vertical stack, not a flex row.
      <div className="space-y-2" data-testid="column">
        <ServerPicker
          projectId="p_1"
          value="att_1"
          onChange={vi.fn()}
    ...
    return container.querySelector('[data-testid="column"]') as HTMLElement;
  }

  it("keeps the clear control on the field's own line", () => {
    const box = column({ variant: "field" }).firstElementChild;

    // Both inside one element — a `contents` wrapper satisfies this too, which
    // is why the line below is the one that matters.
    expect(box).toContainElement(screen.getByTestId("picker"));
    expect(box).toContainElement(screen.getByTestId("server-picker-clear"));
    // And that element is a real row. `display: contents` would put the X back
    // on the column's own line, so asserting containment alone passes over the
    // bug; jsdom has no layout engine, so the classes are the only proxy.
    expect(box).toHaveClass("flex", "w-full", "items-center");
    expect(box).not.toHaveClass("contents");
  });

  it("adds no layout box around the pill", () => {
    // Every caller already puts the pill in its own flex row, so a wrapper
    // would change their spacing. `display: contents` is how the element stays
    // in the DOM without being one.
    const box = column({}).firstElementChild;
    expect(box).toHaveClass("contents");
