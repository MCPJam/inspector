...
describe("PROOF: a freshly mounted card replays the queued turn immediately", () => {
  /**
   * The card owns its OWN `useChatSession`, so it owns its own async auth
   * bootstrap. This effect fires `sendMessage` the moment a stored
   * `broadcastRequest` exists — including on the very first render of a card
   * that just mounted (a host switch remounts them). Nothing here waits for
   * `isSessionBootstrapComplete`, so the turn can leave during the window the
   * sibling proof shows carries no Authorization header.
   */
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("sends on its FIRST render — no bootstrap gate", async () => {
    renderWithBroadcast(request);
    await waitFor(() =>
