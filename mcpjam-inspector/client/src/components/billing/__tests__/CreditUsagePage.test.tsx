...
it("shows recorded daily and feature totals and scopes period changes", async () => {
  query.mockReturnValue({
    totalCredits: 12,
    daily: [{ date: "2026-09-12", credits: 12 }],
    features: [{ name: "Model usage", credits: 12 }],
    truncated: false,
  });
  ...
  expect(query).toHaveBeenLastCalledWith(
    "billing/creditUsage:getOrganizationCreditUsage",
    { organizationId: "org-1", days: 7 },
  );
});
it("distinguishes loading from empty usage", () => {
  ...
  expect(
    screen.getByText("No credit consumption in this period."),
  ).toBeInTheDocument();
});
