 ...
 * abandoned visit left an orphaned host and its scenario in the project — and
...
const renderFlow = (
  onCreateScenario = vi.fn().mockResolvedValue({ scenarioId: "cb1" }),
) => {
...
describe("UserTestingCreateFlow", () => {
  ...
  it("saves the name, the attached server and the access mode in one call", async () => {
    ...
    expect(onCreateScenario).toHaveBeenCalledWith({
      ...
      scenarioMode: "invited_only",
    });
  });

  it("creates one scenario on a double-click, not two", async () => {
    let resolveSave: (v: { scenarioId: string }) => void = () => {};
    const onCreateScenario = vi.fn(
      () => new Promise<{ scenarioId: string }>((r) => (resolveSave = r)),
    );
