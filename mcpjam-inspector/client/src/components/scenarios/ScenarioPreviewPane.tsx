...
/**
 * The live published scenario, embedded, so a scenario can be spot-checked
 ...
 *    inspector into the tester's scenario.
 *
 ...
 *    `IframeRouterError`. We detect the frame leaving the scenario path and
 ...
 */
export function ScenarioPreviewPane({
  publishLink,
  mcpProfile,
  remountKey = "",
  emptyTitle = "No share link yet",
  emptyBody = "Publish this study to get a share link, then come back here to preview it.",
}: {
  ...
  if (!src) {
    return (
      <PreviewEmptyState
        ...
        body="This build serves share links from another origin, so the study can't be embedded. Open it in a new tab instead."
        link={publishLink}
      />
    );
  }
...
function PreviewFrame({
  ...
  // as much "no longer the scenario".
  const handleLoad = () => {
    try {
  ...
  if (navigatedAway) {
    return (
      <PreviewEmptyState
        ...
        body="The preview navigated away from the study. An OAuth sign-in does this. Open it in a real browser tab to complete the flow."
        link={link}
...
/**
 * Tag the embedded run as `preview` traffic. The scenario runtime reads
 * `?surface=` on bootstrap (`readScenarioSurfaceFromUrl`) and carries it onto
