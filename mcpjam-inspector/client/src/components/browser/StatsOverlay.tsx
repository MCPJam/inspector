 ...
 */
export function StatsOverlay({ engine }: { engine: string }) {
  ...
  return (
    <div
      ...
      className="pointer-events-none absolute left-2 top-2 z-10 rounded-md bg-black/70 px-2 py-1.5 font-mono text-[10px] leading-4 text-white"
    >
      <div>
        {live.width || "–"}×{live.height || "–"} @ {live.fps} fps ·{" "}
        {live.kbps} kbps
      </div>
      <div>
        rtt {live.rtt ?? "–"} ms · frame→paint {live.captureToPaintP50 ?? "–"} ms
      </div>
      <div>
...
function daemonDrops(
  ...
  return (
    (dropped.dedupe ?? 0) + (dropped.oversize ?? 0) + (dropped.pacer ?? 0)
  );
}
