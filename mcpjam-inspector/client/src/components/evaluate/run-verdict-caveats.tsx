...
export function RunVerdictCaveats({
  ...
  if (summary.counts?.measurementUnit === "trial") {
    // The distinction the old card shouted and this one states: when the unit
    // is the execution, "2 of 3" is not a count of case variants.
    lines.push(
      `Counts are ${measurementUnitLabel("trial", 0)}, not ${measurementUnitLabel(
        "caseVariant",
        0,
      )}. Each execution is tallied on its own, so a case that ran twice is counted twice.`,
    );
