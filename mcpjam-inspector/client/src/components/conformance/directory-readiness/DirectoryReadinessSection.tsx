                  ...
                  lanes={
                    state.report
                      ? state.report.lanes.map((lane) => ({
                          // Spread FIRST, then the two fields that must win.
                          // `coverage` carries its own `lane`, so the previous
                          // order let the spread silently overwrite the
                          // explicit one — same value today, but the ordering
                          // said the opposite of what it meant, and `status`
                          // lives on the lane rather than on its coverage.
                          ...lane.coverage,
                          lane: lane.lane,
                          status: lane.status,
                        }))
