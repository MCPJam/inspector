...
if (checkOnly) {
  let stamped;
  try {
    stamped = /MCPJAM_BROWSERD_SOURCE_HASH = "([a-f0-9]+)"/.exec(
  ...
  if (stamped === sourceHash) {
    console.log("browserd bundle: up to date");
    process.exit(0);
  }
  console.error(
    "browserd bundle: the checked-in daemon bundle is STALE.\n" +
      `  sources hash to ${sourceHash}\n` +
      `  dist/ was generated from ${stamped ?? "(no hash found)"}\n\n` +
      "The bundle is checked in because the sandbox runs it with no build\n" +
      "step, so a daemon edit that is not re-bundled ships the OLD daemon.\n\n" +
      "Fix: npm run bundle:browserd  (then commit both files in\n" +
      "     server/services/browserd/dist/)",
  );
  process.exit(1);
}
