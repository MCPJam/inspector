...
describe("ChromiumDriver — what an act changed", () => {
  /** A page whose tree gains a line the moment anything is clicked. */
  function openingPage(init: { extra?: boolean } = {}) {
    let opened = init.extra === true;
    const page = fakePage({
      url: "https://x.test/",
      cdpReplies: {
        "Accessibility.getFullAXTree": () =>
          axTree({
            role: "RootWebArea",
            children: [
              { role: "button", name: "Menu", id: 41 },
              ...(opened
                ? [{ role: "menuitem" as const, name: "Settings", id: 42 }]
                : []),
            ],
          }),
        "DOM.getBoxModel": { model: { content: [80, 40, 120, 40, 120, 60, 80, 60] } },
        "DOM.resolveNode": { object: { objectId: "obj-1" } },
      },
    });
    return { page, open: () => (opened = true) };
  }

  async function drive(features: Record<string, unknown>) {
    const { page, open } = openingPage();
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, { features: features as never });
    await driver.execute(cmd({ kind: "navigate", url: "https://x.test/" }));
    await driver.execute(cmd({ kind: "observe", mode: "a11y" }));
    // The click is what opens the menu, so the post-act tree differs.
    page.onAct = open;
    const acted = await driver.execute(
      cmd({ kind: "act", verb: "click", target: { a11yRef: "e1" }, observe: "a11y" }),
    );
    return { acted, driver, page };
  }

  it("reports the lines an act added, beside the full tree", async () => {
    // Every act already returns the whole tree, which on a real page is
    // hundreds of lines of which one or two moved.
    const { acted } = await drive({ changedA11y: true });
    const output = acted.output as {
      a11y?: string;
      changed?: { added: string[]; removed: string[] };
    };
    expect(output.changed?.added).toEqual([
      expect.stringContaining("Settings"),
    ]);
    expect(output.changed?.removed).toEqual([]);
    // BESIDE, not instead: a diff is only meaningful against a tree the reader
    // already has.
    expect(output.a11y).toContain("Settings");
    expect(output.a11y).toContain("Menu");
  });

  it("reports NOTHING with the flag off", async () => {
    const { acted } = await drive({});
    expect(acted.output).not.toHaveProperty("changed");
  });

  it("omits the section when the tree did not change", async () => {
    // A `changed: {added:[],removed:[]}` on every act is a section the model
    // reads and learns nothing from.
    const page = fakePage({
      url: "https://x.test/",
      cdpReplies: {
        "Accessibility.getFullAXTree": axTree({
          role: "RootWebArea",
          children: [{ role: "button", name: "Menu", id: 41 }],
        }),
        "DOM.getBoxModel": { model: { content: [80, 40, 120, 40, 120, 60, 80, 60] } },
        "DOM.resolveNode": { object: { objectId: "obj-1" } },
      },
  ...
  it("reports nothing on the FIRST act of a turn", async () => {
    // There is no previous render to diff against, and "everything appeared"
    // is not a change.
    ...
    await driver.execute(cmd({ kind: "navigate", url: "https://x.test/" }));
    const acted = await driver.execute(
      cmd({ kind: "act", verb: "click", target: { coordinates: [5, 5] }, observe: "a11y" }),
    );
    expect(acted.output).not.toHaveProperty("changed");
  });

  it("reports nothing after a NAVIGATION", async () => {
    // Every line differs across a document, and "everything changed" is noise.
    const { page } = openingPage({ extra: true });
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { changedA11y: true },
    });
  ...
  it("does not diff a page-scoped tree against a SCOPED one", async () => {
    // A `rootRef` render describes a subtree; diffing a whole page against one
    // would report the rest of the page as removed.
    const { page, open } = openingPage();
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { changedA11y: true },
...
describe("ChromiumDriver — the screenshot byte budget", () => {
  /** A page whose capture size depends on the quality it is asked for. */
  function sizedPage(sizeFor: (quality: number | undefined) => number) {
    const asked: Array<number | undefined> = [];
    const page = fakePage({ url: "https://x.test/" });
    page.screenshotBase64 = (async (options?: { quality?: number }) => {
      asked.push(options?.quality);
      return "x".repeat(sizeFor(options?.quality));
    }) as never;
    return { page, asked };
  }

  it("captures ONCE at today's quality when no budget is set", async () => {
    // The compatibility guarantee: no budget means no measurement, no
    // step-down, and the same single call every capture has always made.
    const { page, asked } = sizedPage(() => 10_000);
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context);
    await driver.execute(cmd({ kind: "navigate", url: "https://x.test/" }));
    const res = await driver.execute(
    ...
    expect(res.output).not.toHaveProperty("screenshotCompressed");
  });

  it("steps quality down until it fits, and says so", async () => {
    // 70 is too big, 40 fits.
    const { page, asked } = sizedPage((quality) =>
      quality === 70 ? 5_000 : 900,
    );
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { screenshotMaxBytes: 1_000 },
    });
    await driver.execute(cmd({ kind: "navigate", url: "https://x.test/" }));
    const res = await driver.execute(
      cmd({ kind: "observe", mode: "screenshot" }),
    );
    expect(asked).toEqual([70, 40]);
    // OUTSIDE the untrusted fence, and a boolean: a model that finds a detail
    // illegible needs to know the picture was squeezed rather than assume the
    // site renders badly.
    expect(res.output).toMatchObject({ screenshotCompressed: true });
  });

  it("does NOT flag a capture that fit at the first tier", async () => {
    const { page, asked } = sizedPage(() => 100);
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { screenshotMaxBytes: 1_000 },
    });
    ...
    expect(asked).toEqual([70]);
    expect(res.output).not.toHaveProperty("screenshotCompressed");
  });

  it("KEEPS the smallest tier rather than dropping the picture", async () => {
    // An act paid for a capture. A model that asked to see the page and got
    // nothing cannot tell "too big to send" from "the page is blank", and its
    // next move is to ask again.
    const { page, asked } = sizedPage(() => 50_000);
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { screenshotMaxBytes: 1_000 },
    });
    ...
    expect(asked).toEqual([70, 40, 25, 10]);
    expect(res.output).toMatchObject({
  ...
  it("lets a command's own cap win over the daemon's", async () => {
    // A caller that knows its own context budget knows it better than the box:
    // an eval streaming to a small model and a Playground turn on the same
    // daemon want different answers.
    const { page, asked } = sizedPage((quality) =>
      quality === 70 ? 5_000 : 900,
    );
    const { context } = fakeContext({ pages: [page] });
    const driver = new ChromiumDriver(context, {
      features: { screenshotMaxBytes: 100_000 },
    });
    await driver.execute(cmd({ kind: "navigate", url: "https://x.test/" }));
    await driver.execute(
      cmd({ kind: "observe", mode: "screenshot", maxScreenshotBytes: 1_000 }),
    );
    expect(asked).toEqual([70, 40]);
  });

  it("caps an ACT's screenshot too", async () => {
    const { page, asked } = sizedPage((quality) =>
      quality === 70 ? 5_000 : 900,
    );
    const { context } = fakeContext({ pages: [page] });
