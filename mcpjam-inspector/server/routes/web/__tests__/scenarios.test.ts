...
describe("web routes — scenarios redeem", () => {
  ...
  it("preserves the sign-in code from a bearer-authenticated redemption", async () => {
  ...
  it("surfaces a deployment mismatch when the upstream scenario route is missing", async () => {
    ...
    const response = await postJson(
      app,
      "/api/web/scenarios/redeem",
      { scenarioToken: "scenario-link-token" },
      token,
    );
  ...
  it("returns the redeem payload on success", async () => {
    ...
    expect(data).toMatchObject({
      ...
      projectId: "ws_1",
      accessVersion: 3,
      bootstrap: expect.objectContaining({ name: "Host Styled Scenario" }),
    });
  });
});
