...
describe("agent op registry", () => {
  ...
  it("says HOW MANY paid runs a fan-out proposal starts", () => {
    ...
    // COMPOSE fans out to N paid runs and, when `saveTargets` is set, also
    // edits the suite. Default is ephemeral — a line that said only "Run
    // eval suite smoke" would approve a multiplier (or an attach) nobody
    // mentioned.
