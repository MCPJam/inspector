  ...
  return {
    caseUpsert: {
      committed: cases.map((item, index) => ({
        ...
        // (`convex-id-param.ts`), so a `case_1` that could never exist
...
describe("v1 eval-edit routes", () => {
  ...
  it("PATCH suite maps public settings to internal updateTestSuite args", async () => {
    const res = await request(
      "PATCH",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx",
      {
        name: "Renamed",
        settings: {
          minimumAccuracy: 75,
  ...
  it("GET reports minimumIterations, null when the suite has no floor", async () => {
    ...
    const set = await request("GET", "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx");
  ...
  it("schedule disable preserves the stored interval", async () => {
    const res = await request(
      "PATCH",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx/schedule",
      { enabled: false }
    );
    expect(res.status).toBe(200);
    const args = convexMutationMock.mock.calls.find(
  ...
  it("PATCH case merges partial match options onto the existing override", async () => {
    const res = await request(
      "PATCH",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx/cases/case1xxxxxxxxxxxxxxxxxxxxxxxxxxx",
  ...
  it("create case without models derives the provider for a bare suite default", async () => {
    ...
    const res = await request(
      "POST",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx/cases",
      {
        title: "bare",
        steps: [
  ...
  it("generate persists drafts and reports the generation model", async () => {
    ...
    const res = await request(
      "POST",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx/cases/generate",
      { mode: "normal" }
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
  ...
  it("POST /cases/batch authors every case in ONE mutation", async () => {
    const res = await request(
      "POST",
      "/api/v1/projects/p1/eval-suites/suite1xxxxxxxxxxxxxxxxxxxxxxxxxx/cases/batch",
      {
        cases: [
