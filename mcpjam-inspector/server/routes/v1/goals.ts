/**
 * Public v1 goal surface — the API for what the product calls **Swarms**.
 *
 * NAMING. "Swarm" is not a resource noun here. A swarm is a *container* users
 * author in the UI; what actually executes is a GOAL (a persona pursuing a
 * task against one or more environments) and what it produces is a GOAL
 ...
 * is exposed as `swarmRunId`, which says what it is rather than what table it
 * came from.
 *
 * TWO SURFACES, ONE SET OF HANDLERS. `/goals` and `/goal-runs` are the public
 * paths. `/journeys` and `/journey-runs` are the pre-rename paths, kept
 * working for callers holding references to them: same handler, same
 * authorization, but their responses keep the old field spellings
 * (`journeyId`, `sessionsPerTarget`, `waveId`) and carry `Deprecation: true`.
 * Deleted at GA. `Surface` below is what tells the two apart; nothing else in
 * this file branches on the caller's path.
 *
 ...
 * enforced inside Convex. The Convex names are storage's and do not move.
 *
 * AUTHORING (`POST`/`PATCH`/`DELETE` on goals) lives here alongside the
 * launch it feeds. Before it existed the API could run a goal and read its
 ...
 * /projects/A/goals/{a-goal-in-B}/runs` would happily serve project B's
...
/**
 * Which spelling of the surface the request arrived on.
 *
 * The canonical paths answer with `goalId`, `iterations` and `swarmRunId`; the
 * deprecated `/journeys` paths answer with `journeyId`, `sessionsPerTarget`
 * and `waveId`, because a caller parsing for those is exactly who the alias
 * exists for. The authorization, the Convex calls and the scoping preflights
 * are identical — only the wire vocabulary differs.
 */
type Surface = { legacy: boolean };
const CANONICAL: Surface = { legacy: false };
const LEGACY: Surface = { legacy: true };

/** Register one route on both spellings, deprecating the old one. */
function both(
...
/** The route parameter the surface addresses a goal by. */
function goalIdParam(c: Context, surface: Surface): string {
  return c.req.param(surface.legacy ? "journeyId" : "goalId");
}
...
function toGoalDto(row: JourneyRow, surface: Surface) {
  return {
    id: row._id,
    ...
    // The swarm CONTAINER this goal was authored under, if any. Exposed as
    ...
    ...(row.serverAttachmentId !== null
      ...
      : {}),
    [surface.legacy ? "sessionsPerTarget" : "iterations"]:
      row.config?.sessionsPerTarget ?? null,
...
function toGoalRunDto(row: JourneyRunRow, surface: Surface) {
  return {
    id: row._id,
    projectId: row.projectId,
    [surface.legacy ? "journeyId" : "goalId"]: row.journeyRefId,
    ...
    ...(row.swarmRunGroupId !== undefined
      ? { [surface.legacy ? "waveId" : "swarmRunId"]: row.swarmRunGroupId }
      : {}),
    status: row.status,
...
function toGoalSessionDto(
  ...
  return {
     ...
     * anyone who needs to correlate a goal session with a live conversation.
     */
    chatSessionId: row.chatSessionId,
    projectId: row.projectId,
    ...(row.hostId !== undefined ? { hostId: row.hostId } : {}),
    ...(row.journeyRunId !== undefined ? { runId: row.journeyRunId } : {}),
    ...(row.journeyRefId !== undefined
      ? { [surface.legacy ? "journeyId" : "goalId"]: row.journeyRefId }
      : {}),
...
// the loose `iterations`/`maxTurns` pair → the nested `config` object.
...
//
// Each surface takes only its OWN spelling of the per-target count: a body
// carrying `iterations` on `/journeys`, or `sessionsPerTarget` on `/goals`,
// is a caller mixing two vocabularies and is refused by `strictObject`. That
// is deliberate — an alias exists so old code keeps working unchanged, not so
// new field names leak onto it.

/** Cap on assistant turns per session. Same on both surfaces. */
const maxTurnsField = z.number().int().min(1).max(200);
/**
 * Sessions run against EACH target. Total sessions = targets x this, which
 * is what actually spends — a caller raising this from 1 to 10 on a goal
 * with 4 environments has asked for 40 conversations.
 */
const iterationsField = z.number().int().min(1).max(100);

function createSchemaFor(surface: Surface) {
  return z.strictObject({
    ...
    [surface.legacy ? "sessionsPerTarget" : "iterations"]: iterationsField,
    maxTurns: maxTurnsField,
    setupWrites: z.boolean().optional(),
  });
}

/** The per-target count off a parsed body, whichever surface parsed it. */
function iterationsOf(body: {
  iterations?: number;
  sessionsPerTarget?: number;
}): number | undefined {
  return body.iterations ?? body.sessionsPerTarget;
}

const CREATE_SCHEMAS = {
  canonical: createSchemaFor(CANONICAL),
  legacy: createSchemaFor(LEGACY),
};
 ...
 * other way to say "stop fanning this goal out across environments and go
 * back to its host targets". Convex models the same tri-state upstream, so
 * this is a passthrough rather than an invention.
 */
function updateSchemaFor(surface: Surface) {
  const countField = surface.legacy ? "sessionsPerTarget" : "iterations";
  const noun = surface.legacy ? "journey" : "goal";
  return z
    .strictObject({
      name: z.string().trim().min(1).max(200).optional(),
      ...
      serverAttachmentId: z
        ...
        .optional(),
      hostIds: z.array(z.string().min(1)).optional(),
      [countField]: iterationsField.optional(),
      maxTurns: maxTurnsField.optional(),
      setupWrites: z.boolean().optional(),
    })
    .refine((value) => Object.keys(value).length > 0, {
      message: `Provide at least one ${noun} field to update.`,
    })
    .refine(
      (value) => {
        const iterations = iterationsOf(value);
        return (
      ...
      {
        // Convex takes `config` as one object, so a partial update would have
        // to read-modify-write it — and a concurrent edit between the read and
        // the write would be silently clobbered. Requiring both is a 400 the
        // caller can fix, rather than a lost update they never see.
        message: `${countField} and maxTurns must be updated together; setupWrites requires that pair.`,
      },
    );
}

const UPDATE_SCHEMAS = {
  canonical: updateSchemaFor(CANONICAL),
  legacy: updateSchemaFor(LEGACY),
};
...
function launchSchemaFor(surface: Surface) {
  return z.object({
    ...
    [surface.legacy ? "waveId" : "swarmRunId"]: z
      ...
      .optional(),
    /**
     * Fan the goal out across these project environments instead of its
     * authored targets. Shape-checked here; the backend does the real
     * validation (live, in-project, duplicate-free, capped) inside the launch
     * transaction, because only it can see the project's environments.
     */
    environmentIds: z
      .array(z.string().min(1))
      // NON-EMPTY when present. `[]` is a caller saying "fan out across
      // these", naming none — and the route would have dropped it and
      // launched the goal's AUTHORED targets instead. Silently running
      // something other than what was asked for is bad anywhere; on an
      // operation that spends, it is worse. Omit the field to mean "as
      // authored".
...
const LAUNCH_SCHEMAS = {
  canonical: launchSchemaFor(CANONICAL),
  legacy: launchSchemaFor(LEGACY),
};
 ...
 */
async function requireGoalInProject(
  ...
  if (!row) {
    ...
    // alias's message keeps working; one reading the canonical surface is not
    // told about a resource it has no name for.
    throw new WebRouteError(
      404,
      ErrorCode.NOT_FOUND,
      surface.legacy ? "Journey not found" : "Goal not found",
    );
  }
  return row;
}
...
async function requireRunInProject(
  ...
  if (!run || String(run.projectId) !== projectId) {
    throw new WebRouteError(
      404,
      ErrorCode.NOT_FOUND,
      surface.legacy ? "Journey run not found" : "Goal run not found",
    );
  }
  return run;
}
...
// GET /v1/projects/:projectId/goals   (alias: /journeys)
both(
  "get",
  ...
  async (c, surface) => {
    ...
    return v1PageJson(
      c,
      (rows ?? []).map((row) => toGoalDto(row, surface)),
    );
  },
);

// GET /v1/projects/:projectId/goals/:goalId   (alias: /journeys/:journeyId)
//
// The scoping preflight every other by-id route already runs, exposed as the
// route it always was. `requireGoalInProject` asks Convex for exactly this,
...
both(
  ...
  async (c, surface) => {
    ...
    return v1Resource(
      c,
      toGoalDto(
        await requireGoalInProject(
          client,
          projectId,
          goalIdParam(c, surface),
          surface,
        ),
        surface,
      ),
...
// POST /v1/projects/:projectId/goals   (alias: /journeys)
//
...
// the caller sent it, for that reason. The surface a caller used is NOT part
// of the fingerprint: the same create through either path dedupes against the
// other, which is the point of the alias.
//
...
both(
  ...
  async (c, surface) => {
    ...
    const body = await parseJsonBody(
      c,
      surface.legacy ? CREATE_SCHEMAS.legacy : CREATE_SCHEMAS.canonical,
    );
    const client = createConvexClient(await getConvexBearerForRequest(c));
    const idempotencyKey = c.req.header("idempotency-key")?.trim();

    // SCOPE THE REFERENCES, not just the path. `journeys:createJourney`
    // resolves `personaRefId` and `swarmRefId` on its own and checks
    // membership of whatever project they turn out to live in — so without
    // this a member of two projects could create a goal in A that runs B's
    // persona, and the header of this file promises the gateway does exactly
    // this check.
    ...
    try {
      row = (await client.mutation(
        "journeys:createJourney" as never,
        {
          ...
          // `hostIds` is REQUIRED upstream (legacy compatibility data, unread
          // by environment-based resolution) and an empty array is the honest
          // value for an environment-based goal, which is how everything
          // authored through this API is expected to run.
          ...
          config: {
            sessionsPerTarget: iterationsOf(body),
      ...
      throw translateConvexWriteError(error, {
        resource: surface.legacy ? "Journey" : "Goal",
      });
    }

    return v1Resource(c, toGoalDto(row, surface), 201);
  },
);

// PATCH /v1/projects/:projectId/goals/:goalId   (alias: /journeys/:journeyId)
//
// `null` CLEARS a field (see `updateSchemaFor`); an absent field is left
// alone. Editing a goal does not touch runs already in flight — the launch
...
both(
  ...
  async (c, surface) => {
    const projectId = c.req.param("projectId");
    const goalId = goalIdParam(c, surface);
    const body = await parseJsonBody(
      c,
      surface.legacy ? UPDATE_SCHEMAS.legacy : UPDATE_SCHEMAS.canonical,
    );
    const client = createConvexClient(await getConvexBearerForRequest(c));
    await requireGoalInProject(client, projectId, goalId, surface);

    const iterations = iterationsOf(body);
    let row: JourneyRow;
    try {
      row = (await client.mutation(
        "journeys:updateJourney" as never,
        {
          ...
          // `null` is forwarded VERBATIM — it is the clear signal upstream
          // too, so collapsing it to `undefined` here would turn "stop using
          ...
          ...(body.environmentIds !== undefined
            ? { environmentIds: body.environmentIds }
            : {}),
          ...(iterations !== undefined && body.maxTurns !== undefined
            ? {
                config: {
    ...
    return v1Resource(c, toGoalDto(row, surface));
  },
);

// DELETE /v1/projects/:projectId/goals/:goalId   (alias: /journeys/:journeyId)
//
// ARCHIVES the goal. It leaves the roster and cannot be launched again, but
// its runs, sessions and transcripts stay readable — deleting the results of
// work that already happened is not what anyone means by "remove this goal
...
both(
  ...
  async (c, surface) => {
    ...
    return v1Resource(c, { id: goalId, projectId, archived: true });
  },
);

// GET /v1/projects/:projectId/goals/:goalId/runs
//   (alias: /journeys/:journeyId/runs)
both(
  "get",
  "/projects/:projectId/goals/:goalId/runs",
  ...
  async (c, surface) => {
    ...
    return v1PageJson(
      c,
      result.page.map((row) => toGoalRunDto(row, surface)),
      nextCursorFrom(result),
    );
  },
);

// GET /v1/projects/:projectId/goal-runs/:runId   (alias: /journey-runs/:runId)
both(
  "get",
  "/projects/:projectId/goal-runs/:runId",
  "/projects/:projectId/journey-runs/:runId",
  async (c, surface) => {
    const projectId = c.req.param("projectId");
    const runId = c.req.param("runId");
    const client = createConvexClient(await getConvexBearerForRequest(c));
    const run = await requireRunInProject(client, projectId, runId, surface);

    // The common insights envelope, DETAIL only (lists stay compact) —
    // resolved through the run's swarm run; runHealth rides beside findings,
    // never inside them. Load failure omits the field rather than failing the
    // read.
    ...
    return v1Resource(c, {
      ...toGoalRunDto(run, surface),
      ...(insights ? { insights } : {}),
    });
  },
);

// GET /v1/projects/:projectId/goal-runs/:runId/sessions
//   (alias: /journey-runs/:runId/sessions)
both(
  "get",
  "/projects/:projectId/goal-runs/:runId/sessions",
  "/projects/:projectId/journey-runs/:runId/sessions",
  async (c, surface) => {
    const projectId = c.req.param("projectId");
    const runId = c.req.param("runId");
    const client = createConvexClient(await getConvexBearerForRequest(c));
    ...
    return v1PageJson(
      ...
      result.page.map((row) =>
        toGoalSessionDto(
          row,
          surface,
          outcomeByChatSessionId.get(row.chatSessionId),
        ),
      ),
      nextCursorFrom(result),
    );
...
both(
  "post",
  "/projects/:projectId/goal-runs/:runId/cancel",
  "/projects/:projectId/journey-runs/:runId/cancel",
  async (c, surface) => {
    const projectId = c.req.param("projectId");
    const runId = c.req.param("runId");
    const client = createConvexClient(await getConvexBearerForRequest(c));
    // Same preflight as the reads: `cancelJourneyRun` checks membership but
    // not the project in the path, so without this a member of two projects
    // could cancel B's run through A's URL.
    await requireRunInProject(client, projectId, runId, surface);
    ...
    try {
      ...
      throw translateConvexWriteError(error, {
        resource: surface.legacy ? "Journey run" : "Goal run",
      });
    }
...
// POST /v1/projects/:projectId/goals/:goalId/runs
//   (alias: /journeys/:journeyId/runs)
//
// Launch a goal. Answers **202** with the run id and returns immediately —
// a fan-out can run for hours, so there is nothing useful to wait for. Poll
// `GET /goal-runs/:runId` or stop it with `POST .../cancel`.
//
// IDEMPOTENT, and it has to be: a launch spends model credits, so a retried
// request that got a dropped response must not run the goal twice. The key
...
both(
  ...
  async (c, surface) => {
    ...
    const goalId = goalIdParam(c, surface);
    const client = createConvexClient(await getConvexBearerForRequest(c));
    // Same scoping preflight as the reads, and it matters MORE here: the
    // launch resolves the project from the goal itself, so without this a
    // member of two projects could launch B's goal through A's URL and be
    // billed under a project the URL never named.
    await requireGoalInProject(client, projectId, goalId, surface);
    ...
    // client, and `?? {}` would have quietly turned it into a launch with
    // every option discarded. Let the schema reject it.
    const parsed = (
      surface.legacy ? LAUNCH_SCHEMAS.legacy : LAUNCH_SCHEMAS.canonical
    ...
    try {
      ...
      // `launchJourneyRun` already throws `WebRouteError` for the
      // caller-facing cases; this catches the Convex-shaped ones underneath —
      // above all the beta gate's FEATURE_UNAVAILABLE, which must not collapse
      // into the generic 404 (that would tell a flagged-off customer their
      // goal does not exist).
    ...
    return v1Resource(
      ...
      {
        id: result.runId,
        [surface.legacy ? "journeyId" : "goalId"]: goalId,
        projectId,
        status: "running",
        /**
         * True when an idempotency key replayed onto a run that already
         * exists. The response is otherwise identical, so a caller retrying a
         * dropped request can tell "I launched it" from "it was already going"
